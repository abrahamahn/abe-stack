...
- Database code → successfully moved to `packages/db`
- Storage code → successfully moved to `packages/storage`
- **8 storage test files → accidentally deleted** (not moved)

#### Storage Test Recovery & Migration

**Recovered from git history**:
1. `factory.test.ts` → `packages/storage/src/factory.test.ts`
2. `localStorageProvider.test.ts` → `packages/storage/src/providers/local.test.ts`
3. `s3StorageProvider.test.ts` → `packages/storage/src/providers/s3.test.ts`
4. `signedUrls.test.ts` → `packages/storage/src/signing.test.ts`
5. `normalizeKey.test.ts` → `packages/storage/src/normalize-key.test.ts`
6. `fastify-server.test.ts` → `packages/storage/src/http/server.test.ts`
7. `helpers.test.ts` → `packages/storage/src/http/helpers.test.ts`
8. `signatures.test.ts` → `packages/storage/src/http/signatures.test.ts`
...
- Added `fastify` as devDependency to `packages/storage`
...
**vitest.config.ts**:
- Added sub-path alias resolution: `@abe-stack/core/*` → `packages/core/src/*`
...
**packages/storage**: 8 test files, 136 tests, **ALL PASSING**
...
**packages/db**: 27 test files, 1,209 tests, **ALL PASSING**
...
Successfully recovered all 8 lost storage test files from git history and migrated them to their correct locations in `packages/storage/src/`. All 136 storage tests now passing. Test suite fully restored after data infrastructure consolidation.

---

### Email Package Extraction (`packages/email`)

**Objective**: Extract email infrastructure from `apps/server/src/infrastructure/messaging/email/` into `packages/email`.
...
### Security Package Extraction (`packages/security`)

**Objective**: Extract security infrastructure from `apps/server/src/infrastructure/security/` into `packages/security`.
...
### Jobs Package Extraction (`packages/jobs`)

**Objective**: Extract background job infrastructure from `apps/server/src/infrastructure/jobs/` into `packages/jobs`.
...
1. **`apps/server/package.json`**: Added `@abe-stack/email`, `@abe-stack/http`, `@abe-stack/jobs`, `@abe-stack/security` as workspace dependencies
2. **`apps/server/tsconfig.json`**: Added path aliases and project references for all 4 new packages
3. **`apps/server/src/infrastructure/index.ts`**: Updated 7 export sections:
...
4. **`packages/jobs/src/index.ts`**: Added 5 missing queue type exports (`JobDetails`, `JobListOptions`, `JobListResult`, `JobStatus`, `QueueStats`)
...
### HTTP Package Extraction (`packages/http`)

**Objective**: Extract HTTP infrastructure from `apps/server/src/infrastructure/http/` into a standalone, framework-agnostic `packages/http` package.
...
- **`eslint.config.ts`**: Added `packages/http` and `packages/email` entries for TypeScript project parsing
- **`tsconfig.json`**: Added missing package references (`packages/billing`, `packages/cache`, `packages/email`, `packages/http`, `packages/security`, `packages/storage`)
...
### Logger Infrastructure Expansion (`packages/core`)

**Objective**: Expand `packages/core/src/infrastructure/logger/` with framework-agnostic logger types and utility functions extracted from `apps/server/src/infrastructure/monitor/logger/`.
...
4. **`packages/core/src/index.ts`** - Updated main barrel:
...
5. **`packages/core/src/config/types/infra.ts`** - Updated import from `LogLevel as ConsoleLogLevel` to `ConsoleLogLevel`
...
### Notifications Package Extraction (`packages/notifications`)

**Objective**: Extract notification infrastructure from `apps/server` into a standalone `packages/notifications` package.
...
- **`eslint.config.ts`**: Added `packages/notifications` TypeScript project entry
- **`tsconfig.json`**: Added `packages/notifications` to root project references
...
**Objective**: Expand `packages/billing` from provider-only (Stripe/PayPal provider wrappers) to include billing service logic, HTTP handlers, route definitions, and webhook processing -- completing the migration from `apps/server/src/modules/billing/`.
...
### Auth Package Extraction (`packages/auth`)

**Objective**: Extract auth module from `apps/server/src/modules/auth/` and `apps/server/src/config/auth/` into a standalone `packages/auth` package as Phase 2A Step 1 of the server decomposition plan.
...
- **`packages/auth/package.json`**: Dependencies on `@abe-stack/core`, `@abe-stack/db`, `@abe-stack/http`, `@abe-stack/security`, `argon2`. Fastify as optional peer dependency.
- **`packages/auth/tsconfig.json`**: Extends base config with paths and references for all 4 dependency packages.
- **`eslint.config.ts`**: Added `packages/auth` TypeScript project entry.
- **`tsconfig.json`**: Added `packages/auth` to root project references.
...
### Users Package Extraction (`packages/users`)

**Objective**: Extract user management module from `apps/server/src/modules/users/` into a standalone `packages/users` package as Phase 2A of the server decomposition plan.
...
- **`eslint.config.ts`**: Added `packages/users` TypeScript project entry
- **`tsconfig.json`**: Added `packages/users` to root project references
...
### 2026-01-30 (Thu) -- Phase 3B: Expand `packages/db` with Scripts, Search, and Config

#### Summary

Expanded `packages/db` by extracting database scripts, SQL search provider, and database configuration from `apps/server/` into the package. This continues the server decomposition plan, moving infrastructure concerns into reusable, framework-agnostic packages.

#### New Directories and Files

**`packages/db/src/scripts/`** -- Database CLI scripts (seed, schema push, admin bootstrap):

- `db-push.ts` -- Schema push with 11 CREATE TABLE + 12 CREATE INDEX statements. Exports `pushSchema()` and `getSchemaStatements()`.
- `seed.ts` -- Database seeding with 4 test users. Exports `seed()`, `TEST_USERS`, types `PasswordHasher` and `SeedUser`.
- `bootstrap-admin.ts` -- Production admin user creation with cryptographically secure password generation. Exports `bootstrapAdmin()`.
- `index.ts` -- Barrel exports for scripts module.

**`packages/db/src/search/`** -- SQL search provider:

- `types.ts` -- Search provider type definitions (`ServerSearchProvider`, `SearchContext`, `SqlSearchProviderConfig`, `SqlTableConfig`, etc.). Re-exports shared types from `@abe-stack/core`.
- `sql-provider.ts` -- `SqlSearchProvider` class with offset/cursor pagination, faceted search, compound filters, health checks. Exports `createSqlSearchProvider` factory.
- `sql-provider.test.ts` -- 20 tests covering constructor, capabilities, search, filter operators, compound filters, query limits, health check, factory function.
- `index.ts` -- Barrel exports for search module.

**`packages/db/src/config/`** -- Database configuration loader:
...
- `packages/db/src/index.ts` -- Added 3 new export sections: Database configuration (5 exports), Scripts (7 exports), Search (20 exports including types).
- `packages/db/package.json` -- Added `@abe-stack/core: "workspace:*"` dependency.
- `packages/db/tsconfig.json` -- Added path aliases for `@abe-stack/core` and project reference to `../core`.
...
2. **Renamed `buildConfigConnectionString`**: The server's `database.ts` exported `buildConnectionString(config: DatabaseConfig)`. The existing `packages/db/src/client.ts` already exports `buildConnectionString()` (from env vars). To avoid naming collision in the barrel, the config-based version was renamed to `buildConfigConnectionString`.

3. **Local type definitions for search**: Search types (`ServerSearchProvider`, `SqlSearchProviderConfig`, etc.) are defined in `packages/db/src/search/types.ts` with re-exports of shared types from `@abe-stack/core`. This keeps the package self-contained while sharing core domain types.
...
### Phase 3A: Realtime Package Extraction (`packages/realtime`)

**Objective**: Extract realtime module from `apps/server/src/modules/realtime/` and `apps/server/src/infrastructure/messaging/websocket/` into a standalone `packages/realtime` package.
...
- **`eslint.config.ts`**: Added `packages/realtime` TypeScript project entry
- **`tsconfig.json`**: Added `packages/realtime` to root project references
...
- Project reference: `{ "path": "../../packages/realtime" }`
...
### Search Infrastructure Migration to `packages/db` (Continued)

**Objective**: Complete migration of remaining search infrastructure from `apps/server/src/infrastructure/search/` into `packages/db/src/search/`.
...
**1. `elasticsearch-provider.ts`** (`packages/db/src/search/elasticsearch-provider.ts`):
...
**2. `search-factory.ts`** (`packages/db/src/search/search-factory.ts`):
...
**3. `elasticsearch-provider.test.ts`** (`packages/db/src/search/elasticsearch-provider.test.ts`):
...
**4. `search-factory.test.ts`** (`packages/db/src/search/search-factory.test.ts`):
...
### Monitor Infrastructure Migration to packages/core

**Objective**: Migrate remaining monitor infrastructure from `apps/server/src/infrastructure/monitor/` into `packages/core/src/infrastructure/` to enable reuse across packages without coupling to Fastify.

#### 1. Base Logger Adapter (`packages/core/src/infrastructure/logger/base-logger.ts`)
...
#### 2. Health Check Infrastructure (`packages/core/src/infrastructure/monitor/`)
...
- `packages/core/src/infrastructure/logger/index.ts` -- Added `BaseLogger`, `createLogger`, `createRequestLogger`, `createJobCorrelationId`, `createJobLogger`
- `packages/core/src/infrastructure/monitor/index.ts` -- New barrel for all health types and functions
- `packages/core/src/index.ts` -- Added all new logger and monitor exports
- `packages/core/package.json` -- Added `./infrastructure/logger` and `./infrastructure/monitor` export paths
...
### Media Infrastructure Migration to packages/media (Phase 2)

**Objective**: Migrate remaining media infrastructure from `apps/server/src/infrastructure/media/` into `packages/media/src/` to complete the media package extraction. Phase 1 had already migrated the foundational files (types, file-type, security, validation, ffmpeg-wrapper, image-processing, audio-metadata). This phase migrates the class-based processors, job queue, retry handler, orchestrator, database adapter, streaming utilities, and facade.

#### 1. Class-Based Processors (`packages/media/src/processors/`)
...
#### 2. Job Queue System (`packages/media/src/queue/`)
...
#### 3. Streaming Utilities (`packages/media/src/utils/`)
...
#### 5. Type Unification (`packages/media/src/types.ts`)
...
#### 6. Updated Barrel Exports (`packages/media/src/index.ts`)
...
- `@abe-stack/auth` -> `packages/auth/src/index.ts`
- `@abe-stack/jobs` -> `packages/jobs/src/index.ts`
- `@abe-stack/users` -> `packages/users/src/index.ts`
...
- `infrastructure/security/` (~20 files) -- migrated to `@abe-stack/security`
- `infrastructure/messaging/email/` (~14 files) -- migrated to `@abe-stack/email`
- `infrastructure/jobs/` (~26 files) -- migrated to `@abe-stack/jobs`
- `infrastructure/utils/` (3 files) -- `dateHelpers.ts` unused, no imports found
- `infrastructure/messaging/index.ts` -- dead barrel (nobody imported it; `infrastructure/index.ts` imports directly from `./messaging/websocket`)

**Module files (equivalents exist in packages):**
- `modules/auth/security/audit.ts` + test -- equivalent in `packages/auth/src/security/audit.ts`
- `modules/users/sessions.service.ts` + test -- equivalent in `packages/users/src/handlers/sessions.ts`
- `modules/users/profile.service.ts` + test -- equivalent in `packages/users/src/handlers/avatar.ts`
...
**Objective**: Define a shared `BaseContext` contract in `packages/contracts` that all packages code against, eliminating 5+ duplicate Logger, RequestInfo, and context interface definitions. The server provides the implementation via TypeScript structural subtyping -- zero casting.

#### Phase 0: Define the Contract

**`packages/contracts/src/types.ts`** -- Expanded `Logger` interface with Pino-compatible overloads:
...
**`packages/contracts/src/context.ts`** (NEW) -- Core shared context types:
...
#### Phase 1: Update packages/http Router Types

- `HandlerContext = BaseContext` (was `Record<string, unknown>`)
- Added `@abe-stack/contracts` dependency to `packages/http`
...
|---------|-------------------|--------------|
| `@abe-stack/auth` | `RequestInfo`, `ReplyWithCookies` | `AppContext extends BaseContext` with narrowed `db: DbClient`, `repos: Repositories`, `log: AuthLogger` |
| `@abe-stack/users` | `UsersLogger`, `UsersRequestInfo`, `UsersRequest` | `UsersModuleDeps extends Omit<BaseContext, 'db'>` |
| `@abe-stack/billing` | `BillingLogger`, `BillingRequest` | `BillingAppContext extends BaseContext` with narrowed repos/log |
...
| File | Purpose |
|------|---------|
| `docs/quickstart/README.md` | Index linking to all four guides |
| `docs/quickstart/monorepo.md` | Full-stack setup: prerequisites, clone, env, database, `pnpm dev`, Docker alternative |
| `docs/quickstart/web.md` | Web frontend: ClientEnvironment pattern, path aliases, API integration, routes |
| `docs/quickstart/server.md` | Backend: database setup, API endpoints, architecture overview, provider switches |
| `docs/quickstart/desktop.md` | Electron: port detection, IPC channels, build/package, platform notes |

#### Documentation Updates

- Updated `docs/README.md` navigation table, directory tree, reading paths, and keyword routing to include quickstart section
- Marked TODO item as complete in `docs/todo/TODO.md`
...
| Section | Content |
|---------|---------|
| Database Migrations | Production migration workflow, zero-downtime strategy, pre-migration checklist, rollback procedures (references `docs/deploy/migrations.md` for full detail) |
...
- Marked TODO item as complete in `docs/todo/TODO.md`
- Added `docs/dev/security.md` entry to weekly log
