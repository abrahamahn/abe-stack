...
> **Summary**: Hexagonal architecture package extraction — decomposed `apps/server` into 10+ standalone packages. ~26,000+ lines removed. Shared `BaseContext` contract eliminates duplicate type definitions. 3,000+ new tests. Directory restructure (`packages/` → `infra/`, `modules/`, `shared/`, `client/`). SDK rename (`@bslt/sdk` → `@bslt/engine`). Config migration to packages. Feature flags for modular route gating. 2FA/TOTP + email change flows. UI accessibility (ErrorBoundary, focus management, toast tones). Integration tests for auth API routes. Production Postgres docs, operations manual, security decision documentation, quickstart guides per profile. All migration phases P0–P7 complete.
...
- `@bslt/stores`: 13 `no-confusing-void-expression` errors — converted arrow shorthand to block syntax
- `@bslt/shared`: 27 `no-unnecessary-boolean-literal-compare` + 5 unused `eslint-disable` directives
...
**Barrel cleanup**: Removed re-exports from `@bslt/shared` in `apps/server/main/shared/index.ts`. Updated ~25 consumer files to import errors directly. Golden standard: barrels export only local items.

### Error Counts After Test Exclusion

| Package           | Lint | Type | Total |
| ----------------- | ---- | ---- | ----- |
| @bslt/server | 196  | 262  | 458   |
| @bslt/engine | 93   | 72   | 165   |
| @bslt/web    | 148  | 5    | 153   |
| @bslt/shared | 16   | 1    | 17    |

Passing packages (0 errors): @bslt/shared, @bslt/infra, @bslt/ui, @bslt/stores, @bslt/media, @bslt/desktop
...
Updated mock paths, type imports (changed from `@bslt/shared` → local `./types`), and added vitest sub-path alias resolution (`@bslt/shared/*` → `shared/core/src/*`). All 136 storage tests passing.
...
**Decision**: Scripts accept `PasswordHasher` callback to avoid circular dependency on `@bslt/auth`.
...
| -------------- | -------------------------- | -------------------- |
| Email          | `'./messaging/email'`      | `'@bslt/infra'` |
| Write Service  | `'./jobs/write'`           | `'@bslt/infra'` |
| Rate Limiting  | `'./security/rate-limit'`  | `'@bslt/infra'` |
| JWT Rotation   | `'./security/crypto'`      | `'@bslt/infra'` |
| Queue          | `'./jobs/queue'`           | `'@bslt/infra'` |
| Scheduled Jobs | `'./jobs/scheduled'`       | `'@bslt/infra'` |
| Permissions    | `'./security/permissions'` | `'@bslt/infra'` |
...
Server logger rewired: pure functions from `@bslt/shared`, Fastify-specific `createLogger`/`createRequestLogger` kept local. Test deduplication: 25 → 10 server logger tests (pure function tests now in core: 30 tests).
...
1. **Import transformation (hexagonal decoupling)**: `@/infrastructure` decomposed to `@bslt/infra` (DB, transactions), `@bslt/infra` (rate limiting), local `./types` (narrow interfaces). `@shared` decomposed to `@bslt/shared` (error mapping) + local types.
...
**Phase 2B**: Auth security functions (lockout, events) rewired to `@bslt/auth`. Route maps kept local due to `AppContext` vs `HandlerContext` incompatibility — local modules serve as adapter layer.

**Phase 3C**: Added `@bslt/realtime` as dependency. WebSocket exports remain local due to function signature difference (2-arg local vs 3-arg package with `TokenVerifier` injection) and shared mutable state.

**Phase 4**: Rewired remaining local imports across 19+ source files and 16+ test files:

- `@auth/*` → `@bslt/auth` (hashPassword, verifyPassword, security events, lockout)
- `@users/*` → `@bslt/users` (getUserById, listUsers)
- `@/infrastructure` → `@bslt/infra` (DB types, email, jobs, storage)
- `@monitor/logger` → `@bslt/shared` (Logger type — compatible since files only use info/warn/error/debug)
- `@http` → `@bslt/infra` (validateCsrfToken)
...
| -------------------------- | ------------------------------------------------- | --------------------------------------------------- |
| `@bslt/auth`          | `RequestInfo`, `ReplyWithCookies`                 | `AppContext extends BaseContext`                    |
| `@bslt/users`         | `UsersLogger`, `UsersRequestInfo`, `UsersRequest` | `UsersModuleDeps extends Omit<BaseContext, 'db'>`   |
| `@bslt/billing`       | `BillingLogger`, `BillingRequest`                 | `BillingAppContext extends BaseContext`             |
| `@bslt/realtime`      | `RealtimeLogger`, `RealtimeRequest`               | `RealtimeModuleDeps extends Omit<BaseContext, ...>` |
| `@bslt/notifications` | `NotificationLogger`, `NotificationRequest`       | `NotificationModuleDeps extends BaseContext`        |
...
- Wired `AuthEmailTemplates` from `@bslt/auth` into `IServiceContainer`
- Switched route imports (`authRoutes`, `notificationRoutes`, `realtimeRoutes`) to `@bslt/*` packages directly
...
1,050+ files changed across workspace config, tsconfig, turbo, CI workflows, scripts, and source. Package names unchanged (`@bslt/*`).

### SDK Rename: `@bslt/sdk` → `@bslt/engine`
...
| Dependencies         | 2     | `@bslt/sdk` → `@bslt/engine`   |
...
Fixed stale pnpm symlink (`@bslt/sdk -> ../../../../sdk`) causing old directory recreation.

### Stores Package Relocation

Moved `@bslt/stores` from `infra/stores/` to `client/stores/` — package has React peer dependency, better fit alongside SDK layer. 26 files updated. Added `'sdk/*'` to `pnpm-workspace.yaml` for nested package discovery.
...
Deleted dead `infra/search/` (5 files duplicated in `@bslt/infra`). Removed 6 stale tsconfig aliases, added 3 new (`@health`, `@logger`, `@routes`). 8 barrel files → 3.
...
Moved `bootstrap-admin`, `seed`, `db-push` from server to `tools/scripts/db/`. Decoupled from `@/config` — uses `DEFAULT_ARGON2_CONFIG` from `@bslt/auth` directly. Inlined `UserRole` type to avoid pulling `@bslt/shared` for a single union type. 94 tests passing.

### P1 Cache Service Migration

Replaced server-local sync `CacheService` (133 LOC simple TTL Map) with `@bslt/cache` `MemoryCacheProvider`:
...
- **Updated `shared/types.ts`**: `IServiceContainer.cache` type from `import('@bslt/cache').CacheProvider`
...
| P1    | Cache → `@bslt/cache`               | Completed |
| P2    | HTTP middleware → `@bslt/infra`     | Completed |
| P3    | Pagination → `@bslt/infra`          | Completed |
| P4    | Router types → `@bslt/infra`        | Completed |
| P5    | Request utils → `@bslt/infra`       | Completed |
...
- 5-phase migration plan targeting `@bslt/infra` and `@bslt/cache`
...
| Router type divergence            | Server uses concrete `AppContext`, `@bslt/infra` uses generic `BaseContext` |
...
Verified `@bslt/shared` contains only domain-agnostic primitives and contracts:
...
**Dead notification code removal**: Deleted 4 duplicate provider files from `infrastructure/notifications/` (exact copies of `@bslt/notifications`). Updated `app.ts` to import `createNotificationProviderService` directly from package. Fixed factory name inconsistency (`createNotificationService` → `createNotificationProviderService`).
...
| `utils/request-utils.ts`         | Delete (future)   | Dead code — zero imports, superseded by `@bslt/infra` router |
