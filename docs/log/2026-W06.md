...
>
> - **Monorepo restructure**: `packages/` + `client/` + `apps/` → unified `src/` hierarchy; 5 packages consolidated into `@bslt/core`; full build green (42/42 build + 13/13 test tasks)
> - **`@bslt/db`**: Functional repository refactor (11 repos, 213 tests), Phase 2 expansion (16 repos, 615 tests), schema completion (7 files, 554 tests), package reorg (`write/` → `queue/`, routing to engine), 4 new schema modules (files, email, tenant-settings, activities), build toolchain cleanup (tsup → tsc)
> - **`@bslt/shared`**: Zod removal (23 files → manual `createSchema`), barrel export cleanup (all `export *` eliminated), 4 rounds of deep security audits (SQL injection, ReDoS, timing leaks, `as` cast removal, crypto hardening — 106 test files, 3,580 tests), contracts → domain+core migration, utils reorg, +1,002 tests across 15 new files
> - **`@bslt/server-system`**: 8-phase refactor, comprehensive 6-workstream audit (JWT timing fix, SQL injection parameterization, SMTP socket leak fix, O(1) rate limiter, dead router deletion — 32 new tests)
> - **`@bslt/media`**: 4-phase refactor (remove fluent-ffmpeg/music-metadata, memory safety, entitlement gating), 3 rounds of security audit (FD leaks, buffer bounds, FFmpeg filter injection, dimension validation, unbounded queue, crypto file IDs — 331 tests)
> - **`@bslt/web`**: HomePage rebuild (resizable panel layout, 24 files), demo → ui-library rename, config consolidation
...
- [Wednesday (02-04)](#wednesday-february-4-2026) — Domain module expansion (4 modules, 138 tests), shared verification (86/86 files), HomePage rebuild (24 files), barrel export cleanup, Zod removal, server-system 8-phase refactor, module consolidation (5 → core), `modules/` → `packages/core/` move
...
- [Sunday (02-08)](#sunday-february-8-2026) — Media 4-phase refactor + 3 security audit rounds (331 tests), server-system comprehensive audit (6 work streams, 32 new tests), `main/tools/` audit (CJS elimination, ESM conversion), auth flow Part 1 completion (email change page, TOTP backup hint, 4 test files / 162 tests)
...
- **HIGH**: `@bslt/shared/utils` import fails at runtime (inlined conversion functions); `utils.test.ts` called wrong functions; factory test asserted wrong key count
...
| ------------------------ | -------------------------- | ----------- |
| `@bslt/core/config` | `@bslt/shared/config` | 32          |
| `@bslt/core` (bare) | `@bslt/shared`        | 12          |
| `@bslt/infra`       | `@bslt/db`            | 161         |
...
**Verification**: 141 tests passing across 10 test suites. Zero remaining `@bslt/shared/config` imports.
...
**Config helpers**: Consolidated triple-duplicated `isStrategyEnabled`/`getRefreshCookieOptions` (in `modules/auth/src/utils/`, `apps/server/src/modules/auth/utils/`, and `apps/server/src/config/auth/`) into single canonical file at `packages/engine/src/config/auth-helpers.ts`. All consumers now import from `@bslt/server-system/config`. Deleted 4 duplicate files. 12 tests passing.
...
Key decision: `getDetailedHealth()` and `logStartupSummary()` accept optional `WebSocketStats` parameter (dependency injection). This prevents engine (Tier 2) from importing `@bslt/realtime` (Tier 3+ premium module). The app-level caller (`apps/server`) passes the stats in. Created 4 files, 24 tests passing.
...
- **Re-export cleanup**: Removed `@bslt/db` re-exports from engine barrel
...
**Root cause**: tsconfig misconfiguration — `search/search-factory.ts` imported from `../../migrations/index` (outside src/) violating `rootDir: "./src"`. The `composite: false` setting prevented project references, and there was no reference to `@bslt/shared`.
...
4. Added project reference to `@bslt/shared`
...
### Module Consolidation: 5 Packages → `@bslt/core`

Merged `modules/admin`, `modules/auth`, `modules/billing`, `modules/notifications`, `modules/users` into single `@bslt/core` with subpath exports (`@bslt/core/auth`, `@bslt/core/admin`, etc.). The 5 packages were tightly coupled (admin imports from auth and billing, users imports from auth) but maintained as separate workspace packages requiring inter-package dependency declarations.
...
Then moved filesystem from `modules/` to `packages/core/` for consistent naming. Zero import changes needed since all imports use `@bslt/core` (resolved by pnpm workspace). Updated `pnpm-workspace.yaml`, `tsconfig.json` (replaced 6 broken individual module refs with 1), `eslint.config.ts` (9 changes + deleted stale realtime block), and 149 file headers.
...
src/
  apps/
    web/          → @bslt/web
    server/       → @bslt/server
    desktop/      → @bslt/desktop
  client/
    api/          → @bslt/api
    engine/       → @bslt/client-engine
    react/        → @bslt/react
    ui/           → @bslt/ui
  server/
    core/         → @bslt/core
    db/           → @bslt/db
    engine/       → @bslt/server-system
    media/        → @bslt/media
    realtime/     → @bslt/realtime
    websocket/    → @bslt/websocket
  shared/         → @bslt/shared
```
...
### `@bslt/db` Package Reorganization
...
3. **Removed `rate-limiter.ts`** — in-memory `RateLimiter` had no DB interaction; server-system already had compatible Token Bucket with superset interface. Updated 3 consumers
4. **Moved `routing.ts` to server-system** — Fastify-specific routing (route registration, auth guards) belongs in infrastructure adapter, not DB package. Created `src/routing/` module in server-system. Updated 5 core route files + 4 test files. Added `zod` dependency for `RouteSchema` union type
...
Migrated `@bslt/db` and `@bslt/shared` from `tsup` bundling to `tsc --build` + `ensure-js-extensions`. Removed dual CJS/ESM output (monorepo is ESM-only). Removed unused `fastify`/`zod` devDependencies from db.
...
- Removed duplicate `port.ts` from server-system
...
### `main/server/system` Comprehensive Audit (6 Work Streams, 32 New Tests)
...
All 5 test files passing. 0 type errors in `@bslt/server-system` and `@bslt/server`.
...
4-phase refactor of `@bslt/media`:
...
**User-agent labeling**: Added UA parser + tests, stored `device_name` and `device_type` in sessions. Migration `0019_user_sessions_device_fields.sql`. Aligned `@bslt/db` session table exports for core usage.
...
### Complete Zod Removal from `@bslt/server-system`

Removed the last `zod` dependency in the entire monorepo. The `@bslt/server-system` package had a type-only import (`import type { ZodType } from 'zod'`) in `RouteSchema` union. Since `hasSafeParse()` uses duck-typing (checks for `safeParse` method structurally), the `ZodType` in the union was redundant.
