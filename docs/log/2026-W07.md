...
- Updated `BUSINESS.md` with Sprint 1 & 2 completion status: IAM (Social Login, Sessions, MFA done), Tenancy (Invitations, Members, Permission Gating done), Admin Console and Compliance items marked in-progress
...
- **SessionsList.test.tsx** (9 tests): Added `Heading` mock — component started using `Heading` but test mock only had `Alert`, `Button`, `Skeleton`, `Text`
...
- **PlanManagementPage.test.tsx** (3 tests): Added `Heading` mock — same pattern as SessionsList
- **RouteManifestPage.test.tsx** (2 tests): Added `Checkbox` mock — component uses `Checkbox` in filter UI, test had `Heading` but missed `Checkbox`
...
### Sprint 4 TODO: Comprehensive Checklist Creation

- Expanded Sprint 4 section in `docs/todo/TODO.md` from 6 high-level items to 21 detailed sections (4.1–4.21)
- Sections 4.1–4.14: test backfill for each business domain (auth, sessions, accounts, multi-tenant, RBAC, billing, notifications, audit, compliance, realtime, media, API keys, admin)
- Sections 4.15–4.18: operational quality, blind spot verification, scheduled jobs, client engine
- Each section lists unit tests (colocated), integration tests (`apps/server`), E2E tests (`apps/web/e2e`)
- Cross-referenced to CHECKLIST.md sections 10, 11, 14, Appendix C and BUSINESS.md section 8

### Sprint 3 & 4 Audit: Round 1 Gap Fixes

- Audited Sprint 3 and Sprint 4 against CHECKLIST.md, BUSINESS.md, EXECUTION.md
- Sprint 3 fixes: deduplicated 3.16/3.18 cleanup jobs, fixed 3.11 TOTP QR wording, added BUSINESS Section 2 to cross-ref table
- Sprint 4 fixes: added TOTP_REQUIRED/TOTP_INVALID failure reasons (4.16), API key security notification (4.16), correlation ID tests (4.15), job monitor E2E (4.17)
- Added 3 new Sprint 4 sections: 4.19 (Activity + Flags + Metering), 4.20 (Webhook Delivery), 4.21 (Desktop App)
- Updated Sprint 4 cross-reference table with new sections

### Swagger UI Theme: ABE Stack Design System Alignment

- Rewrote `src/apps/server/src/http/swagger-theme.ts` to precisely match design system tokens
- **Topbar**: Changed from bright blue (`#1a73e8`) to neutral surface (`#f1f3f4` bg + `#dadce0` border), matching app header style
- **Border radius**: All values use `0.625rem` matching `--ui-radius-md` (modals use `1rem` = `--ui-radius-lg`)
- **Typography**: Base font size `0.875rem` matching `--ui-font-size-sm`, system-ui font stack, `-webkit-font-smoothing: antialiased`
- **Buttons**: Authorize = outlined primary (like `.btn-secondary` with primary color), Execute = filled primary (like `.btn-primary`), Cancel = secondary surface
- **HTTP method badges**: GET=primary blue, POST=success green, PUT=warning orange, DELETE=danger red, PATCH=teal
- **Inputs**: Border `#dadce0`, focus ring `0 0 0 2px rgba(26, 115, 232, 0.4)` matching `--ui-focus`
- **Dark mode**: Full `@media (prefers-color-scheme: dark)` block using exact tokens from `darkColors` in `colors.ts` (bg=#202124, surface=#292a2d, border=#3c4043, text=#e8eaed, primary=#8ab4f8)
- **Shadows**: Light `0 1px 3px rgba(60, 64, 67, 0.15), 0 1px 2px rgba(60, 64, 67, 0.1)`, Dark `0 1px 3px rgba(0, 0, 0, 0.4), 0 4px 8px rgba(0, 0, 0, 0.25)` — matching `--ui-color-shadow` exactly
- Wired in `server.ts` via `theme: { css: [{ filename: 'theme.css', content: swaggerThemeCss }] }`
- Type-check and lint pass

### Sprint 3 & 4 Audit: Round 2 Gap Fixes

**Sprint 3 HIGH (3 fixes):**

- Deduplicated Sprint 1.3 / Sprint 3.23 (Security Intelligence) — Sprint 1.3 now defers to Sprint 3.23
- Deduplicated Sprint 2.5 / Sprint 3.24 (Phone/SMS 2FA) — Sprint 2.5 now defers to Sprint 3.24
- Added new Sprint 3.25: Webhook Delivery System (BUSINESS 5.3) — registration CRUD, event subscriptions, delivery pipeline with retry + dead-letter

**Sprint 3 MEDIUM (5 fixes):**

- Added clarifying note to Sprint 3.23 distinguishing token version invalidation from Sprint 1.2 concurrent session limits
- Added to Sprint 3.18: idempotent webhook receiving (Appendix D), IP blocklist hooks (E.5), file scanning hooks (E.7), secret rotation docs (E.7), `pnpm db:reset` command (E.6)
- Added to Sprint 3.17: background job E2E verification (BUSINESS 5.2) + job queue lifecycle integration test
- Fixed Sprint 3 cross-ref table: added 3.25 to System Operations row, corrected webhook coverage claim

**Sprint 4 MEDIUM (5 fixes):**

- Fixed cross-ref table mislabels: "Appendix D" and "Appendix E" → correct source sections
- Added deployment sanity tests to Sprint 4.15 (migrations + seed + bootstrap on fresh DB)
- Added rate limiting + IP allowlist/blocklist tests to Sprint 4.16
- Added new Sprint 4.22: Golden Path Onboarding E2E (register → workspace → invite → plan → dashboard)

**Sprint 3+4 LOW (11 fixes):**

- Sprint 3: tenant switcher → verification reference, cron ownership notes, infra slice reduced-protocol note
- Sprint 4: overlap caveat in preamble, job queue lifecycle test (4.17), BUSINESS column for 4.19/4.20, desktop menu/tray tests (4.21), 4.18 redundancy notes, 4.19 splitting note, 3 new cross-ref rows

### Recovery + Checkpoint Commits (Late Session, Feb 11)

- Detected local change loss caused by repeated `reset: moving to HEAD` entries in reflog.
- Recovered full WIP from `stash@{2026-02-11 03:42:32 +0900}` and re-applied to working tree.
- Added safety checkpointing commits to prevent further loss:
...
### Post-Recovery Quality Sweep + Follow-up Commits

- Ran full gates repeatedly (`pnpm lint`, `pnpm type-check`, `pnpm format:check:ci`, `pnpm test`) and fixed drift issues.
- Formatting drift was mainly in:
  - `docs/todo/TODO.md`
  - `docs/log/2026-W07.md`
  - `src/tools/scripts/db/seed.test.ts`
- Committed post-recovery hygiene updates:
...
### Additional Same-Day Hardening Commits

- `0f3cf587` fixed Docker build-context exclusion bug in `infra/docker/Dockerfile.dockerignore` (`**` pattern issue).
- `5f70c493` resolved 12 CodeQL findings across server/client/tools hotspots.
- `3186579d` removed unsafe eslint-disable reliance by fixing root causes in:
  - `src/apps/server/src/config/factory.ts`
  - `src/apps/server/src/verify-storage-path.ts`
  - `src/shared/src/core/schemas.ts`

### Sprint 3 Tier 1: Quick Wins (5 Items)

Implemented 5 quick-win items from Sprint 3 that had most infrastructure in place but specific gaps remaining.

#### 1. Auth-protect Swagger docs in non-dev (3.17)

- Added `onRequest` hook to swagger-ui route in `src/apps/server/src/server.ts`
- In production, returns 401 unless request has valid auth cookie/header
- Uses existing `extractTokenPayload` from auth middleware
- Non-production environments remain publicly accessible

#### 2. Invoice detail route `GET /api/billing/invoices/:id` (3.2)

- Added `handleGetInvoice` handler in `src/server/core/src/billing/handlers.ts`
- Added `getInvoice` service method in `src/server/core/src/billing/service.ts` — verifies invoice ownership
- Added route entry in `src/server/core/src/billing/routes.ts`
- Tests: handler test + service test verifying ownership check and 404

#### 3. Wire `assertEntitled` as Fastify preHandler middleware (3.2)

- Created `src/server/core/src/billing/middleware.ts` — `requireEntitlement(featureName)` factory returning Fastify preHandler
- Resolves user subscription from request context, calls `resolveEntitlements()` + `assertEntitled()` from `@abe-stack/shared/domain`
- Tests: 6 tests covering entitled access, denied access, missing subscription
- Fixed `exactOptionalPropertyTypes` issues with spread pattern for optional fields

#### 4. General `audit.record()` helper + wire to billing events (3.3)

- Created `src/server/core/src/audit/service.ts` — general audit recording with `inferCategory()` auto-categorization from action prefixes
- Created `src/server/core/src/audit/types.ts` — `AuditAction`, `AuditRecordParams`, `AuditDeps` types
- Wired fire-and-forget `tryAudit()` helper into 5 billing mutation handlers (cancel, resume, update subscription, add/remove payment method)
- Made `auditEvents` optional on `BillingAppContext.repos` for backward compat
- Added missing exports (`AuditEvent`, `AuditEventRepository`, etc.) to `@abe-stack/db` index
- Tests: 11 unit tests for audit service

#### 5. Audit retention config + DB cleanup cron (3.3)

- Added `auditRetentionDays` to `ServerConfig` (default 90, 0 = unlimited)
- Added `AUDIT_RETENTION_DAYS` to `ServerEnv` schema
- Added `deleteOlderThan(cutoff)` to `AuditEventRepository` interface + implementation
- Added `cleanupAuditEvents` handler to `infrastructure.ts` queue handlers — deletes events older than retention period
- Added `@abe-stack/shared/domain` subpath alias to server vitest config (fixes ENOTDIR resolution)
- Tests: 3 config tests (default, custom, unlimited) + 2 repository tests
- Updated `auditRetentionDays: 90` in 7+ test files with `ServerConfig` mocks

#### Verification

- All type-checks pass: `@abe-stack/server`, `@abe-stack/shared`, `@abe-stack/db`, `@abe-stack/core`
- All tests pass: db (2997), server (725), core (2049)
- Pre-existing failures: 2 `handleRefresh` tests in core (unrelated to sprint 3 changes)
