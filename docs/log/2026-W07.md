...
- Added/updated Vitest alias mappings across packages (`server/system`, `server/websocket`, `client/react`, `client/ui`) to resolve workspace package entry failures.
...
- Hardened workflow formatting enforcement path:
  ...
  - repaired YAML block indentation regressions in `.github/workflows/infrastructure-test.yml`.
...
- `pnpm test` passed.
- Targeted package lint/type checks for previously failing sets (`client-engine`, `ui`, `react`, `server-system`, `media`, `db`) passed after fixes.
...
- **Root cause:** Provider-specific heredoc blocks in `.github/workflows/infrastructure-test.yml` had `EOF` delimiters indented 2 spaces after YAML dedenting, causing bash syntax errors (exit code 2) in CI.
- **Fix:** Corrected heredoc delimiter indentation to align with the shell block.
- 1 file changed: `.github/workflows/infrastructure-test.yml` (22 lines, 11 insertions / 11 deletions)
...
- Added `VerifyOptions` interface with `clockToleranceSeconds` to `main/server/system/src/security/jwt.ts`
...
- Type-check passes: @bslt/server-system, @bslt/core, @bslt/shared, @bslt/server, @bslt/web, @bslt/ui
- Lint passes for all changed files

#### E4: Phone/SMS 2FA Stub (2.5) — DEFERRED

- Created SMS provider infrastructure in `server/system/src/sms/`:
...
- Added SMS exports to `@bslt/server-system` main index
...
- Created `main/server/system/src/observability/sentry.test.ts` — 14 tests covering:
...
- Created `RedisCacheProvider` in `main/server/system/src/cache/providers/redis.ts` — ioredis-backed implementation satisfying `CacheProvider` interface with JSON serialization, tag-based invalidation via Redis Sets, pipeline for bulk ops, local stats tracking
...
- Installed `ioredis` in `@bslt/server-system`
...
- `main/server/system/src/cache/config.ts` — Redis provider support in env config loader
- `main/server/system/src/cache/factory.ts` — Redis provider selection
- `main/server/system/src/cache/types.ts` — RedisCacheConfig re-export
- `main/server/system/src/cache/providers/index.ts` — RedisCacheProvider exports
- `main/server/system/src/cache/index.ts` — Redis exports
- `main/server/system/package.json` — added ioredis dependency
...
- **Performance**: Added `lighthouse` job to `continuous-integration.yml` for automated Lighthouse CI runs on full builds.
- **Reliability**: Added `smoke-test` job to `application-deploy.yml` to verify `/api/health` 200 OK status after deployment stabilization.
...
- [x] S3 storage provider (`server-system/storage/providers/s3.ts`) compiles
...
- [x] Health endpoints: `server-system/system/health.ts` + `shared/utils/monitor/health.ts` exist and tests pass
...
- [x] `continuous-integration.yml` runs lint, type-check, test successfully
- [x] `application-deploy.yml` workflow is valid
- [x] `security-scanning.yml` scans run
- [x] `dependency-audit.yml` dependency audit runs
- [x] `deployment-rollback.yml` workflow is valid
- [x] `infrastructure-deploy.yml`, `infrastructure-destroy.yml`, `infrastructure-test.yml` workflows valid
...
- [x] Workflow: `staging-deploy.yml` — deploy to staging on merge to `staging` branch (or manually triggered)
- [x] Service: staging-specific env vars (separate DB, separate Stripe test keys)
- [x] Workflow: `pr-preview.yml` — deploy PR branch to temporary preview environment
...
- [x] CI: deployment smoke test — after deploy, automated health check confirms app is live (`application-deploy.yml` smoke-test job)
...
- [x] Setup: automated dependency audit in CI (weekly schedule — dependency-audit.yml daily + security-scanning.yml weekly)
