...
- `src/apps/server/src/types/context.ts` — added `sms` to service container
- `src/apps/server/src/app.ts` — wired SMS provider
- `src/server/core/src/auth/types.ts` — added `sms` to AppContext
- `src/server/core/src/auth/handlers/login.ts` — fingerprint-based device detection
- `src/server/core/src/auth/service.ts` — SMS challenge detection in authenticateUser
- `src/server/core/src/auth/handlers/sms-challenge.ts` — use `ctx.sms` instead of cast
- `src/server/core/src/auth/utils/jwt.ts` — include tokenVersion in access token
- `src/server/core/src/auth/handlers/refresh.ts` — check tokenVersion on refresh
- `src/server/db/src/repositories/users/users.ts` — added incrementTokenVersion
- `src/apps/web/src/features/auth/hooks/useAuth.ts` — added sendSmsCode, verifySmsLogin
- `src/apps/web/src/features/auth/components/AuthForms.tsx` — wired TOTP + SMS callbacks
- `src/apps/web/src/features/auth/components/LoginForm.tsx` — SMS challenge handling
- `src/apps/web/src/features/auth/pages/AuthPage.tsx` — wired SMS callbacks
- `src/apps/web/src/features/auth/pages/LoginPage.tsx` — wired SMS callbacks
- `src/apps/web/src/features/settings/pages/SettingsPage.tsx` — integrated DevicesList + PhoneManagement
- `src/client/api/src/index.ts` — added device + phone exports
...
- `src/server/core/src/auth/types.ts` — added failure reason constants
- `src/server/core/src/auth/service.ts` — added lifecycle check in authenticateUser
- `src/server/core/src/auth/handlers/login.test.ts` — added missing logNewDeviceLogin mock
- `src/apps/server/src/__tests__/integration/auth.integration.test.ts` — added R1 + R3 tests
- `src/apps/server/src/__tests__/integration/test-utils.ts` — fixed argon2 timeCost
...
- Created `RedisCacheProvider` in `src/server/engine/src/cache/providers/redis.ts` — ioredis-backed implementation satisfying `CacheProvider` interface with JSON serialization, tag-based invalidation via Redis Sets, pipeline for bulk ops, local stats tracking
...
- Created `src/server/core/src/cache/` module — `CacheKeys`, `CacheTags`, `CacheTTL` constants, `cacheAside()` generic helper
...
- Created `ReadReplicaClient` in `src/server/db/src/read-replica.ts` — primary/replica pair with read/write aliases, falls back to primary when no replica configured
...
- `src/shared/src/utils/cache/types.ts` — added RedisCacheConfig
- `src/shared/src/utils/cache/index.ts` — added RedisCacheConfig export
- `src/shared/src/index.ts` — added RedisCacheConfig export
- `src/shared/src/config/types/infra.ts` — added externalConfig fields, readReplicaConnectionString
- `src/shared/src/config/env.schema.ts` — added REDIS_PASSWORD, REDIS_DB, DATABASE_READ_REPLICA_URL
- `src/apps/server/src/config/infra/cache.ts` — pass password, db through from env
- `src/apps/server/src/config/infra/database.ts` — load readReplicaConnectionString
- `src/server/engine/src/cache/config.ts` — Redis provider support in env config loader
- `src/server/engine/src/cache/factory.ts` — Redis provider selection
- `src/server/engine/src/cache/types.ts` — RedisCacheConfig re-export
- `src/server/engine/src/cache/providers/index.ts` — RedisCacheProvider exports
- `src/server/engine/src/cache/index.ts` — Redis exports
- `src/server/engine/package.json` — added ioredis dependency
- `src/server/db/src/index.ts` — ReadReplicaClient exports
- `src/apps/server/src/infrastructure.ts` — InfrastructureOptions, onPubSubMessage callback
- `src/apps/server/src/app.ts` — wired pubsub callback + adapter
- `src/server/core/src/users/types.ts` — added optional cache to UsersModuleDeps
- `src/server/core/src/users/handlers/profile.ts` — cache-aside for handleMe
...
- Created `UndoRedoToolbar.tsx` in `@abe-stack/ui` — undo/redo buttons with platform-aware shortcut hints
...
- `src/client/react/src/stores/toastStore.ts` — added `action` to `ToastMessage`
- `src/client/react/src/hooks/index.ts` — barrel exports
- `src/client/react/src/index.ts` — barrel exports
- `src/client/ui/src/components/Toast.tsx` — action button rendering
- `src/client/ui/src/components/Toast.test.tsx` — action tests
- `src/client/ui/src/components/index.ts` — UndoRedoToolbar export
- `src/client/ui/src/index.ts` — UndoRedoToolbar export
- `src/client/ui/src/styles/components.css` — toast-action styles
- `src/apps/web/src/app/layouts/AppTopLayout.tsx` — AppUndoRedo integration
