...
#### Core Pagination Engine (`@bslt/shared`)
...
#### Backend Middleware (`@bslt/server`)
...
Also removed `sync-import-aliases.ts` automation (was causing vitest resolution issues). Updated `tools/dev/start-dev.ts`, `package.json`, `.github/workflows/continuous-integration.yml`.
...
**Test Count Update:** @bslt/server: 857 tests (all passing)
...
Accessible via `@bslt/shared/crypto` (server-only, uses `node:crypto`). Added `NO_PARENT_REEXPORT` to barrel sync to prevent server-only modules from polluting browser bundles.
...
- **Problem:** `shared/core/src/errors.ts` had simple `HttpError` (8 types, no code/details) while `apps/server/main/shared/errors.ts` had rich `AppError` (17+ types with code, details, toJSON())
...
**Updated:** `errorResponseSchema` in contracts now includes error, code, details fields. Server re-exports from `@bslt/shared`.

**Deleted:** Old `shared/core/src/errors.ts`, `apps/server/main/shared/errors.ts`, and associated test file.

**Backward compatible:** `PermissionError` alias for `ForbiddenError`, `RateLimitError` alias for `TooManyRequestsError`.

#### Test Infrastructure Package (`@bslt/tests`)
...
- Fixed package naming: `@abeahn/ui` -> `@bslt/ui`, `@abeahn/shared` -> `@bslt/shared`
...
| Package         | Disposition                                   |
| --------------- | --------------------------------------------- |
| `@bslt/db`      | Merged into `apps/server/src/infra/database/` |
| `@bslt/storage` | Merged into `apps/server/src/infra/storage/`  |
| `@bslt/setup`   | Removed (CLI scaffolding tool, not needed)    |

#### Package Rename

`packages/api-client` renamed to `packages/sdk`. All imports updated across web, desktop, eslint config, and documentation. Package name remains `@bslt/sdk`.
...
Added `@fastify/websocket`, `fastify-plugin`, `@aws-sdk/client-s3`, `@aws-sdk/credential-providers`, `@aws-sdk/s3-request-presigner` to server. Removed `@bslt/db` and `@bslt/storage` workspace references.
