...
### Implemented: Core Features (High Priority)

Completed all three high-priority core features from TODO.md with production-ready implementations.

---

#### 1. Push Notifications (Backend + Frontend)

VAPID-based Web Push implementation with user preferences.

| Package | Files | Tests |
|---------|-------|-------|
| `packages/core/src/domains/notifications/` | types, schemas, errors | 58 |
| `apps/server/src/infrastructure/notifications/` | web-push-provider, fcm-provider (stub), factory | - |
| `apps/server/src/modules/notifications/` | service, handlers, routes | 50 |
| `packages/sdk/src/notifications/` | client, hooks | 23 |

**SDK Hooks:** `usePushSubscription`, `useNotificationPreferences`, `usePushPermission`
...
Memory and Redis providers with LRU eviction and tag-based invalidation.

| Package | Files | Tests |
|---------|-------|-------|
| `packages/core/src/domains/cache/` | types, errors | 48 |
| `apps/server/src/infrastructure/cache/` | memory-provider, redis-provider, factory, memoize | 99 |
...
- Redis: connection pooling, retry logic, graceful fallback

**Total: 16 files, 147 tests**

---

#### 3. Search & Filtering (Backend + Frontend)

Fluent query builder with SQL provider and cursor pagination.

| Package | Files | Tests |
|---------|-------|-------|
| `packages/core/src/domains/search/` | types, schemas, operators, query-builder, errors | 152 |
| `apps/server/src/infrastructure/search/` | sql-provider, elasticsearch-provider (stub), factory | 18 |
| `packages/sdk/src/search/` | query-builder, serialization, hooks | 23 |

**Filter Operators:** `eq`, `neq`, `gt`, `gte`, `lt`, `lte`, `contains`, `startsWith`, `endsWith`, `like`, `ilike`, `in`, `notIn`, `isNull`, `isNotNull`, `between`, `arrayContains`, `fullText`

**SDK Hooks:** `useSearch`, `useInfiniteSearch`, `useSearchParams`, `useDebounceSearch`
...
Completed comprehensive code review of all implemented features.

**Bug Fixed:**
- `packages/core/src/domains/search/operators.ts` - Date comparison operators (`gt`, `gte`, `lt`, `lte`) were rejecting Date objects. Fixed by allowing Date instances before the object type check.

**Review Status:**

| Feature | Status | Key Validations |
|---------|--------|-----------------|
| Push Notifications | ✅ Ready | VAPID auth, batch sending, expired subscription cleanup |
| Cache Layer | ✅ Ready | O(1) LRU, Redis retry logic, tag invalidation |
| Search & Filtering | ✅ Ready | SQL injection safe, cursor pagination, faceted search |
...
**Completed TODO Items (moved to archive):**
- ✅ Push Notifications (Backend + Frontend)
- ✅ Cache Layer (Backend)
- ✅ Search & Filtering (Backend)
- ✅ Magic Links (Backend) - completed earlier
- ✅ Accessibility (Frontend) - ResizablePanel keyboard support
- ✅ Theme (Frontend) - Density variants, high-contrast mode

---

## 2026-01-22

### Added: OAuth Authentication (Google, GitHub, Apple)

Implemented complete OAuth 2.0 authentication flow for three major providers.

**Files Created:**

| Path | Description |
|------|-------------|
| `apps/server/src/modules/auth/oauth/service.ts` | OAuth business logic - authorization URL generation, callback handling, user linking |
| `apps/server/src/modules/auth/oauth/handlers.ts` | HTTP handlers for OAuth endpoints |
| `apps/server/src/modules/auth/oauth/routes.ts` | Route definitions for initiate, callback, link, unlink |
| `apps/server/src/modules/auth/oauth/providers/` | Provider-specific implementations (Google, GitHub, Apple) |
| `apps/server/src/modules/auth/oauth/types.ts` | OAuth types and interfaces |
| `apps/server/src/modules/auth/oauth/__tests__/` | Unit tests for service and providers |
| `packages/core/src/contracts/oauth.ts` | Zod schemas and ts-rest contract |
| `apps/server/src/infrastructure/database/schema/oauth.ts` | Database schema for oauth_connections table |
...
- `POST /api/auth/oauth/:provider/link` - Link OAuth to existing account (authenticated)
- `DELETE /api/auth/oauth/:provider/unlink` - Unlink OAuth provider (authenticated)
- `GET /api/auth/oauth/connections` - Get user's connected providers (authenticated)

**Features:**

- State parameter with HMAC signing for CSRF protection
- Support for account linking (multiple OAuth providers per user)
- Automatic account creation for new OAuth users
- Email verification via OAuth (providers verify emails)
- Token storage for future API calls to providers

---

### Added: Magic Links Authentication

Implemented passwordless authentication via email magic links.

**Files Created:**

| Path | Description |
|------|-------------|
| `apps/server/src/modules/auth/magic-link/service.ts` | Token generation, verification, rate limiting |
| `apps/server/src/modules/auth/magic-link/handlers.ts` | HTTP handlers for request and verify |
| `apps/server/src/modules/auth/magic-link/routes.ts` | Route definitions |
| `apps/server/src/modules/auth/magic-link/__tests__/` | Unit tests |
| `packages/core/src/contracts/auth.ts` | Added magic link schemas |
| `apps/server/src/infrastructure/database/schema/magic-link.ts` | Database schema for magic_link_tokens table |
...
- SHA-256 hashed tokens (high entropy, single-use)
- 15-minute token expiry
- Rate limiting: 3 requests per email per hour
- Token marked as used atomically with user creation
- Prevents email enumeration (always returns success)
- Automatic email verification for new users

---

### Reduced: Web Bundle Size (73% reduction)

Optimized web app bundle for faster initial load.

**Changes:**

- Removed unused direct dependencies (`zustand`, `@ts-rest/react-query`)
- Added route-based code splitting with `React.lazy()` for all routes
- Converted 20 static markdown imports to dynamic on-demand loading
- Added Vite build optimizations (vendor chunks, CSS code splitting)

**Results:**

| Metric | Before | After | Change |
|--------|--------|-------|--------|
| Initial bundle | 874KB | 234KB | -73% |
| Vendor chunks | monolithic | split (react, router, query) | improved caching |

---

### Updated: CLAUDE.md Package Policy

Added rule requiring user permission before adding new npm packages to prevent dependency bloat.
