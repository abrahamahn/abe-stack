...
**Zod 3.x → 4.x** with compatibility fixes:

- `z.record(z.unknown())` → `z.record(z.string(), z.unknown())`
- `z.SafeParseReturnType` → `ReturnType<typeof schema.safeParse>`
- `ZodIssueMinimal.path` now `PropertyKey[]` (supports symbols)
- Deprecated `z.string().email()` → `z.email()`, `.uuid()` → `z.uuid()`, `.url()` → `z.url()`
- Updated `formatValidationErrors` to handle symbol paths

**React Router 6.x → 7.x** -- updated ProtectedRoute test mocks to use same react-router-dom instance.

**React 19.x** and other dependency updates applied across all packages.

**Post-upgrade fixes** (`ba457f82`): fixed Vite desktop config import path, removed Node.js-only exports (`media`, port utils) from `@abe-stack/core` main entry for browser compat. Server apps import from `@abe-stack/core/media` and `@abe-stack/core/utils` subpaths instead.
...
- Bundles to CommonJS for Node.js/Electron compatibility, supports `@abe-stack/core` subpath exports
...
428-file commit consolidating the full server restructure (`infra/` → `infrastructure/`):
...
- Moved web tests from `src/test/` → `src/__tests__/`, UI test setup restructured
...
- Extracted stores (`toastStore`, `undoRedoStore`) from `@abe-stack/core` to `@abe-stack/stores` package
...
- **Config consolidation**: unified into `loaders/`, `schema/`, `utils/` with simplified `@abe-stack/shared/config` entry point
...
- Lint errors reduced from 2,482 → 838 (66% reduction)
