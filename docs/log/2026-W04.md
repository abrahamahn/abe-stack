...
Replaced `react-router-dom` with a custom minimal router implementation (~150 lines) in `packages/ui/src/router/`.
...
- `MemoryRouter` for testing (in-memory navigation)

**Components & Hooks (drop-in replacements):**
- `Router` / `BrowserRouter` - Browser history provider
- `MemoryRouter` - Testing provider
- `Routes`, `Route` - Route configuration
- `Link`, `Navigate` - Navigation components
- `Outlet` - Nested route rendering
- `useNavigate`, `useLocation`, `useParams`, `useSearchParams` - Router hooks

**Files Created:**
- `packages/ui/src/router/context.tsx` - Router providers and context
- `packages/ui/src/router/hooks.ts` - Navigation hooks
- `packages/ui/src/router/components.tsx` - Route matching and components
- `packages/ui/src/router/index.ts` - Barrel exports

**Files Updated:**
- All app imports changed from `react-router-dom` to `@abe-stack/ui`
- Removed `react-router-dom` from `apps/web/package.json`
- Updated test mocks to use custom router
...
- Smaller bundle size
- Consistent with "framework independence" philosophy
- Hooks organized by category
...
### Completed: Comprehensive Lint and Type Error Fixes

Fixed 46 lint errors and multiple type errors across the entire codebase.

#### Router System Improvements

| Change | Details |
|--------|---------|
| RouteProps extended | Added `children` and `index` props support for nested routes |
| MemoryRouter enhanced | initialEntries now accepts location objects with state |
| Context functions | Added explicit return type annotations |
| Navigate calls | Removed void operators on void functions |

#### Web App Fixes

| File | Fix |
|------|-----|
| `AuthPage.tsx` | Fixed URLSearchParams type safety with proper null checks |
| `ConfirmEmailPage.tsx` | Fixed searchParams destructuring type safety |
| `ResetPasswordPage.tsx` | Fixed searchParams type safety |
| `Dashboard.tsx` | Removed console.error, use proper error handling |
| `ApiProvider.test.tsx` | Added missing argument to vi.importActual |
| `ProtectedRoute.test.tsx` | Fixed route props for children and index |

#### Server Fixes

| File | Fix |
|------|-----|
| `bootstrap-admin.ts` | Fixed string concatenation type safety, import order |
| `notifications/routes.ts` | Fixed async/await by using Promise.resolve |
| `queue.test.ts` | Increased timeouts (1.5s→3s, 500ms→1s) for CI stability |

#### Package Fixes

| Package | File | Fix |
|---------|------|-----|
| `@abe-stack/ui` | `useHistoryNav.tsx` | Removed void operators on navigate() |
| `@abe-stack/ui` | `router/components.tsx` | Removed unused type parameter, use const |
| `@abe-stack/ui` | `router/context.tsx` | Added return types, fixed import order |
