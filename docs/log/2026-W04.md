...
### Core Architecture & Build Fixes

**Separation of Server-Side Infrastructure:**
Resolved a critical architectural issue where server-only code (`node:events`, `node:util`, `PubSub`) was leaking into the web application via `@abe-stack/core`.

- Moved server infrastructure to `@abe-stack/core/pubsub`.
- Updated `@abe-stack/server` to use specific subpath imports.
- Eliminated `Buffer is not defined` errors in the web app without requiring client-side polyfills.

**Build Segregation and Segregation Fixes:**
Resolved 150+ build errors in `@abe-stack/server` and `@abe-stack/core` by establishing strict package boundaries:
- **Cleaned Stray Artifacts**: Deleted dozens of `.js` and `.d.ts` files accidentally generated in `packages/core/src`.
- **Package Entry Points**: Updated `packages/core/package.json` to point `types` and `exports` to the `dist` directory, preventing source-level contamination.
- **TSConfig Refining**: Updated server `tsconfig` to resolve `@abe-stack/core` via compiled type definitions instead of source files, resolving `TS6059` and `TS6307` errors.

### Lint & Type Resolution (70+ Fixes)

Verified project health by resolving over 70 lint and type errors:
- **UI Package**: Fixed `cn` utility types and CSS theme syncing issues.
- **Media Package**: Resolved `sharp` type resolution conflicts and fixed missing exports.
- **Server App**: Fixed async/await errors in routes/handlers and pubsub type resolution.
- **Web App**: Fixed DemoPage types and unsafe property assignments.

### Environment & Configuration

- **Centralized Config**: Migrated `.env` and `.env.test` to `.config/env/` directory.
- **Path Resolution**: Fixed root path resolution in `@abe-stack/core/check.ts` to properly resolve the `.config` directory from any package depth.
- **Dev Credentials**: Updated `DATABASE_URL` and `PORT` (8080) for smoother local development onboarding.

### Repository Health & Cleanup

- **Legacy Test Repair**: Restored widespread syntax corruption in `apps/server/legacy-tests`. A previous mass-edit had stripped the `config` identifier from variable names and property accessors across 10+ files, rendering them unparseable.
- **Hook Cleanup**: Removed broken pre-commit hooks (`config:generate`, `sync:headers`) that referenced non-existent files.
