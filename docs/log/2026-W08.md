...
Tagged and published v1.0.0 of the BSLT API. Changelog format follows [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) and [Semantic Versioning](https://semver.org/spec/v2.0.0.html).
...
- [x] Workflow: E2E test step in `continuous-integration.yml` — install Playwright browsers, run in headless mode
...
- [x] CI: `application-deploy.yml` workflow deploys to production on merge to `main` (or manual trigger)
- [x] CI: rollback procedure tested — `deployment-rollback.yml` reverts to previous known-good deployment
