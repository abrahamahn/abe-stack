...
2. Reads all `.sql` files from `main/server/db/migrations/`, sorted alphabetically
...
1. Determine the next sequence number (check the highest `NNNN` in `main/server/db/migrations/`)
...
# File: main/server/db/migrations/0027_add_user_timezone.sql
