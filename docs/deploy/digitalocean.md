...
# Restore
gunzip -c backup.sql.gz | docker compose -f infra/docker/production/docker-compose.prod.yml \
  exec -T postgres psql -U postgres abe_stack

# Restart API
docker compose -f infra/docker/production/docker-compose.prod.yml start api
```
...
docker compose -f infra/docker/production/docker-compose.prod.yml logs -f --tail=100
```
...
docker compose -f infra/docker/production/docker-compose.prod.yml ps postgres

# Check postgres logs
docker compose -f infra/docker/production/docker-compose.prod.yml logs postgres

# Test connection
docker compose -f infra/docker/production/docker-compose.prod.yml exec postgres \
