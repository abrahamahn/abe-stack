...
docker compose -f config/docker/docker-compose.prod.yml exec caddy \
  caddy list-modules --packages
```

### Staging Mode (Testing)

To avoid Let's Encrypt rate limits during testing, uncomment the staging ACME CA in `config/caddy/Caddyfile`:
...
2. Configure trusted proxies (see `docs/deploy/trusted-proxy-setup.md`):
...
docker run -p 80:80 \
  -v $(pwd)/config/caddy/Caddyfile.dev:/etc/caddy/Caddyfile \
  --network host \
  caddy:2-alpine
```
...
See `docs/deploy/trusted-proxy-setup.md` for proxy trust configuration.
