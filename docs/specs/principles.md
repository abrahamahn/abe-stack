...
| --- | ----------------------------- | ------------------------------------------- | --------------------------- |
| 1   | **DRY** - One source of truth | Prevents drift, reduces maintenance         | Extract to `packages/core`  |
...
| 4   | **Framework-Agnostic Core**   | Platform portability, migration flexibility | No React in `packages/core` |
...
| kebab-case | Configuration files         | `vite.config.ts`, `.config/env/.config/env/.env.development` |
...
- `packages/sdk/src/client.ts` - Pure TypeScript client
- `packages/sdk/src/react-query.ts` - React Query hooks

### Form Validation

1. Define schema in `packages/core/src/validation/`
2. Use `safeParse()` before submit
3. Display field errors from `error.flatten().fieldErrors`

### Environment Variables

1. Define Zod schema in `packages/core/src/env.ts`
...
| --------------------------------------- | -------------------------- | -------------------------------- |
| Business logic in components            | Hard to test, not reusable | Move to `packages/core`          |
| Duplicate type definitions              | Drift and bugs             | Single source in `packages/core` |
...
| `@abe-stack/core` | Business logic package |
| `@abe-stack/ui`   | UI component package   |
| `@abe-stack/sdk`  | API client package     |
