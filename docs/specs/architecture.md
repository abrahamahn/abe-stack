...
- When you need to understand layering or server `backend/core` boundaries.
...
- **Framework-agnostic core** in `shared/core`
- **Hexagonal architecture** on server with `backend/core` separation
...
- React is a renderer only → Keep logic in `shared/core`
...
| Rule                    | Description                                      |
| ----------------------- | ------------------------------------------------ |
| No reverse deps         | `backend/*`, `client/` cannot import from `apps` |
...
| `@abe-stack/client-engine` | `@abe-stack/shared` |
...
├─────────────────────────────────────────────────────────────┤
│                   backend/engine/                      │
│   (Adapters: cache, config, mailer, storage, security)       │
└─────────────────────────────────────────────────────────────┘
```

| Layer               | Purpose                                 | Examples                                        |
| ------------------- | --------------------------------------- | ----------------------------------------------- |
| **backend/core/**   | Business logic, use cases, domain rules | `auth/service.ts`, `users/routes.ts`            |
| **backend/engine/** | External system adapters                | `cache/`, `mailer/`, `storage/`, `queue/`, etc. |
...
- `backend/core/` can import from `backend/engine/` (dependency flows inward)
- `backend/engine/` never imports from `backend/core/`
- Each package exports a clean interface via `index.ts`
...
- Extract shared logic to `shared/core`
- Use `shared/ui` only for reusable UI components
- Keep API contracts in shared, consume from server/client

### Framework-Agnostic Core

- Shared logic must not import React or platform APIs
- Provide React hooks in `sdk` or app layers
...
- Define schemas with Zod in `shared/core/src/env.ts`
...
The SDK (`sdk`) provides a complete client-side state management solution for real-time, offline-first applications.
