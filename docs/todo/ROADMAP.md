...
Restructure from role-based (`apps/`, `backend/`, `client/`) to layer-based (`frontend/`, `backend/`, `shared/`) architecture. See [Architecture](./specs/architecture.md) for details.
...
- [ ] Remove old `apps/`, `backend/`, `client/` directories
...
- [x] `SubscriptionCache` for ref-counted record subscriptions → `@abe-stack/client-engine/subscriptions`
- [x] `LoaderCache` for Suspense-friendly, deduped loaders → `@abe-stack/client-engine/cache`
...
| **Low**      | UI           | UI Library lazy loading, code standardization |
