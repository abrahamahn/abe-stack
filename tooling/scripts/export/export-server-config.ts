// tooling/scripts/export/export-server-config.ts
...
function resolveImportPath(importPath: string, fromFile: string): string | null {
  ...
  for (const [alias, srcPath] of Object.entries(PACKAGE_ALIASES)) {
    if (importPath === alias) {
      // Direct package import: @abe-stack/core -> packages/core/src/index.ts
      resolvedPath = path.join(REPO_ROOT, srcPath, 'index.ts');
      return fs.existsSync(resolvedPath) ? resolvedPath : null;
    }
    if (importPath.startsWith(`${alias}/`)) {
      // Subpath import: @abe-stack/core/config -> packages/core/src/config/index.ts
...
function exportServerConfig(): void {
  ...
  const byCoreConfig = sortedFiles.filter((f) => f.startsWith('packages/core/src/config'));
  const byServerConfig = sortedFiles.filter((f) => f.startsWith('apps/server/src/config'));
  const byDependencies = sortedFiles.filter(
    (f) => !f.startsWith('packages/core/src/config') && !f.startsWith('apps/server/src/config')
  );

  console.log(`\n📊 Summary:`);
  console.log(`   - packages/core/src/config: ${byCoreConfig.length} files`);
