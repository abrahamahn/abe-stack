// packages/core/src/config/types/notification.test.ts
...
describe('OneSignalSchema', () => {
  ...
  it('should reject empty string restApiKey', () => {
    ...
    if (!result.success) {
      expect(result.error.issues[0]?.message).toContain(
        'ONESIGNAL_REST_API_KEY is required',
      );
    }
  });
  ...
  it('should reject empty string userAuthKey', () => {
    ...
    if (!result.success) {
      expect(result.error.issues[0]?.message).toContain(
        'ONESIGNAL_USER_AUTH_KEY is required',
      );
    }
  });
...
describe('SnsSchema', () => {
  ...
  it('should validate SNS configuration with optional topicArn', () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.topicArn).toBe(
  ...
  it('should reject empty string accessKeyId', () => {
    ...
    if (!result.success) {
      expect(result.error.issues[0]?.message).toContain(
        'AWS_SNS_ACCESS_KEY_ID is required',
      );
    }
  });
  ...
  it('should reject empty string secretAccessKey', () => {
    ...
    if (!result.success) {
      expect(result.error.issues[0]?.message).toContain(
        'AWS_SNS_SECRET_ACCESS_KEY is required',
      );
    }
  });
  ...
  describe('Error message clarity', () => {
    ...
    it('should provide field path in error for nested validation failures', () => {
      ...
      if (!result.success) {
        ...
        expect(
          pathsWithErrors.some((path) => path.includes('enableLogging')),
        ).toBe(true);
      }
    });
  });
