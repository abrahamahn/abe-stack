// packages/security/src/crypto/jwt-rotation.test.ts
import { JwtError, sign as jwtSign } from '@abe-stack/core/infrastructure/crypto';
...
import type { JwtRotationConfig } from './jwt-rotation';
...
describe('JWT Rotation', () => {
  describe('signWithRotation', () => {
    test('should sign token with current secret', () => {
      const config: JwtRotationConfig = { secret: CURRENT_SECRET };
    ...
    test('should throw if secret is missing', () => {
      const config: JwtRotationConfig = { secret: '' };
  ...
  describe('verifyWithRotation', () => {
    ...
    test('should verify token signed with previous secret during rotation', () => {
      ...
      // Verify with rotation config that has both secrets
      const config: JwtRotationConfig = {
        secret: CURRENT_SECRET,
        previousSecret: PREVIOUS_SECRET,
      };
  ...
  describe('createJwtRotationHandler', () => {
    ...
    test('should provide config info', () => {
      const handler = createJwtRotationHandler({
        secret: CURRENT_SECRET,
        previousSecret: PREVIOUS_SECRET,
      });
