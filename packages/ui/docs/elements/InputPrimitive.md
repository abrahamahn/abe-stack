...
```tsx
import { InputElement } from 'abeahn-ui/elements';
```
...
<InputElement type="number" placeholder="Age" />
...
<div>
  ...
  <InputElement id="username" type="text" />
</div>
```
...
<InputElement type="text" value={value} onChange={(e) => setValue(e.target.value)} />;
```
...
<InputElement as={CustomInput} />;
```

### Disabled State

```tsx
<InputElement type="text" disabled placeholder="Disabled input" />
```
...
- [Source](../../src/elements/InputElement.tsx)
