...
const UndoRedoHarness = (
  ...
  return (
    <div>
      ...
      <button type="button" onClick={triggerUndo} data-testid="undo-button">
        Undo
      </button>
      <button type="button" onClick={triggerRedo} data-testid="redo-button">
        Redo
      </button>
    </div>
  );
...
const StatefulUndoRedoHarness = (): ReactElement => {
  ...
  return (
    <div>
      <span data-testid="value">{history[index]}</span>
      <span data-testid="can-undo">{String(canUndo)}</span>
      <span data-testid="can-redo">{String(canRedo)}</span>
      <button type="button" onClick={addValue} data-testid="add-button">
        Add
      </button>
      <button type="button" onClick={triggerUndo} data-testid="undo-button">
        Undo
...
describe('useUndoRedoShortcuts', () => {
  describe('undo shortcuts', () => {
    it('triggers onUndo for Ctrl+Z', () => {
      const onUndo = vi.fn();
      const onRedo = vi.fn();
      render(<UndoRedoHarness onUndo={onUndo} onRedo={onRedo} />);
    ...
    it('does not trigger onUndo for just Z key', () => {
      const onUndo = vi.fn();
      render(<UndoRedoHarness onUndo={onUndo} />);
    ...
    it('does not trigger onUndo when canUndo is false', () => {
      const onUndo = vi.fn();
      render(<UndoRedoHarness onUndo={onUndo} canUndo={false} />);
  ...
  describe('redo shortcuts', () => {
    ...
    it('does not trigger onRedo when canRedo is false', () => {
      const onRedo = vi.fn();
      render(<UndoRedoHarness onRedo={onRedo} canRedo={false} />);
  ...
  describe('enabled option', () => {
    it('does not trigger when disabled', () => {
      const onUndo = vi.fn();
      const onRedo = vi.fn();
      render(<UndoRedoHarness onUndo={onUndo} onRedo={onRedo} enabled={false} />);
  ...
  describe('skipInputs option', () => {
    ...
    it('skips inputs when skipInputs is true', () => {
      const onUndo = vi.fn();
      render(<UndoRedoHarness onUndo={onUndo} skipInputs />);
  ...
  describe('manual triggers', () => {
    ...
    it('triggerRedo calls onRedo', () => {
      const onRedo = vi.fn();
      render(<UndoRedoHarness onRedo={onRedo} />);
  ...
  describe('cleanup', () => {
    it('removes event listener on unmount', () => {
      const onUndo = vi.fn();
      const { unmount } = render(<UndoRedoHarness onUndo={onUndo} />);
  ...
  describe('integration with state', () => {
    it('works with stateful undo/redo', () => {
      render(<StatefulUndoRedoHarness />);
