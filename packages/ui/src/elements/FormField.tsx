...
export const FormField = forwardRef<HTMLDivElement, FormFieldProps>((props, ref) => {
  ...
  return (
    <div ref={ref} className={`ui-form-field ${className}`.trim()} {...rest}>
      <label htmlFor={htmlFor} className="ui-form-field-label">
        {label}
        {required ? <span className="ui-form-field-required">*</span> : null}
      </label>
      <div className="ui-form-field-input">{children}</div>
      {error ? (
        <span className="ui-form-field-error" role="alert">
          {error}
        </span>
      ) : null}
      {helperText && !error ? <span className="ui-form-field-helper">{helperText}</span> : null}
    </div>
  );
});
