// packages/db/src/config/database.ts
/**
 * Database Configuration Loader
 *
 * Loads database configuration from environment variables.
 * Supports multiple providers: PostgreSQL, MongoDB, SQLite, JSON.
 *
 * @module
 */
 ...
 * @returns Connection string for the configured provider.
 */
export function buildConfigConnectionString(config: DatabaseConfig): string {
 ...
 *
 * @param config - Database configuration object.
 * @returns Connection string with password replaced by `****`.
 */
export function getSafeConnectionString(config: DatabaseConfig): string {
  const full = buildConfigConnectionString(config);
  return full.replace(/:([^:@]+)@/, ':****@');
}
 ...
 * @returns `true` if the configuration is for PostgreSQL.
 */
export const isPostgres = (config: DatabaseConfig): config is PostgresConfig =>
  config.provider === 'postgresql';

/**
 * Type guard for JSON database configuration.
 *
 * @param config - Database configuration to check.
 * @returns `true` if the configuration is for JSON database.
 */
