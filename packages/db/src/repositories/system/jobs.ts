// packages/db/src/repositories/system/jobs.ts
/**
  ...
  return {
    async create(data: NewJob): Promise<Job> {
      ...
      const result = await db.queryOne<Record<string, unknown>>(
        insert(JOBS_TABLE).values(snakeData).returningAll().toSql(),
      );
      if (result === null) {
    ...
    async findById(id: string): Promise<Job | null> {
      const result = await db.queryOne<Record<string, unknown>>(
        select(JOBS_TABLE).where(eq('id', id)).toSql(),
      );
    ...
    async delete(id: string): Promise<boolean> {
      const count = await db.execute(
        deleteFrom(JOBS_TABLE).where(eq('id', id)).toSql(),
      );
      return count > 0;
    },
