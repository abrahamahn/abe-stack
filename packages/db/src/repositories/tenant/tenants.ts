// packages/db/src/repositories/tenant/tenants.ts
/**
...
export function createTenantRepository(db: RawDb): TenantRepository {
  return {
    ...
    async findById(id: string): Promise<Tenant | null> {
      const result = await db.queryOne<Record<string, unknown>>(
        select(TENANTS_TABLE).where(eq('id', id)).toSql(),
      );
    ...
    async findBySlug(slug: string): Promise<Tenant | null> {
      const result = await db.queryOne<Record<string, unknown>>(
        select(TENANTS_TABLE).where(eq('slug', slug)).toSql(),
      );
      return result !== null ? transformTenant(result) : null;
    },
    ...
    async delete(id: string): Promise<boolean> {
      const count = await db.execute(
        deleteFrom(TENANTS_TABLE).where(eq('id', id)).toSql(),
      );
      return count > 0;
    },
  };
