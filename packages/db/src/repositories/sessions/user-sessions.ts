// packages/db/src/repositories/sessions/user-sessions.ts
/**
...
export function createUserSessionRepository(db: RawDb): UserSessionRepository {
  return {
    ...
    async findById(id: string): Promise<UserSession | null> {
      const result = await db.queryOne<Record<string, unknown>>(
        select(USER_SESSIONS_TABLE).where(eq('id', id)).toSql(),
      );
    ...
    async deleteByUserId(userId: string): Promise<number> {
      return db.execute(
        deleteFrom(USER_SESSIONS_TABLE).where(eq('user_id', userId)).toSql(),
      );
    },
  };
}
