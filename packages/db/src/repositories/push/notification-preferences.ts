// packages/db/src/repositories/push/notification-preferences.ts
/**
...
import type { QuietHoursConfig, TypePreferences } from '../../schema/index';
...
export function createNotificationPreferenceRepository(
  ...
  return {
    ...
    async upsert(data: NewNotificationPreference): Promise<NotificationPreference> {
      ...
      const result = await db.queryOne<Record<string, unknown>>({ text, values });
      if (result === null) {
