// packages/db/src/repositories/billing/plans.test.ts
/**
...
describe('createPlanRepository', () => {
  ...
  describe('listAll', () => {
    it('should return all plans including inactive', async () => {
      ...
      expect(result.some((plan) => plan.isActive === false)).toBe(true);
      expect(result.some((plan) => plan.isActive === true)).toBe(true);
    });
