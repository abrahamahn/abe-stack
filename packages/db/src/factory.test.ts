// packages/db/src/factory.test.ts
/**
 * Tests for Repository Factory
 *
 * Validates that createRepositories returns all 17 repository keys
...
describe('createRepositories', () => {
  ...
  it('should create all 17 repository keys', () => {
    const ctx = createRepositories(testConnectionString);
  ...
  it('should have exactly 17 keys in repos', () => {
    const ctx = createRepositories(testConnectionString);
    const repoKeys = Object.keys(ctx.repos);

    expect(repoKeys).toHaveLength(17);
  });
