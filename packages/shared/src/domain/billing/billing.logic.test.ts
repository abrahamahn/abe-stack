// shared/src/domain/billing/billing.logic.test.ts
...
describe('billing.logic', () => {
  ...
  describe('hasActiveSubscription', () => {
    ...
    it('returns false when subscription is not active', () => {
      expect(
        hasActiveSubscription(createSubscription({ status: 'canceled' }), 'plan_pro'),
      ).toBe(false);
    });
  ...
  describe('getEffectivePlan', () => {
    ...
    it('returns matching plan for active subscription', () => {
      const result = getEffectivePlan(createSubscription({ planId: parsePlanId('plan_pro') }), plans);
  ...
  describe('canResumeSubscription', () => {
    ...
    it('returns false for canceled subscription even with cancelAtPeriodEnd', () => {
      expect(
        canResumeSubscription(
          createSubscription({ status: 'canceled', cancelAtPeriodEnd: true }),
        ),
      ).toBe(false);
    });
