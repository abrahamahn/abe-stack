...
export const auditEntrySchema: Schema<AuditEntry> = createSchema((data: unknown) => {
  ...
  const actor = obj['actor'] as Record<string, unknown> | undefined;
  ...
  if (target === undefined || typeof target['type'] !== 'string' || typeof target['id'] !== 'string') {
    throw new Error('target.type and target.id required');
  }

  // Build actor object conditionally
  const actorResult: AuditEntry['actor'] = { id: actor['id'] as string };
  ...
  const result: AuditEntry = {
    id: obj['id'] as string,
    timestamp: obj['timestamp'] instanceof Date ? obj['timestamp'] : new Date(obj['timestamp'] as string),
    actor: actorResult,
