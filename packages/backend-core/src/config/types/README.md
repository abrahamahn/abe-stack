// packages/core/src/config/types/README.md
...
**Why separate from `packages/contracts`?**

| Aspect         | `packages/contracts`   | `config/types`                |
| -------------- | ---------------------- | ----------------------------- |
...
```typescript
import { type DatabaseConfig } from '@abe-stack/shared/config';
...
- API request/response types → `packages/contracts`
- Business logic types → `packages/core/src/modules`
- Infrastructure implementation → `packages/core/src/infrastructure`
...
```typescript
// ❌ This is an API contract, belongs in packages/contracts
...
This directory was previously named `contracts/` but was renamed to `types/` to avoid confusion with `packages/contracts` (API contracts).

**Old path:** `packages/core/src/config/contracts/`  
**New path:** `packages/core/src/config/types/`
