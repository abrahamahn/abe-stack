// packages/core/src/config/types/auth.test.ts
...
describe('auth.ts type definitions', () => {
  ...
  describe('AuthConfig', () => {
    ...
    describe('top-level structure', () => {
      ...
      it('should accept bffMode as boolean', () => {
        expectTypeOf(validAuthConfig.bffMode).toEqualTypeOf<boolean>();

        const bffEnabled: AuthConfig = { ...validAuthConfig, bffMode: true };
        const bffDisabled: AuthConfig = { ...validAuthConfig, bffMode: false };

        expect(bffEnabled.bffMode).toBe(true);
        expect(bffDisabled.bffMode).toBe(false);
      });
    });
    ...
    describe('password policy configuration', () => {
      it('should enforce zxcvbn score literal type', () => {
        expectTypeOf(validAuthConfig.password.minZxcvbnScore).toEqualTypeOf<
          0 | 1 | 2 | 3 | 4
        >();
  ...
  describe('type integration and edge cases', () => {
    ...
    it('should enforce readonly behavior for literal unions', () => {
      ...
      expect(['local', 'magic', 'webauthn', 'google', 'github', 'facebook', 'microsoft', 'apple']).toContain(strategy);
