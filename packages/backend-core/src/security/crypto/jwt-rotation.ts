// packages/backend-core/src/security/crypto/jwt-rotation.ts
/**
...
/**
 * Configuration for JWT secret rotation
 *
 * @param secret - Current (primary) secret used for signing and verification
 * @param previousSecret - Previous secret - used only for verification during rotation
 */
...
/**
 * Options for JWT rotation signing operations
 *
 * @param config - Override config for this operation
 */
 ...
 * @throws JwtError if secret is missing
 *
 * @example
 * ```typescript
 * const config = {
...
export interface JwtRotationHandler {
  ...
  /** Check which secret was used to sign a token */
 ...
 *
 * @param config - Rotation config with current and optional previous secrets
 * @returns JwtRotationHandler with sign, verify, checkSecret, isRotating, and getConfig methods
 */
