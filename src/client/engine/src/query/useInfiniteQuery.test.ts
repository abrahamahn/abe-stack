...
describe('useInfiniteQuery', () => {
  ...
  describe('fetchNextPage', () => {
    ...
    it('should handle errors during fetchNextPage', async () => {
      ...
      const queryFn = vi.fn(
        ({ pageParam }: { pageParam: number | undefined }): Promise<PageData> => {
          const pageIndex = pageParam ?? 0;
          if (pageIndex === 1) {
            return Promise.reject(new Error('Failed to fetch page 1'));
          }
          const page = pages[pageIndex];
          if (page == null) {
  ...
  describe('edge cases', () => {
      ...
      const queryFn = vi.fn(
        ({
          ...
          if (pageParam === undefined || pageParam === 'cursor-1') {
            const page = stringPages[0];
            if (page == null) {
              return Promise.reject(new Error('Page not found'));
            }
            return Promise.resolve(page);
          }
          const page = stringPages[1];
          if (page == null) {
            return Promise.reject(new Error('Page not found'));
          }
          return Promise.resolve(page);
    ...
    it('should handle null as page param', async () => {
      const queryFn = vi.fn(({ pageParam }: { pageParam: number | null }): Promise<PageData> => {
        ...
        const page = mockPages[pageParam];
        if (page == null) {
          return Promise.reject(new Error('Page not found'));
        }
        return Promise.resolve(page);
