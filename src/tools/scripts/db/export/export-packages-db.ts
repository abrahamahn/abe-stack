// tools/scripts/db/export/export-packages-db.ts
import fs from 'fs';
...
const packagePath = path.join(root, 'backend', 'db');
const outputPath = path.join(root, 'db-code-export.txt');
...
/**
 * Exports all source files from backend/db into a single text file.
 * Excludes test files, node_modules, dist, and other build artifacts.
 */
const run = (): void => {
  console.log('Exporting @abe-stack/db source code...');
  ...
  let output = '';
  output += '================================================================================\n';
  output += 'ABE STACK - DB PACKAGE SOURCE CODE EXPORT\n';
  ...
  for (const relPath of allFiles) {
    ...
    const displayPath = path.join('packages', 'db', relPath);
  ...
  console.log(`\nDB code exported to ${outputPath}`);
