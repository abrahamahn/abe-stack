// tools/scripts/db/export/export-engine.ts
import fs from 'fs';
...
const packagePath = path.join(root, 'backend', 'engine');
const outputPath = path.join(root, 'engine-code-export.txt');
...
/**
 * Exports all source files from backend/engine into a single text file.
 * Excludes test files, node_modules, dist, and other build artifacts.
 */
const run = (): void => {
  console.log('Exporting @abe-stack/engine source code...');
  ...
  let output = '';
  output += '================================================================================\n';
  output += 'ABE STACK - ENGINE PACKAGE SOURCE CODE EXPORT\n';
  ...
  for (const relPath of allFiles) {
    ...
    const displayPath = path.join('packages', 'engine', relPath);
  ...
  console.log(`\nEngine code exported to ${outputPath}`);
