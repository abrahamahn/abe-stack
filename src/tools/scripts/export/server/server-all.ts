// src/tools/scripts/export/server/server-all.ts
/**
 * Server Packages Code Exporter
 *
 * Exports all source code from src/server/* (core, db, engine, media, realtime, websocket)
 * into a single file for code review or AI context. Excludes test files.
 *
 * @usage pnpm tsx src/tools/scripts/export/server/server-all.ts
 * @usage pnpm tsx src/tools/scripts/export/server/server-all.ts --verbose
 */
...
const run = (): void => {
  ...
  output += `Source: src/server/ (${SERVER_PACKAGES.join(', ')})\n\n`;
  output += '---\n\n';
