// tools/scripts/export/export-ui-code.ts
...
async function main(): Promise<void> {
  ...
  const repoRoot = path.resolve(__dirname, '..', '..', '..'); // Go up three levels: tools/scripts/export -> tools/scripts -> tools -> root
  ...
  if (options.includePaths.length > 0) {
    ...
    // Default behavior: export client/ui and apps/web/src/features/ui-library
    const uiDir = path.resolve(repoRoot, 'packages', 'ui');
    const uiLibraryDir = path.resolve(repoRoot, 'apps', 'web', 'src', 'features', 'ui-library');
    await collectFiles(uiDir, files, options);
    await collectFiles(uiLibraryDir, files, options);
  }
...
async function collectFiles(dir: string, out: string[], options: ExportOptions): Promise<void> {
  ...
  try {
    ...
    // Skip missing optional directories (e.g., ui-library absent)
    return;
  }
