...
function printEnhancedSection(message, type = "info") {
  const colors = {
    info: blue,
    warning: yellow,
    success: green,
...
// Create enhanced spinner with consistent styling
...
function isPrerequisiteInstalled(prerequisite) {
  try {
    switch (prerequisite) {
      case "node":
  ...
  if (needsInstall) {
    ...
    if (confirmInstall) {
      ...
      try {
        ...
        installSpinner.fail(`Failed to install ${packageManagerName}`);
...
async function installPrerequisite(prerequisite) {
  const commands = {
    node: {
  ...
  try {
    const command =
      commands[prerequisite][currentOS] || commands[prerequisite].default;
    ...
    spinner.fail(`Failed to install ${prerequisite}`);
    console.error(error.message);
    return false;
  }
}
...
async function main() {
  ...
  const cleanupTerminal = () => {
    try {
      ...
      console.error("Error during terminal cleanup:", err);
    }
  };
...
async function runInteractiveSetup(dockerAvailable, devEnvPath, prodEnvPath) {
  ...
  try {
    ...
    printEnhancedSection(`Error: ${error.message}`, "error");
    process.exit(1);
  }
  ...
  if (answers.useDocker) {
    ...
    try {
      ...
      const dbSpinner = createSpinner({
        text: "Waiting for PostgreSQL to initialize...",
        spinner: "dots12",
      });
        ...
        try {
          ...
          const seedCommand =
            currentOS === OS.WINDOWS
              ...
              : "npm run seed:demo";
          const seedProcess = execSync(seedCommand, {
  ...
  if (!answers.useDocker) {
    ...
    try {
      ...
      console.error(red("Error updating environment file:"), error.message);
    }
  }
...
async function configureLocalEnvironment(devEnvPath, _prodEnvPath) {
...
async function verifyDatabaseSettings(devEnvPath) {
  ...
  try {
    ...
    console.error("Error verifying database settings:", error);
  }
}
