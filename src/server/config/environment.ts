...
export interface PubsubApi {
  ...
  // Add other pubsub methods as needed
}

// Server environment type that combines all needed services
...
export function parseEnv<T>(
  ...
  if (parser) {
    try {
      ...
      throw new Error(
        `Failed to parse environment variable ${key}: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
...
export const env: EnvConfig = {
  ...
  JWT_REFRESH_SECRET: parseEnv<string>(
    "JWT_REFRESH_SECRET",
    "development-jwt-refresh-secret",
  ),
  ...
  // File Storage
  UPLOADS_DIR: parseEnv<string>(
    "UPLOADS_DIR",
    path.resolve(process.cwd(), "uploads"),
  ),
  MAX_FILE_SIZE: parseEnv<number>("MAX_FILE_SIZE", 10 * 1024 * 1024, parseInt),
  QUEUE_PATH: parseEnv<string>(
    "QUEUE_PATH",
    path.resolve(process.cwd(), "queue"),
  ),

  // Security
  SIGNATURE_SECRET: Buffer.from(
    parseEnv<string>("SIGNATURE_SECRET", "signature-secret-key"),
  ),
  PASSWORD_SALT: Buffer.from(
    parseEnv<string>("PASSWORD_SALT", "password-salt"),
  ),
