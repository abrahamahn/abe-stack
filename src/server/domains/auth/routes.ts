...
import { User, UserAttributes } from '../../database/models/auth/User';
...
import { AuthTokenService, TokenType } from './services/auth-token.service';
...
router.patch(
  '/profile', 
  authenticate, 
  customValidate(updateProfileSchema), 
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const user = await User.findByPk(authenticatedReq.user.userId) as unknown as {
        id: string;
...
router.post(
  '/change-password', 
  authenticate, 
  customValidate(changePasswordSchema), 
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const authenticatedReq = req as AuthenticatedRequest;
...
router.post(
  '/2fa/enable', 
  authenticate, 
  customValidate(twoFactorEnableSchema), 
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const authenticatedReq = req as AuthenticatedRequest;
      ...
      const backupCodes = await authService.enable2FA(authenticatedReq.user.userId, token);
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await authService.disable2FA(authenticatedReq.user.userId);
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const backupCodes = await authService.regenerate2FABackupCodes(authenticatedReq.user.userId);
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const status = await authService.getTwoFactorStatus(authenticatedReq.user.userId);
...
router.post(
  '/2fa/verify', 
  customValidate(twoFactorVerifySchema), 
  async (req: Request, res: Response, next: NextFunction) => {
    try {
