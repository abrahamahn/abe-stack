...
export class CommentRepository extends BaseRepository<CommentAttributes> {
  ...
  async findByPostId(options: {
    ...
    try {
      const result = await DatabaseConnectionManager.getPool().query(query, params);
      return result.rows;
    } catch (error) {
   ...
   */
  async findWithUser(id: string, client?: Pool): Promise<(CommentAttributes & { user: any }) | null> {
    const query = `
      SELECT 
        c.*,
        json_build_object(
   ...
   */
  async findWithLikes(id: string, limit: number = 20, offset: number = 0, client?: Pool): Promise<(CommentAttributes & { likes: any[] }) | null> {
    const query = `
      SELECT 
        c.*,
        COALESCE(
