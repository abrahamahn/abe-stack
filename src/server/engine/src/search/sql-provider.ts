// src/server/engine/src/search/sql-provider.ts
/**
 * SQL Search Provider
 *
...
> implements ServerSearchProvider<TRecord> {
  ...
  private translateCompound(
    ...
    const fragments = compound.conditions
      .map((c: FilterCondition<TRecord> | CompoundFilter<TRecord>) =>
        this.translateFilter(c, depth + 1, context),
      )
      .filter((c: SqlFragment | undefined): c is SqlFragment => c !== undefined);
    ...
    const combinedText = fragments
      .map((f: SqlFragment) => `(${f.text})`)
      .join(compound.operator === LOGICAL_OPERATORS.AND ? ' AND ' : ' OR ');
    const combinedValues = fragments.flatMap((f: SqlFragment) => f.values);
