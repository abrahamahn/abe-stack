import type { BaseContext } from '@abe-stack/shared/contracts';
import type {
  FastifyInstance,
  FastifyReply,
  FastifyRequest,
 ...
 * Accepts objects with `parse` and `safeParse` methods (e.g., Zod schemas,
...
export type RouteSchema = ZodType | FastifySchema | ValidationSchema;
...
/**
 * Detect whether a schema supports `safeParse` (Zod or ValidationSchema).
 ...
 * - Zod / ValidationSchema (safeParse): validated in the handler wrapper
  ...
  for (const [path, route] of routes) {
    ...
    const baseOptions: Partial<RouteOptions> = {
      ...
      handler: async (req: FastifyRequest, reply: FastifyReply) => {
        ...
        // Validate body with Zod/ValidationSchema if present
