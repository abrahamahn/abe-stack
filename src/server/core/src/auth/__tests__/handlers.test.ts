// src/server/core/src/auth/__tests__/handlers.test.ts
/**
...
import type { Repositories, RawDb } from '@abe-stack/db'; // Added import for Repositories and RawDb
...
describe('handleVerifyEmail', () => {
  ...
  test('should return success response with auth tokens for valid verification', async () => {
    ...
    expect(verifyEmailSpy).toHaveBeenCalledWith(ctx.db, ctx.repos, ctx.config.auth, 'verify-token');
    expect(setRefreshTokenCookieSpy).toHaveBeenCalledWith(reply, 'refresh-token', ctx.config.auth);
  });
