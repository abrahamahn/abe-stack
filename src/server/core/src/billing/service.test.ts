// src/server/core/src/billing/service.test.ts
/**
...
import type { CustomerMapping, Invoice, PaymentMethod, Plan, Subscription } from '@abe-stack/db';
import type { CheckoutParams, BillingService } from '@abe-stack/shared';
...
/**
 * Create a fully mocked BillingRepositories object.
 *
 * @returns Mocked billing repositories with all methods as vi.fn()
 * @complexity O(1)
 */
...
/**
 * Create a fully mocked BillingService provider.
 *
 * @returns Mocked billing provider with all methods as vi.fn()
 * @complexity O(1)
 */
...
/**
 * Create a mock Plan record.
 *
 * @param overrides - Optional partial overrides for plan fields
 ...
 *
 * @param overrides - Optional partial overrides for subscription fields
...
/**
 * Create a mock CustomerMapping record.
 *
 * @param overrides - Optional partial overrides for mapping fields
 * @returns Mock customer mapping object
 * @complexity O(1)
 */
 ...
 *
 * @param overrides - Optional partial overrides for invoice fields
...
/**
 * Create a mock PaymentMethod record.
 *
 * @param overrides - Optional partial overrides for payment method fields
 * @returns Mock payment method object
 * @complexity O(1)
 */
...
describe('setDefaultPaymentMethod', () => {
  ...
  describe('error cases', () => {
    it('should throw PaymentMethodNotFoundError if payment method not found', async () => {
      ...
      await expect(
        setDefaultPaymentMethod(repos, provider, 'user-1', 'pm-1'),
      ).rejects.toMatchObject({ name: 'PaymentMethodNotFoundError' });
    });
    ...
    it('should throw CustomerNotFoundError if customer mapping not found', async () => {
      ...
      await expect(
        setDefaultPaymentMethod(repos, provider, 'user-1', 'pm-1'),
      ).rejects.toMatchObject({ name: 'CustomerNotFoundError' });
    });
