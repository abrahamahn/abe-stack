...
export class QueueService {
   ...
   */
  private initializeQueue<K extends QueueTypeValue>(
    queueType: K,
  ): InMemoryQueue<QueueJobType[K]> {
    const queue = new InMemoryQueue<QueueJobType[K]>((job) =>
      Promise.resolve(job),
    );
   ...
   * Get a queue by type
   */
  public getQueue<K extends QueueTypeValue>(
    queueType: K,
  ): InMemoryQueue<QueueJobType[K]> | undefined {
    return this.queues.get(queueType) as
      | InMemoryQueue<QueueJobType[K]>
      | undefined;
  }
