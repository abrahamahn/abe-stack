...
export class SearchService extends BaseService {
  ...
  async search(
    ...
    try {
      ...
      const cachedResults =
        await this.cacheManager.get<SearchResults>(cacheKey);
      if (cachedResults) {
  ...
  async saveSearchQuery(
    ...
    try {
      ...
      await this.cacheManager.delete(suggestionCacheKey);

      // Also invalidate popular searches
      const popularCacheKey = `${CACHE_KEY_PREFIX}popular:10`;
      await this.cacheManager.delete(popularCacheKey);
    } catch (error) {
  ...
  async getPopularSearches(limit: number = 10): Promise<string[]> {
    try {
      ...
      // Try to get from cache
      const cachedPopular = await this.cacheManager.get<string[]>(cacheKey);
      if (cachedPopular) {
      ...
      // Cache the results
      await this.cacheManager.set(cacheKey, queries, POPULAR_CACHE_TTL);
