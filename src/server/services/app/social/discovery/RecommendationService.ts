...
import { Group } from "@database/models/community/Group";
import { Media } from "@database/models/media/Media";
import { Post } from "@database/models/social/Post";
import { UserRepository } from "@database/repositories/auth/UserRepository";
import { GroupRepository } from "@database/repositories/community/GroupRepository";
import { MediaRepository } from "@database/repositories/media/MediaRepository";
import { HashtagRepository } from "@database/repositories/social/HashtagRepository";
import { PostRepository } from "@database/repositories/social/PostRepository";
...
export class RecommendationService extends BaseService {
  ...
  async getPersonalizedRecommendations(
    ...
    try {
      ...
      const cachedResults =
        await this.cacheManager.get<PaginatedResult<RecommendedItem>>(cacheKey);
      if (cachedResults) {
      ...
      // Cache the results
      await this.cacheManager.set(cacheKey, result, RECOMMENDATION_CACHE_TTL);
  ...
  async getRecommendedUsers(
    ...
    try {
      ...
      // Try to get from cache
      const cachedResults =
        await this.cacheManager.get<PaginatedResult<User>>(cacheKey);
      if (cachedResults) {
        return cachedResults;
      }
  ...
  async getTrendingContent(
    ...
    try {
      ...
      // Cache the results
      await this.cacheManager.set(cacheKey, result, TRENDING_CACHE_TTL);
  ...
  async getRecommendedGroups(
    ...
    try {
      ...
      // Try to get from cache
      const cachedResults =
        await this.cacheManager.get<PaginatedResult<Group>>(cacheKey);
      if (cachedResults) {
        return cachedResults;
      }
