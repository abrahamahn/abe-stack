...
export abstract class BaseRepository<T extends BaseModel> {
  ...
  async create(data: Partial<T>, client?: Pool): Promise<T> {
    ...
    const snakeCaseData = Object.entries(data).reduce((acc, [key, value]) => {
      acc[this.camelToSnake(key)] = value;
      return acc;
    }, {} as Record<string, unknown>) as unknown as Partial<T>;
    ...
    try {
      this.logger.info('Creating record:', {
        ...
        data: Object.entries(snakeCaseData).reduce((acc, [key, value]) => {
      ...
      this.logger.info('Generated SQL:', {
        ...
        values: values.map((v, i) => {
          if (typeof values[i] === 'string' && values[i].toString().includes('password')) {
            return '[REDACTED]';
          }
  ...
  async delete(id: string, client?: Pool): Promise<boolean> {
    ...
    try {
      ...
      return queryResult.rowCount != null && queryResult.rowCount > 0;
    } catch (error) {
