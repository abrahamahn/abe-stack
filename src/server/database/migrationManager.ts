...
export class MigrationManager {
  ...
  private async ensureMigrationsTable(): Promise<void> {
    ...
    try {
      await DatabaseConnectionManager.query(query);
    } catch (error) {
      this.logger.error('Error creating migrations table', error);
      throw error;
    }
  }
  ...
  private async getExecutedMigrations(): Promise<string[]> {
    ...
    try {
      const result = await DatabaseConnectionManager.query<{ name: string }[]>(query);
