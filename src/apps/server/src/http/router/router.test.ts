// apps/server/src/http/router/router.test.ts
import { loginRequestSchema } from '@abe-stack/shared';
 ...
 * Since HandlerContext is Record<string, unknown>, we provide
 * a minimal context that mimics what a server would inject.
 */
...
/**
 * Create a mock auth guard factory for testing.
 * Returns a no-op guard that always passes.
 */
function createMockAuthGuardFactory(): RouterOptions['authGuardFactory'] {
  return vi.fn((_secret: string, _role?: string) => {
    return (_request: FastifyRequest, _reply: FastifyReply, done: (err?: Error) => void) => {
      done();
    };
  });
}
...
describe('registerRouteMap', () => {
  ...
  let authGuardFactory: RouterOptions['authGuardFactory'];

  beforeEach(() => {
    app = createMockFastify();
    ctx = createMockContext();
    authGuardFactory = createMockAuthGuardFactory();
  });
  ...
  test('should register routes with different HTTP methods', () => {
    const routes: RouteMap = {
      ...
      ['items/delete']: publicRoute(
        'DELETE',
        vi.fn().mockResolvedValue({ status: 204, body: null }),
      ),
    };
...
describe('Integration with @abe-stack/shared schemas', () => {
  let app: MockFastifyInstance;
  let ctx: HandlerContext;
  let authGuardFactory: RouterOptions['authGuardFactory'];
  ...
  test('should work with loginRequestSchema from @abe-stack/shared', () => {
    const handler = vi.fn().mockResolvedValue({ status: 200, body: { token: 'abc' } });
