// apps/web/src/features/ui-library/pages/UILibraryPage.test.tsx
import { MemoryRouter } from '@abe-stack/ui';
import { ClientEnvironmentProvider } from '@app/ClientEnvironment';
import { act, fireEvent, render, screen, waitFor, within } from '@testing-library/react';
import { UILibraryPage } from '@ui-library/pages/UILibraryPage';
...
const { mockNavigate, mockCycleDensity, mockCycleContrastMode, mockGetComponentDocsLazyFnFn } =
  vi.hoisted(() => ({
    mockNavigate: vi.fn(),
    mockCycleDensity: vi.fn(),
    mockCycleContrastMode: vi.fn(),
    mockGetComponentDocsLazyFnFn: vi.fn(),
  }));
...
vi.mock('@ui-library/utils/lazyDocs', () => ({
  getComponentDocsLazy: (id: string, category: string, name: string): Promise<string | null> =>
    Promise.resolve(mockGetComponentDocsLazyFnFn(id, category, name) as string | null),
...
describe('UILibraryPage', () => {
  beforeEach(() => {
    ...
    localStorage.clear();
    mockGetComponentDocsLazyFnFn.mockReturnValue(null);
  });
  ...
  const renderUILibraryPage = (): ReturnType<typeof render> => {
  ...
  describe('Documentation Panel', () => {
    ...
    it('renders markdown documentation when available', async () => {
      mockGetComponentDocsLazyFnFn.mockReturnValue(
      ...
      await waitFor(() => {
        expect(mockGetComponentDocsLazyFnFn).toHaveBeenCalledWith('button', 'elements', 'Button');
      });
