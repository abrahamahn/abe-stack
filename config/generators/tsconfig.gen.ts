 ...
 *   config/ts/tsconfig.react.json
 *   config/ts/tsconfig.node.json
 *   config/ts/tsconfig.eslint.json
 *   tsconfig.json (root)
...
/**
 * Generate base tsconfig files in config/ts/
 */
function generateBaseTsconfigs(
  ...
  const tsDir = path.join(ROOT, 'config/ts');
...
function getExtendsPath(projectDir: string, extendsType: 'react' | 'node' | 'base'): string {
  const relativePath = path.relative(projectDir, path.join(ROOT, 'config/ts'));
  const configMap = {
