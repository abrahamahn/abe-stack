// shared/ui/src/hooks/useKeyboardShortcuts.test.ts
...
const ShortcutsHarness = ({ shortcuts }: { shortcuts: KeyboardShortcut[] }): React.ReactElement => {
...
describe('useKeyboardShortcuts', () => {
  it('triggers handler when key is pressed', () => {
    const handler = vi.fn();
    render(React.createElement(ShortcutsHarness, { shortcuts: [{ key: 'L', handler }] }));
    fireEvent.keyDown(window, { key: 'L' });
    expect(handler).toHaveBeenCalledTimes(1);
  });
  it('is case-insensitive for key matching', () => {
    const handler = vi.fn();
    render(React.createElement(ShortcutsHarness, { shortcuts: [{ key: 'r', handler }] }));
    fireEvent.keyDown(window, { key: 'R' });
    expect(handler).toHaveBeenCalledTimes(1);
  });
  it('handles Escape key', () => {
    const handler = vi.fn();
    render(React.createElement(ShortcutsHarness, { shortcuts: [{ key: 'Escape', handler }] }));
  ...
  it('respects ctrlKey modifier', () => {
    const handler = vi.fn();
    render(
      React.createElement(ShortcutsHarness, { shortcuts: [{ key: 'K', handler, ctrlKey: true }] }),
    );
  ...
  it('respects shiftKey modifier', () => {
    const handler = vi.fn();
    render(
      React.createElement(ShortcutsHarness, { shortcuts: [{ key: 'S', handler, shiftKey: true }] }),
    );
  ...
  it('only triggers first matching shortcut', () => {
    ...
    render(
      React.createElement(ShortcutsHarness, {
        shortcuts: [
  ...
  it('can disable shortcuts with enabled option', () => {
    const handler = vi.fn();
    const DisabledHarness = (): React.ReactElement => {
      useKeyboardShortcuts([{ key: 'L', handler }], { enabled: false });
      return React.createElement('div', null, 'Test');
    };
    render(React.createElement(DisabledHarness, null));
    fireEvent.keyDown(window, { key: 'L' });
    expect(handler).not.toHaveBeenCalled();
  });
  it('cleans up event listener on unmount', () => {
    ...
    const { unmount } = render(
      React.createElement(ShortcutsHarness, { shortcuts: [{ key: 'L', handler }] }),
    );
    unmount();
    fireEvent.keyDown(window, { key: 'L' });
    expect(handler).not.toHaveBeenCalled();
