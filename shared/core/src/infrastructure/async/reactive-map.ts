// shared/core/src/infrastructure/async/reactive-map.ts
 ...
 *
 * Features:
 * - Per-key subscription (only notified for keys you care about)
 * - Atomic batch writes (all writes applied before notifications)
 * - Automatic listener cleanup
 * - Standard Map-like interface (get, set, delete, has, etc.)
 *
 ...
 * ```
 *
 * @example Atomic batch updates
 * ```typescript
 * // All writes are applied before any listener is notified
 * cache.write([
 *   { key: 'user-1', value: updatedUser1 },
 *   { key: 'user-2', value: updatedUser2 },
 *   { key: 'user-3', value: undefined }, // Deletes user-3
 * ]);
 * ```
 *
 * @example Real-time sync pattern
 * ```typescript
 * const entityCache = new ReactiveMap<string, Entity>();
 *
 * websocket.on('entity:updated', (id, data) => {
 *   entityCache.set(id, data);
 * });
 *
 * // Components subscribe to specific entities they display
 * entityCache.subscribe(entityId, (entity) => {
 *   renderEntity(entity);
 * });
 * ```
 */
