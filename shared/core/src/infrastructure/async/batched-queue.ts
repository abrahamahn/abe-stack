// shared/core/src/infrastructure/async/batched-queue.ts
 ...
 *
 * This utility is part of the public API and available for use when you need to
 * optimize multiple similar async operations by combining them into batches.
 * Common use cases:
 * - Database queries: Combine multiple `SELECT * WHERE id = ?` into a single `WHERE id IN (?)`
 * - API calls: Batch multiple requests to the same endpoint
 * - Cache warming: Fetch multiple items in a single operation
 *
 * Features:
 * - Automatic batching with configurable delay
 * - Parallel batch processing with concurrency limits
 * - Per-item promise resolution (callers get individual results)
 * - Error isolation (one failed batch doesn't affect other batches)
 *
 * @typeParam I - The input type for individual items
 * @typeParam O - The output type for individual items
 *
 * @example Database query batching (N+1 problem solution)
 * ```typescript
 ...
 * ```typescript
 * const batchedProductFetch = new BatchedQueue({
 *   processBatch: async (skus: string[]) => {
