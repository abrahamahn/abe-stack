...
describe('Database Utils', () => {
  ...
  describe('toSnakeCase', () => {
    ...
    test('should handle acronyms', () => {
      ...
      expect(toSnakeCase('iOSVersion')).toBe('i_os_version');
      expect(toSnakeCase('URLPath')).toBe('url_path');
    });

    test('should handle numbers', () => {
      expect(toSnakeCase('version2Number')).toBe('version2_number');
      expect(toSnakeCase('item1Id')).toBe('item1_id');
    });
    ...
    test('should handle strings with underscores', () => {
      expect(toSnakeCase('already_underscored')).toBe('already_underscored');
      expect(toSnakeCase('mixed_Case_and_underscore')).toBe('mixed__case_and_underscore');
    });
  });
  ...
  describe('toCamelCase', () => {
    ...
    test('should handle strings with hyphens', () => {
      ...
      expect(toCamelCase('very-long-field-name')).toBe('veryLongFieldName');
    });

    test('should handle mixed separators', () => {
      expect(toCamelCase('field_name-with-mixed_separators')).toBe('fieldNameWithMixedSeparators');
    });

    test('should handle numbers', () => {
      expect(toCamelCase('field_name_2')).toBe('fieldName2');
      expect(toCamelCase('version_2_number')).toBe('version2Number');
    });
  });
  ...
  describe('Integration tests', () => {
    ...
    test('toSnakeCase and toCamelCase should be inverses for simple cases', () => {
      const original = 'firstName';

      const snakeCased = toSnakeCase(original);
      const camelCased = toCamelCase(snakeCased);

      expect(camelCased).toBe(original.toLowerCase().charAt(0) + original.slice(1));
    });
