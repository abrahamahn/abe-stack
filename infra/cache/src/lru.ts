// infra/cache/src/lru.ts
/**
 * Efficient LRU (Least Recently Used) Cache Implementation
 *
 * Provides O(1) get, set, and deletion operations using a combination
 * of Map and doubly-linked list. This is the SINGLE SOURCE OF TRUTH
 * for all LRU operations in the cache package.
 *
 * @complexity
 * - get: O(1)
 * - set: O(1)
...
export class LRUCache<K, V> {
  ...
  set(key: K, value: V, ttlOverride?: number): void {
    ...
    // Create and add new node - conditionally include expiresAt
   ...
   *
   * Note: This does NOT check expiration or move the entry.
   * Use `get()` to access with expiration check.
   *
   * @param key - The cache key
   * @returns True if the key exists (may be expired)
   * @complexity O(1)
   */
  has(key: K): boolean {
    const node = this.cache.get(key);
