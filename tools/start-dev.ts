...
class EnhancedLogger {
  ...
  private formatStructuredData(data: Record<string, unknown>, indent = ''): void {
    Object.entries(data).forEach(([key, value]) => {
      if (typeof value === 'object' && value !== null) {
        ...
        console.log(`${indent}${colors.cyan}${key}:${colors.reset} ${this.formatValue(value)}`);
      }
    });
  }
  ...
  private formatError(error: Error | string, indent = ''): void {
    ...
    if (error.message) {
      console.log(`${indent}${colors.red}Error: ${error.message}${colors.reset}`);
    }

    if ('code' in error && error.code) {
      console.log(`${indent}${colors.yellow}Code: ${error.code}${colors.reset}`);
    }
  ...
  displayInfrastructureDashboard(): void {
    ...
    console.log(border);
    console.log(
      `│${colors.bright} Development Services${colors.reset}`.padEnd(tableWidth + 8) + '│',
    );
    ...
    Object.entries(this.infrastructureStatus).forEach(([service, status]) => {
      ...
      if ('connections' in status && status.connections)
        details += ` Connections: ${status.connections}`;
  ...
  displaySystemInfo(): void {
    ...
    this.log(LogLevel.INFO, 'SYSTEM', `Operating System: ${osInfo.name} (${osInfo.platform})`, {
      data: {
        arch: os.arch(),
      ...
      if (osInfo.platform === 'windows') {
        try {
          ...
          if (output.includes('LISTENING')) {
            ...
            try {
              const processName = execSync(`tasklist /fi "PID eq ${pid}" /fo csv /nh`, {
                encoding: 'utf8',
                stdio: 'pipe',
              })
                .split(',')[0]
  ...
  async killProcessOnPortEnhanced(port: number): Promise<boolean> {
    ...
    try {
      ...
      this.log(LogLevel.ERROR, 'SYSTEM', `Failed to kill process on port ${port}`, {
  ...
  async monitorService(serviceName: string, port: number, timeoutSeconds = 30): Promise<boolean> {
    return new Promise((resolve) => {
      this.log(LogLevel.INFO, 'MONITOR', `Waiting for ${serviceName} to start on port ${port}...`);
      ...
      const checkInterval = setInterval(async () => {
        ...
        if (portInfo.inUse) {
          ...
          this.log(LogLevel.SUCCESS, 'MONITOR', `${serviceName} is now running on port ${port}`, {
            data: {
          ...
          if (this.infrastructureStatus[serviceKey]) {
            this.infrastructureStatus[serviceKey].status = 'running';
            this.infrastructureStatus[serviceKey].port = port;
            this.infrastructureStatus[serviceKey].pid = portInfo.process
              ? parseInt(portInfo.process.pid)
              : null;
  ...
  startInfrastructureMonitoring(intervalSeconds = 30): void {
    ...
    setInterval(async () => {
      this.log(LogLevel.DEBUG, 'MONITOR', 'Running infrastructure health check...');

      if (this.infrastructureStatus.backend.status === 'running') {
        const health = await this.checkServiceHealth(this.infrastructureStatus.backend.port!);
        if (!health.healthy) {
    ...
    this.log(LogLevel.SYSTEM, 'SUMMARY', `Session completed after ${uptime}s`, {
      data: {
        ...
        services: Object.keys(this.infrastructureStatus).filter(
          (key) =>
            this.infrastructureStatus[key as keyof InfrastructureStatus].status === 'running',
        ),
      },
    });
  }
  ...
  integrateServerStatus(serverStatusData: ServerStatusData): void {
    ...
    try {
      ...
      this.log(LogLevel.INFO, 'INTEGRATION', 'Received server infrastructure status', {
        data: {
      ...
      this.log(LogLevel.WARN, 'INTEGRATION', 'Error integrating server status', {
        error: error as Error,
      });
    }
  }
  ...
  private displayIntegratedInfrastructure(serverStatusData: ServerStatusData): void {
    ...
    console.log(`${colors.brightCyan}🚀 Development Environment:${colors.reset}`);
    console.log(
    ...
    console.log(`${colors.brightMagenta}🏗️ Server Infrastructure:${colors.reset}`);
    console.log(
    ...
    if (infrastructure?.services) {
      ...
      if (activeServices.length > 0) {
        ...
        console.log(`   ${colors.gray}No additional services active${colors.reset}`);
      }
      console.log('');
    }
    ...
    console.log(`   Environment: ${server?.environment || process.env.NODE_ENV || 'development'}`);
    console.log('');
  }
...
function checkPostgreSQL(): boolean {
  ...
  if (isRunning) {
    ...
    colorLog('  Please ensure PostgreSQL is running before continuing.', 'yellow');
    return false;
  }
}
  ...
  return new Promise((resolve) => {
    ...
    const checkInterval = setInterval(() => {
      if (checkPort(port)) {
        ...
        clearInterval(checkInterval);
        colorLog(`✗ Timeout waiting for ${serviceName} to start on port ${port}`, 'red');
        resolve(false);
      }
    }, 1000);
  });
  ...
  if (fs.existsSync(scriptPath)) {
    ...
    try {
      if (osInfo.platform === 'windows') {
        ...
        execSync(`chmod +x "${scriptPath}" && "${scriptPath}" ${args.join(' ')}`, {
          stdio: 'inherit',
          cwd: process.cwd(),
          timeout: 60000, // 60 second timeout
        });
...
async function startDevelopmentEnvironment(options: StartupOptions = {}): Promise<void> {
  ...
  try {
    ...
    if (packageJson.name !== 'abe-stack') {
      logger.log(LogLevel.WARN, 'SYSTEM', `Project name mismatch: ${packageJson.name}`);
      if (!options.force) {
        logger.log(LogLevel.ERROR, 'SYSTEM', 'Use --force to continue with non-ABE Stack project');
        process.exit(1);
      }
    }
  ...
  if (!options.nodeOnly && runPlatformSpecificScript(osInfo, process.argv.slice(2))) {
    return;
  }
  ...
  if (dbInfo.inUse) {
    logger.log(LogLevel.SUCCESS, 'DATABASE', `PostgreSQL is running on port ${PORTS.POSTGRES}`, {
      data: {
    ...
    logger.log(LogLevel.WARN, 'DATABASE', `PostgreSQL not detected on port ${PORTS.POSTGRES}`);
    if (!options.force) {
      logger.log(
        LogLevel.ERROR,
        'SYSTEM',
  ...
  const backendStarted = await logger.monitorService('Backend', PORTS.BACKEND, 15);
  if (!backendStarted) {
  ...
  const frontendStarted = await logger.monitorService('Frontend', PORTS.FRONTEND, 15);
  if (!frontendStarted) {
  ...
  if (options.quiet) {
    logger.log(LogLevel.INFO, 'SYSTEM', 'Running in quiet mode - server logs are hidden');
  } else {
    logger.log(LogLevel.INFO, 'SYSTEM', 'Server logs are displayed with enhanced formatting');
  }
