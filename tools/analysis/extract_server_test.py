...
def extract_code(base_dir, target_subfolder, output_filename, file_extensions=None):
    """
    Extract code from a specific subfolder within src/tests directory
    and save it to a custom-named output file.
    
    Args:
        base_dir (str): Base directory (should point to src/tests)
        target_subfolder (str): Target subfolder within src/tests
...
def main():
    parser = argparse.ArgumentParser(description='Extract code from specific directories within src/tests')
    parser.add_argument('--subfolder', required=True, help='Subfolder within src/tests to extract code from')
    ...
    # Determine the src/tests directory path
    current_dir = os.path.dirname(os.path.abspath(__file__))
    
    # Navigate up to find src/tests directory
    # Assumes the script is in src/tools/code_analysis
    src_server_dir = os.path.abspath(os.path.join(current_dir, '..', '..', '..', 'src', 'tests'))
    
    if not os.path.isdir(src_server_dir):
        print(f"Error: src/tests directory not found at {src_server_dir}")
