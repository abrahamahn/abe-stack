...
async function setupEnvironment(envConfig: EnvConfig, step: number, total: number): Promise<void> {
  ...
  if (existsSync(ENV_DEV)) {
    const overwrite = await promptYesNo(
      '.env.development already exists. Overwrite with new configuration?',
      false,
    );
    if (!overwrite) {
      logInfo('Keeping existing .env.development');
      return;
    }
  }
  ...
  logSuccess('Created .env.development with your configuration');
}
...
function skipDockerStep(step: number, total: number): void {
  ...
  logInfo('The connection will use the settings from .env.development');
}
