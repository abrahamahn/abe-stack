// tools/scripts/export/export-shared.ts
import fs from 'fs';
...
const outputPath = path.join(root, 'shared-code.txt');
...
const extensionWhitelist = new Set<string>(['.ts', '.tsx', '.js', '.jsx', '.json', '.yaml', '.yml']);
...
const isTestFile = (filename: string, relPath: string): boolean => {
  ...
  if (relPath.split(path.sep).some(part => part === 'test' || part === 'tests' || part === '__tests__')) return true;
  return false;
};
...
const run = (): void => {
  ...
  for (const relPath of allFiles) {
    ...
    const displayPath = path.join('packages', 'shared', relPath);
