// packages/backend-core/src/db/migrate.ts
...
async function runMigrations() {
  ...
  initEnv();
  const connectionString = process.env.DATABASE_URL || 'postgres://postgres:postgres@localhost:5432/abe_stack_dev';
  ...
  try {
    ...
    const sqlFiles = files
      ...
      .sort(); // Ensure order like 0000, 0001, etc.

    // Get applied migrations
    const appliedMigrations = await db.query<{ name: string }>(
      { text: 'SELECT name FROM migrations', values: [] }
    );
