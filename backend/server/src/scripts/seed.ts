...
async function main(): Promise<void> {
  const connectionString = await resolveConnectionStringWithFallback(
    process.env,
  );
  const db = createDbClient(connectionString);

  console.log(
    `🌱 Connected to database (port ${process.env.POSTGRES_PORT || "unknown"})`,
  );
  ...
  try {
    ...
    if (existing.length === 0) {
      ...
      await db.insert(users).values({
        ...
        passwordHash:
          "$2b$10$EpWaTgiFJcCNi078F/8OBOd0.o.Jq1b1/2C1.t1.1.1.1.1.1.1.1", // "password" hash
        name: "Admin User",
      });
