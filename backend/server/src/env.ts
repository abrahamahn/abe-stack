...
import { createDbClient, loginAttempts, users } from "@db";
...
export function createConfig(
  ...
  return {
    ...
    auth: {
      ...
      refreshTokenGracePeriodSeconds: parseInt(
        process.env.REFRESH_TOKEN_GRACE_PERIOD || "30",
        10,
      ),
    ...
    password: {
      ...
      minZxcvbnScore: parseInt(process.env.PASSWORD_MIN_SCORE || "3", 10) as
        | 0
        | 1
        | 2
        | 3
...
function createEmailService(isProduction: boolean): EmailService {
  return {
    sendEmail: (options): Promise<{ success: boolean; messageId?: string }> => {
      if (!isProduction) {
        // eslint-disable-next-line no-console
        console.log("[DEV EMAIL]", options.to, options.subject);
        return Promise.resolve({
          success: true,
          messageId: "dev-" + String(Date.now()),
        });
      }
      // TODO: Implement production email (SendGrid, SES, etc.)
      return Promise.resolve({
        success: true,
        messageId: "prod-" + String(Date.now()),
      });
    },
  };
}
...
function createSecurityService(config: ServerConfig): SecurityService {
  ...
  return {
    createAccessToken(userId: string, email: string, role: string): string {
      return jwt.sign({ userId, email, role }, jwtSecret, {
        expiresIn: config.auth
          .accessTokenExpiry as jwt.SignOptions["expiresIn"],
      });
    },
    ...
    async isAccountLocked(db: DbClient, email: string): Promise<boolean> {
      const failedCount = await this.getFailedAttemptCount(db, email);
      return failedCount >= config.lockout.maxAttempts;
    },

    async getFailedAttemptCount(db: DbClient, email: string): Promise<number> {
      const windowStart = new Date(
        Date.now() - config.lockout.lockoutDurationMs,
      );
    ...
    calculateDelay(attemptCount: number): number {
      ...
      return Math.min(
        config.lockout.baseDelayMs * Math.pow(2, attemptCount - 2),
        30000,
      );
    },
    ...
    async applyProgressiveDelay(db: DbClient, email: string): Promise<void> {
    ...
    async unlockAccount(
      ...
      await db
        ...
        .where(
          and(eq(loginAttempts.email, email), eq(loginAttempts.success, false)),
        );
