// client/src/__tests__/offline-workflow.integration.test.ts
/**
...
describe('Offline Workflow Integration', () => {
  ...
  describe('optimistic updates with queue', () => {
    ...
    it('should track pending writes for records', () => {
      const { cache, queue, applyOptimisticUpdate } = system;
      mockOnline = false;
    ...
    it('should rollback optimistic update on server rejection', async () => {
      ...
      const { rollbacks, transaction } = applyOptimisticUpdate([
        { table: 'user', id: 'u1', updates: { name: 'Invalid Name' } as unknown as Partial<User> },
      ]);
  ...
  describe('offline queue persistence', () => {
    it('should persist queue to localStorage', () => {
      ...
      const { transaction } = applyOptimisticUpdate([
        { table: 'user', id: 'u1', updates: { name: 'Offline Update' } as Partial<User> },
      ]);
  ...
  describe('online/offline transitions', () => {
    ...
    it('should batch multiple offline operations', async () => {
      ...
      const { transaction: tx2 } = applyOptimisticUpdate([
        { table: 'user', id: 'u2', updates: { name: 'Bob Updated' } as Partial<User> },
      ]);
      const { transaction: tx3 } = applyOptimisticUpdate([
        { table: 'post', id: 'p1', updates: { title: 'Post Updated' } as Partial<Post> },
      ]);
  ...
  describe('multi-record transactions', () => {
    it('should handle transaction with multiple operations', async () => {
      ...
      const { transaction } = applyOptimisticUpdate([
        { table: 'user', id: 'u1', updates: { status: 'inactive' } as unknown as Partial<User> },
        {
          table: 'post',
          id: 'p1',
          updates: { title: 'Updated Title' } as unknown as Partial<Post>,
        },
      ]);
  ...
  describe('concurrent operations', () => {
    it('should handle rapid successive updates to same record', async () => {
      ...
      // Rapid updates
      const { transaction: tx1 } = applyOptimisticUpdate([
        { table: 'user', id: 'u1', updates: { name: 'Update 1' } as unknown as Partial<User> },
      ]);
      const { transaction: tx2 } = applyOptimisticUpdate([
        { table: 'user', id: 'u1', updates: { name: 'Update 2' } as unknown as Partial<User> },
      ]);
      const { transaction: tx3 } = applyOptimisticUpdate([
        { table: 'user', id: 'u1', updates: { name: 'Update 3' } as unknown as Partial<User> },
      ]);
  ...
  describe('cache and storage coordination', () => {
    it('should sync optimistic updates to storage on confirm', async () => {
      ...
      const { transaction } = applyOptimisticUpdate([
        { table: 'user', id: 'u1', updates: { name: 'Alice Updated' } as unknown as Partial<User> },
      ]);
      ...
      const { transaction } = applyOptimisticUpdate([
        {
          table: 'user',
          id: 'u1',
          updates: { name: 'Conflicting Update' } as unknown as Partial<User>,
        },
      ]);
      ...
      const { transaction } = applyOptimisticUpdate([
        {
          table: 'user',
          id: 'u1',
          updates: { name: 'Forbidden Update' } as unknown as Partial<User>,
        },
      ]);
  ...
  describe('pending write subscriptions', () => {
    it('should notify subscribers of pending write changes', async () => {
      ...
      const { transaction } = applyOptimisticUpdate([
        { table: 'user', id: 'u1', updates: { name: 'Update' } as unknown as Partial<User> },
      ]);
