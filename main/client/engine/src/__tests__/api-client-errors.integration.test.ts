// main/client/engine/src/__tests__/api-client-errors.integration.test.ts
/**
...
describe('API Client Error Handling Integration', () => {
  ...
  describe('NetworkError', () => {
    it('should capture original error', () => {
      ...
      expect((networkError as AppError).message).toBe('Request failed');
    });

    it('should have status 0', () => {
      const error = new NetworkError();
      expect((error as AppError).statusCode).toBe(0);
    });
  });
  ...
  describe('TimeoutError', () => {
    it('should capture timeout duration', () => {
      ...
      expect((error as AppError).message).toBe('Request timed out');
    });
  });
  ...
  describe('Error recovery patterns', () => {
    it('should support retry on retryable errors', async () => {
      ...
      // Mock sequence: first two calls fail with 500, third succeeds
      mockFetch
        .mockResolvedValueOnce(createMockResponse(500, { message: 'Server error' }))
        .mockResolvedValueOnce(createMockResponse(500, { message: 'Server error' }))
        .mockResolvedValue(createMockResponse(200, createValidUserResponse()));
      ...
      expect(result).toEqual(createValidUserResponse());
