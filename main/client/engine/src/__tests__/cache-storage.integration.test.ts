// client/src/__tests__/integration/cache-storage.integration.test.ts
/**
...
function createCacheStorageSystem(): {
  ...
  const syncToStorage = async <T extends keyof TestTables & string>(
    ...
    return storage.setRecord(table as TestTableNames, record as unknown as VersionedRecord);
  };
  ...
  const loadFromStorage = async <T extends keyof TestTables & string>(
    ...
    await storage.ready();
    const record = await storage.getRecord<TestTables[T] & VersionedRecord>({
...
describe('RecordCache + RecordStorage Integration', () => {
  ...
  describe('optimistic updates with rollback', () => {
    ...
    it('should commit optimistic update when server confirms', async () => {
      ...
      (cache as unknown as RecordCache<TestTables>).set(
        'user',
        'u1',
        confirmedUser as unknown as User,
        { force: true },
      );
  ...
  describe('multi-table operations', () => {
    ...
    it('should handle batch updates across tables', async () => {
      const updates: Array<{
        ...
        {
          table: 'user',
          record: { id: 'u1', version: 1, name: 'Alice', email: 'alice@test.com' } as User,
        },
        {
          table: 'user',
          record: { id: 'u2', version: 1, name: 'Bob', email: 'bob@test.com' } as User,
        },
        {
          table: 'post',
          record: { id: 'p1', version: 1, title: 'Post 1', authorId: 'u1' } as Post,
        },
        {
          table: 'post',
          record: { id: 'p2', version: 1, title: 'Post 2', authorId: 'u2' } as Post,
        },
      ];
      ...
      for (const { table, record } of updates) {
        await storage.setRecord(table as TestTableNames, record as unknown as VersionedRecord);
      }
  ...
  describe('cache listeners with storage sync', () => {
    it('should trigger cache listeners on updates', async () => {
      ...
      (
        cache as unknown as { subscribe: (table: string, id: string, listener: () => void) => void }
      ).subscribe('user', 'u1', listener);
    ...
    it('should coordinate storage events with cache updates', async () => {
      ...
      storage.subscribe((event) => {
        const entry: { type: string; table: string; id?: string } = {
          type: event.type,
          table: event.table,
        };
