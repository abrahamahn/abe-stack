// main/client/react/src/hooks/useKeyboardShortcut.test.ts
...
const ShortcutHarness = ({
  ...
  return createElement(
    ...
    createElement('textarea', { 'data-testid': 'textarea' }),
    createElement('div', {
...
const KeyBindingsHarness = ({
  ...
  return createElement('div', null, createElement('button', { type: 'button' }, 'Test Button'));
};
// ============================================================================
...
describe('useKeyboardShortcut', () => {
  describe('basic functionality', () => {
    it('triggers handler when key is pressed', () => {
      const handler = vi.fn();
      render(createElement(ShortcutHarness, { shortcutKey: 'z', handler: handler }));
      fireEvent.keyDown(window, { key: 'z' });
      expect(handler).toHaveBeenCalledTimes(1);
    });
    it('is case-insensitive for key matching', () => {
      const handler = vi.fn();
      render(createElement(ShortcutHarness, { shortcutKey: 'a', handler: handler }));
      fireEvent.keyDown(window, { key: 'A' });
      expect(handler).toHaveBeenCalledTimes(1);
    });
    it('handles special keys like Escape', () => {
      const handler = vi.fn();
      render(createElement(ShortcutHarness, { shortcutKey: 'Escape', handler: handler }));
      fireEvent.keyDown(window, { key: 'Escape' });
      expect(handler).toHaveBeenCalledTimes(1);
    });
    it('passes the event to the handler', () => {
      const handler = vi.fn();
      render(createElement(ShortcutHarness, { shortcutKey: 's', handler: handler }));
  ...
  describe('modifier keys', () => {
    it('requires ctrl when ctrl option is true', () => {
      const handler = vi.fn();
      render(createElement(ShortcutHarness, { shortcutKey: 'z', ctrl: true, handler: handler }));
    ...
    it('requires shift when shift option is true', () => {
      const handler = vi.fn();
      render(createElement(ShortcutHarness, { shortcutKey: 'z', shift: true, handler: handler }));
      fireEvent.keyDown(window, { key: 'z' });
      expect(handler).not.toHaveBeenCalled();
    ...
    it('requires alt when alt option is true', () => {
      const handler = vi.fn();
      render(createElement(ShortcutHarness, { shortcutKey: 'z', alt: true, handler: handler }));
      fireEvent.keyDown(window, { key: 'z' });
      expect(handler).not.toHaveBeenCalled();
    ...
    it('requires meta when meta option is true', () => {
      const handler = vi.fn();
      render(createElement(ShortcutHarness, { shortcutKey: 'z', meta: true, handler: handler }));
      fireEvent.keyDown(window, { key: 'z' });
      expect(handler).not.toHaveBeenCalled();
  ...
  describe('enabled option', () => {
    ...
    it('triggers when enabled', () => {
      const handler = vi.fn();
      render(createElement(ShortcutHarness, { shortcutKey: 'z', handler: handler, enabled: true }));
      fireEvent.keyDown(window, { key: 'z' });
      expect(handler).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe('cleanup', () => {
    it('removes event listener on unmount', () => {
      ...
      const { unmount } = render(
        createElement(ShortcutHarness, { shortcutKey: 'z', handler: handler }),
      );
      unmount();
      fireEvent.keyDown(window, { key: 'z' });
      expect(handler).not.toHaveBeenCalled();
    ...
    it('removes event listener when disabled', () => {
      ...
      const { rerender } = render(
        createElement(ShortcutHarness, { shortcutKey: 'z', handler: handler, enabled: true }),
      );
      fireEvent.keyDown(window, { key: 'z' });
      expect(handler).toHaveBeenCalledTimes(1);
      rerender(
...
describe('useKeyBindings', () => {
  it('handles multiple bindings', () => {
    ...
    const ctrlZ = 'ctrl+z';
    render(
      createElement(KeyBindingsHarness, {
        bindings: {
          [ctrlS]: saveHandler,
          [ctrlZ]: undoHandler,
        },
