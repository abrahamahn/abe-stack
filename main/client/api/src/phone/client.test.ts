...
describe('createPhoneClient', () => {
  ...
  describe('removePhone', () => {
    it('should remove phone number with authentication', async () => {
      ...
      expect(result).toEqual(mockResponse);
      expect(mockFetch).toHaveBeenCalledWith(
        'http://localhost:3001/api/users/me/phone/delete',
        expect.objectContaining({
          method: 'DELETE',
          credentials: 'include',
        }),
  ...
  describe('request configuration', () => {
    ...
    it('should construct correct API URLs', async () => {
      const impl: (...args: any[]) => any = (input: RequestInfo | URL) => {
        const url = input instanceof URL ? input.href : typeof input === 'string' ? input : '';
        if (url.endsWith('/users/me/phone/verify')) {
          return Promise.resolve({
            ok: true,
            json: () => Promise.resolve({ verified: true }),
          });
        }
        if (url.endsWith('/auth/sms/verify')) {
          return Promise.resolve({
            ok: true,
            json: () =>
              Promise.resolve({
