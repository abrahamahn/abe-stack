// main/client/api/src/errors.test.ts
import {
  BadRequestError,
  ConflictError,
  ForbiddenError,
...
describe('SDK Errors', () => {
  describe('ApiError', () => {
    it('should create error with status and message', () => {
      ...
      expect((error as unknown as AppError).message).toBe('Not found');
      expect(error.status).toBe(404);
      expect((error as unknown as Error).name).toBe('ApiError');
    });
    ...
    it('should create error with code and details', () => {
      ...
      expect((error as unknown as AppError).code).toBe('VALIDATION_ERROR');
      expect((error as unknown as AppError).details).toEqual({ field: 'email' });
    });
  ...
  describe('NetworkError', () => {
    it('should create error with default message', () => {
      ...
      expect((error as unknown as AppError).message).toBe('Network request failed');
      expect((error as unknown as Error).name).toBe('NetworkError');
      expect((error as unknown as AppError).code).toBe('NETWORK_ERROR');
    });
  ...
  describe('TimeoutError', () => {
    it('should create error with default message', () => {
      ...
      expect((error as unknown as AppError).message).toBe('Request timed out');
      expect((error as unknown as Error).name).toBe('TimeoutError');
      expect((error as unknown as AppError).code).toBe('TIMEOUT_ERROR');
    });

    it('should include timeout duration', () => {
      const error = new TimeoutError('Request timed out', 5000);
      expect(error.timeoutMs).toBe(5000);
      expect((error as unknown as AppError).details).toEqual({ timeoutMs: 5000 });
    });
  });
