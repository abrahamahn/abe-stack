...
export class ServerManager {
    ...
    async start(): Promise<void> {
        try {
            ...
            registerWebSocket(this.app.server, { ...this.context, resolveTableName } as any, { verifyToken });
            ...
            if (this.context) {
                 await this.stop().catch(err => { console.error('Failed to stop during startup error:', err); });
            }
            process.exit(1);
        }
    }
