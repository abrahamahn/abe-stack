...
describe('Tenant Management API Integration Tests', () => {
  ...
  // ==========================================================================
  // Sprint 3.12 Invitation Lifecycle Hardening
  // ==========================================================================

  describe('invitation lifecycle hardening', () => {
    it('regenerate invitation refreshes expiry and returns success', async () => {
      const now = new Date();
      const oldExpiry = new Date(now.getTime() + 24 * 60 * 60 * 1000);
      const newExpiry = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);

      mockRepos.tenants.findById.mockResolvedValue({
        id: 'tenant-regen-hardening',
        name: 'Regen Hardening Tenant',
        slug: 'regen-hardening-tenant',
        ownerId: 'owner-regen-hardening',
        createdAt: now,
      });
      mockRepos.memberships.findByTenantAndUser.mockResolvedValue({
        id: 'mb-regen-hardening',
        tenantId: 'tenant-regen-hardening',
        userId: 'owner-regen-hardening',
        role: 'owner',
      });
      mockRepos.invitations.findById.mockResolvedValue({
        id: 'inv-regen-hardening',
        tenantId: 'tenant-regen-hardening',
        email: 'regen-target@example.com',
        role: 'member',
        status: 'pending',
        invitedById: 'owner-regen-hardening',
        expiresAt: oldExpiry,
        acceptedAt: null,
        createdAt: now,
      });
      mockRepos.invitations.update.mockResolvedValue({
        id: 'inv-regen-hardening',
        tenantId: 'tenant-regen-hardening',
        email: 'regen-target@example.com',
        role: 'member',
      ...
      const token = createTestJwt({
        userId: 'owner-regen-hardening',
        email: 'owner-regen-hardening@example.com',
        role: 'user',
      });
      const response = await testServer.inject(
        buildAuthenticatedRequest({
          method: 'POST',
          url: '/api/tenants/tenant-regen-hardening/invitations/inv-regen-hardening/regenerate',
          accessToken: token,
        }),
      );
      ...
      expect(response.statusCode).toBe(200);
      const body = parseJsonResponse(response) as { message: string };
      expect(body.message.toLowerCase()).toContain('resent');
      expect(mockRepos.invitations.update).toHaveBeenCalledWith(
        'inv-regen-hardening',
        expect.objectContaining({ expiresAt: expect.any(Date) }),
      );
    });

    it('create invitation rejects when max pending invitations is exceeded', async () => {
      const now = new Date();
      mockRepos.tenants.findById.mockResolvedValue({
        id: 'tenant-max-pending',
        name: 'Max Pending Tenant',
        slug: 'max-pending-tenant',
        ownerId: 'owner-max-pending',
        createdAt: now,
      });
      mockRepos.memberships.findByTenantAndUser.mockResolvedValue({
        id: 'mb-max-pending',
        tenantId: 'tenant-max-pending',
        userId: 'owner-max-pending',
        role: 'owner',
      });
      mockRepos.invitations.findPendingByTenantAndEmail.mockResolvedValue(null);
      mockRepos.invitations.countPendingByTenantId.mockResolvedValue(1000);

      const token = createTestJwt({
        userId: 'owner-max-pending',
        email: 'owner-max-pending@example.com',
        role: 'user',
      });
      const response = await testServer.inject(
        buildAuthenticatedRequest({
          method: 'POST',
          url: '/api/tenants/tenant-max-pending/invitations',
          accessToken: token,
          payload: { email: 'new-target@example.com', role: 'member' },
        }),
      );

      expect(response.statusCode).toBe(400);
      const body = parseJsonResponse(response) as { message: string };
      expect(body.message.toLowerCase()).toContain('maximum');
    });
  });
});
