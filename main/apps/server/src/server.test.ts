...
const {
  mockFastifyInstance,
  mockRegisterPlugins,
  mockCreateConsoleLogger,
  mockRegisterLoggingMiddleware,
} = vi.hoisted(() => {
  ...
  const mockRegisterLoggingMiddleware = vi.fn();
...
function createMockApp(): MockAppContext {
  return {
    context: {
      db: {} as never,
      repos: {} as never,
...
describe('createServer', () => {
  ...
  describe('basic server creation', () => {
    it('should create Fastify instance with correct configuration in test environment', async () => {
      ...
      expect(mockRegisterLoggingMiddleware).toHaveBeenCalledWith(
        mockFastifyInstance,
    ...
    it('should disable pretty JSON logs when logging.prettyJson is false', async () => {
      const config = createMockConfig();
      config.server.logging.prettyJson = false;
      const db = createMockDb();
      ...
      expect(mockCreateConsoleLogger).not.toHaveBeenCalled();
    });

    it('should allow pretty JSON logs in production when logging.prettyJson is true', async () => {
      const config = createMockConfig({ env: 'production' });
      config.server.logging.prettyJson = true;
      const db = createMockDb();
      ...
      expect(mockCreateConsoleLogger).toHaveBeenCalledWith('info');
    });
  });
  ...
  describe('hybrid context hook', () => {
    it('should attach context to request when app is provided', async () => {
      ...
      await createServer({ config, db, app: app as never });
