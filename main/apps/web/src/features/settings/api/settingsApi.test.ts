// main/apps/web/src/features/settings/api/settingsApi.test.ts
/**
...
describe('createSettingsApi', () => {
  ...
  describe('configuration', () => {
    ...
    it('should use default fetch when fetchImpl not provided', () => {
  ...
  describe('updateProfile', () => {
    ...
    it('should retry once after fetching CSRF token on missing CSRF error', async () => {
      const request: UpdateProfileRequest = { firstName: 'Jane', lastName: 'Doe' };
      ...
      mockFetch
        .mockResolvedValueOnce(
          new Response(JSON.stringify({ message: 'Missing CSRF token in request' }), {
            status: 403,
          }),
        )
        .mockResolvedValueOnce(new Response(JSON.stringify({ token: 'csrf-123' }), { status: 200 }))
      ...
      const headers = mockFetch.mock.calls[2]?.[1]?.headers as Headers;
  ...
  describe('edge cases', () => {
    ...
    it('should handle very large session lists', async () => {
      const largeSessions: Session[] = Array.from(
        { length: 1000 },
        (_, i) =>
          ({
            id: `session-${i}`,
