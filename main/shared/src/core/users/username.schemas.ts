...
export const updateUsernameResponseSchema: Schema<UpdateUsernameResponse> = createSchema(
  (data: unknown) => {
    const obj = (data !== null && typeof data === 'object' ? data : {}) as Record<string, unknown>;
    return {
      username: parseString(obj['username'], 'username', { min: 2 }).toLowerCase(),
      nextChangeAllowedAt: parseString(obj['nextChangeAllowedAt'], 'nextChangeAllowedAt'),
    };
  },
);
