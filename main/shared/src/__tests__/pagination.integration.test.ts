...
import { PaginationError as UtilsPaginationError } from '../utils/pagination';
import { createCursorForItem, decodeCursor, encodeCursor } from '../utils/pagination/cursor';
import {
...
describe('Pagination Integration', () => {
  describe('Cursor pagination with realistic data', () => {
    ...
    it('should paginate articles by date descending', () => {
      ...
      const page2 = paginateArrayWithCursor(
        ...
        page1.nextCursor !== null
          ? { cursor: page1.nextCursor, limit: 10, sortOrder: 'desc' }
          : { limit: 10, sortOrder: 'desc' },
        'publishedAt',
      );
    ...
    it('should handle pagination through all items', () => {
      ...
      while (true) {
        const result: { data: Article[]; hasNext: boolean; nextCursor: string | null } =
          paginateArrayWithCursor(
            articles,
            cursor !== undefined
  ...
  describe('Large dataset pagination', () => {
    ...
    it('should efficiently paginate 10000 items', () => {
      ...
      const page1 = paginateLargeArrayWithCursor(users, { limit: 100, sortOrder: 'desc' }, 'karma');
      ...
      for (let i = 0; i < 10; i++) {
        const page = paginateLargeArrayWithCursor(
          users,
          cursor !== null
            ? { cursor, limit: 100, sortOrder: 'desc' }
      ...
      // Relaxed thresholds to account for WSL/CI environments with slower I/O.
      expect(firstPageTime).toBeLessThan(5000);
      expect(midPagesTime).toBeLessThan(5000);
    });
  ...
  describe('Edge cases and error handling', () => {
    it('should handle single item pagination', () => {
      ...
      const result = paginateArrayWithCursor(items, { limit: 10, sortOrder: 'desc' }, 'score');
    ...
    it('should handle limit larger than dataset', () => {
      ...
      const result = paginateArrayWithCursor(items, { limit: 100, sortOrder: 'desc' }, 'score');
    ...
    it('should handle items with same sort value', () => {
      ...
      // First page
      const page1 = paginateArrayWithCursor(items, { limit: 2, sortOrder: 'desc' }, 'score');
      ...
      const page2 = paginateArrayWithCursor(
        items,
        page1.nextCursor !== null
          ? { cursor: page1.nextCursor, limit: 2, sortOrder: 'desc' }
  ...
  describe('UtilsPaginationError (from utils/pagination.ts)', () => {
    it('should create error with type and message', () => {
      const error = new UtilsPaginationError('INVALID_CURSOR', 'The cursor format is invalid');
    ...
    it('should include pagination type in error', () => {
      const error = new UtilsPaginationError('CURSOR_SORT_MISMATCH', 'Sort mismatch');

      expect(error.type).toBe('CURSOR_SORT_MISMATCH');
    });

    it('should support static type checking methods', () => {
      const error = new UtilsPaginationError('INVALID_CURSOR', 'Invalid cursor');

      expect(UtilsPaginationError.isPaginationError(error)).toBe(true);
      expect(UtilsPaginationError.isType(error, 'INVALID_CURSOR')).toBe(true);
      expect(UtilsPaginationError.isType(error, 'INVALID_LIMIT')).toBe(false);
    });
  });
});
