// main/shared/src/primitives/config/types/index.test.ts
/**
...
function createMinimalAppConfig(): AppConfig {
  return {
    env: 'development',
    server: {
    logging: { clientErrorLevel: "warn", requestContext: true },
      host: 'localhost',
      port: 8080,
...
describe('AppConfig', () => {
  ...
  describe('edge cases', () => {
    it('should handle auth proxy defaults', () => {
      const config: AppConfig = {
        ...createMinimalAppConfig(),
        auth: {
          ...createMinimalAppConfig().auth,
      ...
      expect(config.auth.proxy.trustProxy).toBe(false);
    });
