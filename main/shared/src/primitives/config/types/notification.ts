// main/shared/src/primitives/config/types/notification.ts
/**
...
export const OneSignalSchema: Schema<OneSignalConfig> = createSchema<OneSignalConfig>(
  (data: unknown) => {
    ...
    return {
      ...
      settings: parseOptional(obj['settings'], (v: unknown) => {
        const s = parseObject(v, 'settings');
        return {
          enableLogging: parseOptional(s['enableLogging'], (val: unknown) =>
...
export const CourierSchema: Schema<CourierConfig> = createSchema<CourierConfig>((data: unknown) => {
  ...
  return {
    ...
    apiUrl: parseOptional(obj['apiUrl'], (v: unknown) => parseString(v, 'apiUrl')),
    settings: parseOptional(obj['settings'], (v: unknown) => {
      const s = parseObject(v, 'settings');
      return {
        enableLogging: parseOptional(s['enableLogging'], (val: unknown) =>
...
export const SnsSchema: Schema<SnsConfig> = createSchema<SnsConfig>((data: unknown) => {
  ...
  return {
    ...
    topicArn: parseOptional(obj['topicArn'], (v: unknown) => parseString(v, 'topicArn')),
  };
});
...
export const NotificationEnvSchema: Schema<NotificationEnv> = createSchema<NotificationEnv>(
  (data: unknown) => {
    ...
    return {
      NOTIFICATIONS_PROVIDER: parseOptional(obj['NOTIFICATIONS_PROVIDER'], (v: unknown) =>
        createEnumSchema<NotificationEnvProvider>(
          NOTIFICATION_ENV_PROVIDERS,
          'NOTIFICATIONS_PROVIDER',
        ).parse(v),
      ),
      ONESIGNAL_REST_API_KEY: parseOptional(obj['ONESIGNAL_REST_API_KEY'], (v: unknown) =>
        parseString(v, 'ONESIGNAL_REST_API_KEY'),
      ),
      ONESIGNAL_USER_AUTH_KEY: parseOptional(obj['ONESIGNAL_USER_AUTH_KEY'], (v: unknown) =>
        parseString(v, 'ONESIGNAL_USER_AUTH_KEY'),
      ),
      ONESIGNAL_APP_ID: parseOptional(obj['ONESIGNAL_APP_ID'], (v: unknown) =>
        parseString(v, 'ONESIGNAL_APP_ID'),
      ),
      ONESIGNAL_ENABLE_LOGGING: parseOptional(obj['ONESIGNAL_ENABLE_LOGGING'], (v: unknown) =>
        trueFalseSchema.parse(v),
      ),
      COURIER_API_KEY: parseOptional(obj['COURIER_API_KEY'], (v: unknown) =>
        parseString(v, 'COURIER_API_KEY'),
      ),
      COURIER_API_URL: parseOptional(obj['COURIER_API_URL'], (v: unknown) =>
        parseString(v, 'COURIER_API_URL'),
      ),
      COURIER_ENABLE_LOGGING: parseOptional(obj['COURIER_ENABLE_LOGGING'], (v: unknown) =>
        trueFalseSchema.parse(v),
      ),
      KNOCK_SECRET_KEY: parseOptional(obj['KNOCK_SECRET_KEY'], (v: unknown) =>
        parseString(v, 'KNOCK_SECRET_KEY'),
      ),
      KNOCK_API_URL: parseOptional(obj['KNOCK_API_URL'], (v: unknown) => parseString(v, 'KNOCK_API_URL')),
      KNOCK_ENABLE_LOGGING: parseOptional(obj['KNOCK_ENABLE_LOGGING'], (v: unknown) =>
        trueFalseSchema.parse(v),
      ),
      FCM_CREDENTIALS: parseOptional(obj['FCM_CREDENTIALS'], (v: unknown) =>
        parseString(v, 'FCM_CREDENTIALS'),
      ),
      FCM_PROJECT_ID: parseOptional(obj['FCM_PROJECT_ID'], (v: unknown) => parseString(v, 'FCM_PROJECT_ID')),
      AWS_SNS_ACCESS_KEY_ID: parseOptional(obj['AWS_SNS_ACCESS_KEY_ID'], (v: unknown) =>
        parseString(v, 'AWS_SNS_ACCESS_KEY_ID'),
      ),
      AWS_SNS_SECRET_ACCESS_KEY: parseOptional(obj['AWS_SNS_SECRET_ACCESS_KEY'], (v: unknown) =>
        parseString(v, 'AWS_SNS_SECRET_ACCESS_KEY'),
      ),
      AWS_SNS_REGION: parseOptional(obj['AWS_SNS_REGION'], (v: unknown) => parseString(v, 'AWS_SNS_REGION')),
      AWS_SNS_TOPIC_ARN: parseOptional(obj['AWS_SNS_TOPIC_ARN'], (v: unknown) =>
        parseString(v, 'AWS_SNS_TOPIC_ARN'),
      ),
      BRAZE_API_KEY: parseOptional(obj['BRAZE_API_KEY'], (v: unknown) => parseString(v, 'BRAZE_API_KEY')),
      BRAZE_API_URL: parseOptional(obj['BRAZE_API_URL'], (v: unknown) => parseString(v, 'BRAZE_API_URL')),
      BRAZE_ENABLE_LOGGING: parseOptional(obj['BRAZE_ENABLE_LOGGING'], (v: unknown) =>
        trueFalseSchema.parse(v),
      ),
    };
  },
