// main/shared/src/system/cache/errors.test.ts
...
describe('cache errors', () => {
  ...
  describe('adversarial: unknown error wrapping', () => {
    ...
    test('wrapping a CacheError subclass preserves the exact subclass', () => {
      ...
      expect((result as CacheTimeoutError).operation).toBe('hgetall');
      expect((result as CacheTimeoutError).timeoutMs).toBe(5000);
    });
  });
});
