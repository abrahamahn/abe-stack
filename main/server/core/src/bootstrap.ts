...
const consoleBaseLogger: BaseLogger = {
    trace: (data: LogData, msg: string) => { console.trace(msg, data); },
    debug: (data: LogData, msg: string) => { console.debug(msg, data); },
    info: (data: LogData, msg: string) => { console.info(msg, data); },
    warn: (data: LogData, msg: string) => { console.warn(msg, data); },
    error: (data: LogData, msg: string) => { console.error(msg, data); },
    fatal: (data: LogData, msg: string) => { console.error('[FATAL]', msg, data); },
...
export async function bootstrapSystem(options?: { config?: AppConfig }): Promise<SystemContext> {
    ...
    const pgPubSub = createPostgresPubSub({
        ...
        channel: 'app_events',
        onError: (err) => { logger.error({ err }, 'PubSub error'); },
    });
    ...
    const search: ServerSearchProvider = {
        ...
        searchWithCursor: async () => ({ data: [], nextCursor: null, prevCursor: null, hasNext: false, hasPrev: false, limit: 10 }),
