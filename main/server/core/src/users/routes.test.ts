// main/server/core/src/users/routes.test.ts
/**
 * User Routes Tests
 *
...
describe('User Routes Definition', () => {
  ...
  describe('Route Schema Assignments', () => {
    ...
    test('session routes should not have request body schemas', () => {
      ...
      expect(userRoutes.get('users/me/sessions/:id')!.schema).toBe(emptyBodySchema);
      expect(userRoutes.get('users/me/sessions/revoke-all')!.schema).toBe(emptyBodySchema);
    });
  });
