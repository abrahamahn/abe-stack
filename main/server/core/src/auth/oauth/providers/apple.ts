// main/server/core/src/auth/oauth/providers/apple.ts
/**
...
function decodeIdTokenPayload(idToken: string): AppleIdTokenPayload {
  ...
  try {
    ...
    return JSON.parse(payloadStr) as AppleIdTokenPayload;
  } catch {
...
async function verifyAndDecodeIdToken(
  ...
  if (payloadDecoded.iss !== APPLE_ISSUER) {
    throw new OAuthError(
      `Invalid issuer: ${payloadDecoded.iss}. Expected: ${APPLE_ISSUER}`,
      'apple',
      'INVALID_ISSUER',
    );
  }
  ...
  if (payloadDecoded.aud !== clientId) {
    throw new OAuthError(
      `Invalid audience: ${payloadDecoded.aud}. Expected: ${clientId}`,
      'apple',
      'INVALID_AUDIENCE',
    );
  }
...
export function createAppleProvider(config: AppleProviderConfig): OAuthProviderClient {
  ...
  return {
    ...
    async getUserInfo(accessToken: string): Promise<OAuthUserInfo> {
      ...
      const emailVerified =
        payloadDecoded.email_verified === true || payloadDecoded.email_verified === 'true';
