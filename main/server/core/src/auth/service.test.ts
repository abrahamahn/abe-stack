...
describe('registerUser', () => {
  ...
  describe('when user does not exist', () => {
    ...
    it('should create user agreement when tosAccepted is true', async () => {
      const email = 'tosuser@example.com';
      const password = 'StrongPass123!';
      const username = 'tosuser';
      const firstName = 'TOS';
      const lastName = 'User';
      const baseUrl = 'http://localhost:3000';
      ...
      await registerUser(
        ...
        lastName,
        baseUrl,
        { tosAccepted: true, ipAddress },
      );

      expect(repos.legalDocuments.findLatestByType).toHaveBeenCalledWith('terms-of-service');
      // db.execute called for token creation AND user agreement
...
describe('verifyEmail', () => {
  ...
  it('should verify email and return auth result', async () => {
    ...
    expect(repos.memberships.findByUserId).toHaveBeenCalledWith(userId);
    // Since it's fire-and-forget, we might not catch the createTenant call in this event loop
