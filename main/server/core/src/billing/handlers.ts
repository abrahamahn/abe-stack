// main/server/core/src/billing/handlers.ts
/**
 * Billing Handlers
 *
 ...
 * @param body - Optional redirect URL
 * @param request - HTTP request
 * @returns 200 with portal URL
 * @complexity O(1)
 */
  ...
  try {
    ...
    const returnUrl =
      typeof body.returnUrl === 'string' && body.returnUrl !== ''
        ? body.returnUrl
        : ctx.config.billing.urls.checkoutCancelUrl; // Reuse cancel as default return

    const session = await createPortalSession(repos, provider, user.userId, returnUrl);
