// main/server/engine/src/storage/http/helpers.test.ts
...
describe('normalizeFilename', () => {
  ...
  describe('special character handling', () => {
    ...
    test('should preserve allowed characters', () => {
      ...
      expect(normalizeFilename('document 2024.pdf')).toBe('document_2024.pdf');
    });
    ...
    test('should handle unicode characters', () => {
      // Regex replaces consecutive non-allowed chars with single underscore
      expect(normalizeFilename('café.txt')).toBe('caf_.txt');
      expect(normalizeFilename('日本語.pdf')).toBe('_.pdf');
      expect(normalizeFilename('日a語.pdf')).toBe('_a_.pdf');
    });
  });
