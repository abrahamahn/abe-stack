// main/server/db/src/repositories/features/feature-flags.ts
/**
...
export function createFeatureFlagRepository(db: RawDb): FeatureFlagRepository {
  return {
    ...
    async findByKey(key: string): Promise<FeatureFlag | null> {
      const result = await db.queryOne(select(FEATURE_FLAGS_TABLE).where(eq('key', key)).toSql());
      return result !== null ? transformFeatureFlag(result) : null;
    },

    async findAll(): Promise<FeatureFlag[]> {
      const results = await db.query(select(FEATURE_FLAGS_TABLE).orderBy('key', 'asc').toSql());
      return results.map(transformFeatureFlag);
    },

    async findEnabled(): Promise<FeatureFlag[]> {
      const results = await db.query(
        select(FEATURE_FLAGS_TABLE).where(eq('is_enabled', true)).orderBy('key', 'asc').toSql(),
      );
      return results.map(transformFeatureFlag);
    },
    ...
    async update(key: string, data: UpdateFeatureFlag): Promise<FeatureFlag | null> {
      ...
      const result = await db.queryOne(
        update(FEATURE_FLAGS_TABLE).set(snakeData).where(eq('key', key)).returningAll().toSql(),
      );
      return result !== null ? transformFeatureFlag(result) : null;
    },

    async delete(key: string): Promise<boolean> {
      const count = await db.execute(deleteFrom(FEATURE_FLAGS_TABLE).where(eq('key', key)).toSql());
      return count > 0;
    },
  };
}
