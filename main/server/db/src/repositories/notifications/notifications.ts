// main/server/db/src/repositories/notifications/notifications.ts
/**
...
export function createNotificationRepository(db: RawDb): NotificationRepository {
  return {
    ...
    async findById(id: string): Promise<Notification | null> {
      const result = await db.queryOne(select(NOTIFICATIONS_TABLE).where(eq('id', id)).toSql());
    ...
    async delete(id: string): Promise<boolean> {
      const count = await db.execute(deleteFrom(NOTIFICATIONS_TABLE).where(eq('id', id)).toSql());
      return count > 0;
    },
  };
}
