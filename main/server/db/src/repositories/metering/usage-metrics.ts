// main/server/db/src/repositories/metering/usage-metrics.ts
/**
...
export function createUsageMetricRepository(db: RawDb): UsageMetricRepository {
  return {
    ...
    async findByKey(key: string): Promise<UsageMetric | null> {
      const result = await db.queryOne(select(USAGE_METRICS_TABLE).where(eq('key', key)).toSql());
      return result !== null ? transformMetric(result) : null;
    },

    async findAll(): Promise<UsageMetric[]> {
      const results = await db.query(select(USAGE_METRICS_TABLE).orderBy('key', 'asc').toSql());
    ...
    async update(key: string, data: UpdateUsageMetric): Promise<UsageMetric | null> {
      ...
      const result = await db.queryOne(
        update(USAGE_METRICS_TABLE).set(snakeData).where(eq('key', key)).returningAll().toSql(),
      );
      return result !== null ? transformMetric(result) : null;
    },

    async delete(key: string): Promise<boolean> {
      const count = await db.execute(deleteFrom(USAGE_METRICS_TABLE).where(eq('key', key)).toSql());
      return count > 0;
    },
  };
}
