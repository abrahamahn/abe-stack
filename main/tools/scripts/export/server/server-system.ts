// main/tools/scripts/export/server/server-system.ts
/**
 * System Package Code Exporter
 *
 * Exports all source code from main/server/system with comments stripped,
 * excluding test files. Output written to the repo root as server-system.txt.
 *
 * @usage pnpm tsx main/tools/scripts/export/server/server-system.ts
 */
...
const PACKAGE_SRC = path.join(REPO_ROOT, 'main', 'server', 'system', 'src');
const OUTPUT_FILE = path.join(REPO_ROOT, 'server-system.txt');
...
function run(): void {
  ...
  let output = '';
  output += '================================================================================\n';
  output += 'ABE STACK — SERVER SYSTEM SOURCE CODE EXPORT (COMMENTS STRIPPED)\n';
  output += `Generated: ${new Date().toISOString()}\n`;
  output += `Source: main/server/system/src\n`;
  output += `Total Files: ${String(files.length)}\n`;
  output += '================================================================================\n\n';

  for (const relPath of files) {
    const fullPath = path.join(PACKAGE_SRC, relPath);
    const displayPath = path.join('main', 'server', 'system', 'src', relPath);
