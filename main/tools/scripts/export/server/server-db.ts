// main/tools/scripts/export/server/server-db.ts
/**
 * Database Package Code Exporter
 *
 * Exports all source code from main/server/db (schema, migrations, repositories)
 * into a single file for code review or AI context.
 *
 * @usage pnpm tsx main/tools/scripts/export/server/server-db.ts
 */
...
function run(): void {
  ...
  for (const file of files) {
    ...
    // Only include files actually in main/server/db (sanity check)
