...
function startTurboDev(filters: string[]): ChildProcess {
  ...
  const processLine = (line: string, isError = false): void => {
    const withoutCR = line.replace(/\r$/, '');
    const normalized = withoutCR.trim();
    ...
    if (match) {
      ...
      // Turbo inserts one separator space after ':dev:'; preserve all indentation after that.
      const message = match[2].startsWith(' ') ? match[2].slice(1) : match[2];
      if (message.trim() === '') return;
      if (scope === 'server') {
        const normalizedServer = normalizeServerLine(message);
        if (normalizedServer.passthrough !== undefined) {
          const target = isError ? process.stderr : process.stdout;
          target.write(`${normalizedServer.passthrough}\n`);
          return;
        }
        if (
          normalizedServer.scope !== undefined &&
          normalizedServer.level !== undefined &&
          normalizedServer.message !== undefined
        ) {
          logLine(
            normalizedServer.scope,
            normalizedServer.message,
            isError ? 'error' : normalizedServer.level,
            normalizedServer.time,
          );
          return;
        }
        logLine('server', message, isError ? 'error' : 'info');
        return;
      }
  ...
  turbo.stdout?.on('data', (data) => processOutput(data, false));
...
async function main(): Promise<void> {
  ...
  const watchers = [
    startWatcher('main/tools/sync/sync-file-headers.ts'),
    startWatcher('main/tools/sync/sync-css-theme.ts'),
  ];
