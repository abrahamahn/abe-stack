// main/tools/scripts/path/all.ts
/**
 * Exports ALL project files to .tmp/PATH-all.md in a categorized format.
 * Categories: Root, docs, infra, config, .github, main/apps/*, main/client/*, main/server/*, main/shared, main/tools, Styles, Barrels, Markdown
...
interface CategorizedFiles {
  ...
  config: string[];
  github: string[];
  /** Source code packages keyed by e.g. 'main/apps/web', 'main/shared' */
...
function categorizeFiles(allFiles: string[]): CategorizedFiles {
  ...
  for (const file of allFiles) {
    ...
    // CSS/Styles
    if (file.endsWith('.css') || file.startsWith('main/client/ui/src/styles/')) {
    ...
    switch (parts[0]) {
      case 'src': {
        // Group src/ files by package: src/<layer>/<pkg> or main/shared, main/tools
        let pkgKey: string;
