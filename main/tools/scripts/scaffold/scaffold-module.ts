// main/tools/scripts/scaffold/scaffold-module.ts
/**
 * Module Scaffold CLI
 *
 * Generates a complete module skeleton in main/server/core/src/<name>/
...
const CORE_DIR = path.resolve('main/server/core/src');
const ROUTES_FILE = path.resolve('main/apps/server/src/routes/routes.ts');
...
export function generateTemplates(name: string): Map<string, string> {
  ...
  files.set(
    'types.ts',
    `// main/server/core/src/${name}/types.ts
/**
 * ${pascal} Module Types
 *
...
`,
  ...
  files.set(
    'service.ts',
    `// main/server/core/src/${name}/service.ts
/**
 * ${pascal} Service
 *
...
`,
  ...
  files.set(
    'service.test.ts',
    `// main/server/core/src/${name}/service.test.ts
...
`,
  ...
  files.set(
    'handlers.ts',
    `// main/server/core/src/${name}/handlers.ts
/**
 * ${pascal} Handlers
 *
...
`,
  ...
  files.set(
    'handlers.test.ts',
    `// main/server/core/src/${name}/handlers.test.ts
import { describe, expect, test, vi } from 'vitest';
...
`,
  ...
  files.set(
    'routes.ts',
    `// main/server/core/src/${name}/routes.ts
/**
 * ${pascal} Routes
 *
...
`,
  ...
  files.set(
    'index.ts',
    `// main/server/core/src/${name}/index.ts
/**
 * ${pascal} Module
 */
