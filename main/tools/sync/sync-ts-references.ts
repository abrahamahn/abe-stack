// main/tools/sync/sync-ts-references.ts
...
const SCAN_DIRS = ['main/apps', 'main/client', 'main/server', 'main/shared'];
...
function main(): void {
  ...
  const needsUpdate =
    currentPaths.size !== newPaths.size || [...newPaths].some((p) => !currentPaths.has(p));
