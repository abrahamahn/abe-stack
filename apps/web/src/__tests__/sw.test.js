// apps/web/src/__tests__/sw.test.js
/**
...
describe('Service Worker (sw.js)', () => {
  ...
  beforeEach(() => {
    ...
    mockCaches = {
      ...
      delete: vi.fn(async (name) => cacheStore.delete(name)),
    };
    ...
    mockSelf = {
      ...
      clients: {
        ...
        openWindow: vi.fn(async (url) => new MockClient(url)),
      },
      registration: {
  ...
  describe('Utility Functions', () => {
    ...
    describe('Pattern Matching', () => {
      it('should identify static assets correctly', async () => {
        ...
        for (const request of requests) {
          ...
          const isStatic = /\.(js|css|png|woff2)$/.test(url.pathname);
          expect(isStatic).toBe(true);
        }
      });

      it('should identify API requests correctly', async () => {
      ...
      it('should identify HTML requests correctly', async () => {
        const htmlRequests = [
  ...
  describe('Lifecycle Events', () => {
    ...
    describe('fetch event', () => {
      it('should route API requests to networkFirst', async () => {
        const request = new Request('https://example.com/api/users');
      ...
      it('should route static assets to cacheFirst', async () => {
        const request = new Request('https://example.com/app.js');
        const event = new MockFetchEvent(request);
      ...
      it('should route HTML requests to staleWhileRevalidate', async () => {
        const request = new Request('https://example.com/page.html');
        const event = new MockFetchEvent(request);
      ...
      it('should ignore non-GET requests for non-API', async () => {
        const request = new Request('https://example.com/page.html', {
          method: 'POST',
        });
    ...
    describe('message event', () => {
      ...
      it('should handle CLEAR_CACHE message', async () => {
        const event = new MockExtendableEvent('message');
        ...
        const clearPromise = (async () => {
          ...
          await Promise.all(names.map((name) => mockCaches.delete(name)));
        })();
  ...
  describe('Push Notifications', () => {
    describe('push event', () => {
      ...
      it('should remove undefined notification options', async () => {
        ...
        const pushPromise = (async () => {
          ...
          await mockSelf.registration.showNotification(title, notificationOptions);
        })();

        event.waitUntil(pushPromise);
        await event._waitForAll();

        const callArgs = mockSelf.registration.showNotification.mock.calls[0][1];
    ...
    describe('notificationclick event', () => {
      ...
      it('should focus existing window if available', async () => {
        ...
        expect(client._messages[0].type).toBe('NOTIFICATION_CLICK');
      });
  ...
  describe('Edge Cases', () => {
    it('should handle Response.clone() correctly', async () => {
