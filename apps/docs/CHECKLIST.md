...
| Symbol | Meaning |
|--------|---------|
| S | `packages/db/src/schema/<module>.ts` - TypeScript column types + table constants |
| M | `packages/db/migrations/NNNN_<name>.sql` - SQL DDL |
| R | `packages/db/src/repositories/<module>/` - functional repo (`create*Repository`) |
| D | `packages/shared/src/domain/<module>/` - Zod schemas + domain logic |
...
| `tenants` | [x] `schema/tenant.ts` | [x] `0001_tenant.sql` | [ ] `repositories/tenant/` | [x] `domain/tenant/` | Workspaces / organizations |
| `memberships` | [x] `schema/tenant.ts` | [x] `0001_tenant.sql` | [ ] `repositories/tenant/` | [x] `domain/membership/` | User-to-tenant roles |
| `invitations` | [x] `schema/tenant.ts` | [x] `0001_tenant.sql` | [ ] `repositories/tenant/` | [x] `domain/membership/` | Email invite flow |
| `user_sessions` | [x] `schema/sessions.ts` | [x] `0002_sessions.sql` | [ ] `repositories/auth/` | [ ] | Active device list + revoke |
...
| Table | S | M | R | D | Notes |
|-------|---|---|---|---|-------|
| `notifications` | [x] `schema/notifications.ts` | [x] `0004_notifications.sql` | [ ] `repositories/notifications/` | [x] `domain/notifications/` | In-app notification records |
...
| Table | S | M | R | D | Notes |
|-------|---|---|---|---|-------|
| `jobs` | [x] `schema/system.ts` | [x] `0005_system.sql` | [ ] `repositories/system/` | [ ] | Background job queue |
| `audit_events` | [x] `schema/system.ts` | [x] `0005_system.sql` | [ ] `repositories/system/` | [x] `domain/audit-log/` | General audit log |
| `webhooks` | [x] `schema/system.ts` | [x] `0005_system.sql` | [ ] `repositories/system/` | [ ] | Registered webhook endpoints |
| `webhook_deliveries` | [x] `schema/system.ts` | [x] `0005_system.sql` | [ ] `repositories/system/` | [ ] | Delivery log + replay |

### Module 6: Feature Management

| Table | S | M | R | D | Notes |
|-------|---|---|---|---|-------|
| `feature_flags` | [x] `schema/features.ts` | [x] `0006_features.sql` | [ ] `repositories/features/` | [x] `domain/feature-flags/` | Global feature toggles |
| `tenant_feature_overrides` | [x] `schema/features.ts` | [x] `0006_features.sql` | [ ] `repositories/features/` | [x] `domain/feature-flags/` | Per-tenant overrides |

### Module 7: Usage Metering

| Table | S | M | R | D | Notes |
|-------|---|---|---|---|-------|
| `usage_metrics` | [x] `schema/metering.ts` | [x] `0007_metering.sql` | [ ] `repositories/metering/` | [x] `domain/usage-metering/` | Metric definitions |
| `usage_snapshots` | [x] `schema/metering.ts` | [x] `0007_metering.sql` | [ ] `repositories/metering/` | [x] `domain/usage-metering/` | Recorded usage data |

### Module 8: Compliance

| Table | S | M | R | D | Notes |
|-------|---|---|---|---|-------|
| `legal_documents` | [x] `schema/compliance.ts` | [x] `0008_compliance.sql` | [ ] `repositories/compliance/` | [ ] | ToS, Privacy Policy versions |
| `user_agreements` | [x] `schema/compliance.ts` | [x] `0008_compliance.sql` | [ ] `repositories/compliance/` | [ ] | User acceptance records |
| `consent_logs` | [x] `schema/compliance.ts` | [x] `0008_compliance.sql` | [ ] `repositories/compliance/` | [ ] | GDPR consent audit trail |
...
Total tables:  34
  Schema done:   34 / 34
  Migration done: 34 / 34
  Repo done:     17 / 34
  Domain done:   23 / 34  (packages/shared contracts)
```
...
- [x] `packages/shared/src/domain/tenant/tenant.schemas.ts` - Zod schemas
- [x] `packages/shared/src/domain/membership/membership.schemas.ts` - Zod schemas
- [x] `packages/shared/src/domain/membership/membership.logic.ts` - Role helpers
- [x] `packages/db/src/schema/tenant.ts` - DB types
- [x] `packages/db/migrations/0001_tenant.sql` - DDL
- [ ] `packages/db/src/repositories/tenant/` - Repos (tenants, memberships, invitations)
- [ ] `modules/users/` - Tenant CRUD service + invite accept
...
- [-] `user_sessions` table (migration done, schema + repo needed)
...
- [x] `packages/shared/src/domain/users/users.roles.ts` - Role definitions
- [x] `packages/shared/src/domain/users/users.permissions.ts` - Permission strings
- [ ] Per-tenant role enforcement middleware
- [ ] `<Can permission="...">` UI gating component

#### 1.5 Audit logs

- [x] `packages/shared/src/domain/audit-log/audit-log.schemas.ts` - Zod schemas
- [x] `packages/shared/src/domain/audit-log/audit-log.logic.ts` - Event builder
- [x] `packages/db/src/schema/system.ts` - `audit_events` table types
- [x] `packages/db/migrations/0005_system.sql` - DDL
- [ ] `packages/db/src/repositories/system/` - Audit repo
...
- [x] `packages/shared/src/domain/billing/` - All billing domain contracts
...
- [x] `packages/shared/src/domain/notifications/` - Domain contracts
- [-] `notifications` table (migration done, schema + repo needed)
...
- [x] Shared schema package (`packages/shared` with Zod)
...
- [x] Postgres connection + pooling (`packages/db/src/client.ts`)
- [x] Raw `postgres` driver (no ORM)
- [x] Type-safe SQL builder (`packages/db/src/builder/`)
- [x] Migrations system (runner + all migrations 0000-0008 written)
- [x] Seed scripts (`packages/db/src/scripts/seed.ts`)
- [x] Bootstrap admin script (`packages/db/src/scripts/bootstrap-admin.ts`)

### 4.4 Background jobs

- [-] `jobs` table (migration done, schema + repo needed)
- [x] Write service (`packages/db/src/write/write-service.ts`)
...
- [x] Standard error model + error codes (`packages/shared/src/core/errors.ts`)
...
- [x] `packages/shared/src/domain/feature-flags/` - Domain contracts
- [-] `feature_flags` + `tenant_feature_overrides` tables (migration done, schema + repo needed)
- [ ] Evaluation helper middleware
- [ ] Admin UI for flag management

### 7.4 Compliance

- [-] `legal_documents`, `user_agreements`, `consent_logs` tables (migration done, schema + repo needed)
...
## Current Priority: Backend Engine (`packages/backend-core`)
...
| `packages/db/src/index.ts` utils path | [x] Fixed | Changed `./utils/index` → `./utils/transaction` (naming conflict) |
...
### Step 2: New schemas + migrations + repos (nothing exists yet)

```
[-] 0004_notifications.sql + schema/notifications.ts + repositories/notifications/
[-] 0005_system.sql        + schema/system.ts        + repositories/system/
```

### Step 3: Feature tables (depend on tenants)

```
[-] 0006_features.sql  + schema/features.ts  + repositories/features/
[-] 0007_metering.sql  + schema/metering.ts  + repositories/metering/
```

### Step 4: Compliance (lowest priority)

```
[-] 0008_compliance.sql + schema/compliance.ts + repositories/compliance/
```

### After DB layer: vertical slices

Once the DB layer is solid, work module-by-module:
1. **Auth module** - wire user_sessions, magic_link, oauth into `modules/auth`
2. **Tenant module** - wire tenants, memberships, invitations into `modules/users` (or new `modules/tenant`)
3. **Billing module** - wire all billing repos into `modules/billing` + Stripe webhooks
