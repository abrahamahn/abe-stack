...
### Lint Error Resolution - @abe-stack/kernel

**Package**: `@abe-stack/kernel` | **Errors Fixed**: 32
...
- Removed re-exports from `@abe-stack/kernel` in `apps/server/src/shared/index.ts`
- Updated ~25 consumer files to import errors directly from `@abe-stack/kernel`
...
| @abe-stack/kernel/contracts | 16 | 1 | 17 |
...
- Changed imports from `@abe-stack/kernel` → local `./types` or `../types`
...
- Added sub-path alias resolution: `@abe-stack/kernel/*` → `shared/core/src/*`
- Enables imports like `@abe-stack/kernel/config` to resolve correctly in tests
- Pattern:
  ```typescript
  alias: {
    '@abe-stack/kernel': path.resolve(__dirname, 'shared/core/src/index.ts'),
    '@abe-stack/kernel/*': path.resolve(__dirname, 'shared/core/src/*'),
  }
  ```
...
- Pattern: `from './types'` instead of `from '@abe-stack/kernel'`
...
- `@abe-stack/kernel` → root exports
- `@abe-stack/kernel/*` → sub-path exports (e.g., config, validation)
...
**Objective**: Rewire `apps/server/src/infrastructure/monitor/logger/` to import pure functions and types from `@abe-stack/kernel/infrastructure/logger` instead of defining them locally.

#### Changes

1. **`types.ts`** - Replaced local type definitions with re-exports from `@abe-stack/kernel/infrastructure/logger`:
...
2. **`logger.ts`** - Removed duplicated pure functions, re-exports from core:
   ...
   - Added `export { ... } from '@abe-stack/kernel/infrastructure/logger'` for all pure functions
...
3. **`middleware.ts`** - Updated imports:
   - `createRequestContext`, `generateCorrelationId`, `getOrCreateCorrelationId` now imported from `@abe-stack/kernel/infrastructure/logger`
...
**4. Dependencies**: Package depends on `@abe-stack/kernel` (notification types, schemas, errors), `@abe-stack/infra` (DbClient, table constants, query builders), and `@abe-stack/infra` (route registration helpers). Fastify is a peer dependency.
...
- `@shared` (server-specific context) decomposed to:
  - `@abe-stack/kernel` for `mapErrorToHttpResponse` + error types
...
- `@/config` changed to `./config` or `@abe-stack/kernel`
...
**4. DRY Error Mapping**: Created `createErrorMapperLogger()` in `types.ts` to adapt `AuthLogger` to the `ErrorMapperLogger` interface used by `@abe-stack/kernel`'s `mapErrorToHttpResponse`. This replaced inline logger adapter code that was duplicated across all 10+ handler files.
...
- **`modules/auth/package.json`**: Dependencies on `@abe-stack/kernel`, `@abe-stack/infra`, `@abe-stack/infra`, `@abe-stack/infra`, `argon2`. Fastify as optional peer dependency.
...
**5. Dependencies**: `@abe-stack/kernel` (errors, types, validatePassword), `@abe-stack/infra` (repositories), `@abe-stack/infra` (route registration), `@abe-stack/auth` (password hashing), `@abe-stack/infra` (file storage). Fastify as optional peer dependency.
...
| File                   | Purpose                                                                                                                                        |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `types.ts`             | Search provider type definitions (`ServerSearchProvider`, `SearchContext`, `SqlSearchProviderConfig`, etc.). Re-exports from `@abe-stack/kernel` |
...
- `infra/db/package.json` -- Added `@abe-stack/kernel: "workspace:*"` dependency.
- `infra/db/tsconfig.json` -- Added path aliases for `@abe-stack/kernel` and project reference to `../core`.
...
3. **Local type definitions for search**: Search types (`ServerSearchProvider`, `SqlSearchProviderConfig`, etc.) are defined in `infra/src/db/search/types.ts` with re-exports of shared types from `@abe-stack/kernel`. This keeps the package self-contained while sharing core domain types.
...
#### 3. `@monitor/logger` to `@abe-stack/kernel` (8 files)

All 8 files imported `type { Logger }` from the local `@monitor/logger` alias. The contracts `Logger` type (exported from `@abe-stack/kernel` main barrel) is compatible since these files only use `info`, `warn`, `error`, `debug` methods (all present in both Logger interfaces).

**Files**:

- `apps/server/src/infrastructure/jobs/queue/queueServer.ts`
- `apps/server/src/infrastructure/jobs/write/writeService.ts` (merged duplicate `@abe-stack/kernel` type imports)
...
- **Type-check**: `pnpm --filter @abe-stack/kernel type-check` -- 0 errors
- **ESLint**: 0 errors on all source files
...
- Added `@abe-stack/kernel` as workspace dependency for the `Logger` type used by queue/retry modules
...
- Added `@abe-stack/kernel/contracts` dependency to `infra/http`

#### Phase 2: Consolidate Package Types (5 packages)

For each package, replaced duplicate Logger/RequestInfo/RequestWithCookies definitions with imports from `@abe-stack/kernel/contracts`, using transition aliases for backward compatibility:
...
**tsconfig resolution**: Removed `@abe-stack/kernel/contracts` from all package tsconfig `paths` entries. Packages resolve contracts via `node_modules` symlinks to built `dist/` output, avoiding rootDir violations.

#### Phase 3: Server AppContext + BaseContext Compliance

- Added `import type { BaseContext } from '@abe-stack/kernel/contracts'` to `apps/server/src/shared/types.ts`
- Added `@abe-stack/kernel/contracts` to server's `package.json` dependencies
...
| -------------------------- | ---------- | ------------------------ |
| `@abe-stack/kernel/contracts`     | 0 errors   | N/A                      |
...
| **SDK vitest config**    | Added resolve aliases for `@abe-stack/kernel` to prevent duplicate module instances causing `instanceof` failures (38 tests)            |
...
- `IServiceContainer.cache` type changed from `import('@abe-stack/kernel').CacheProvider` → `import('@abe-stack/cache').CacheProvider` (type was moved to cache package during earlier restructure)
...
- **`apps/server/src/app.ts`** — Redirected imports:
  ...
  - `NotificationService` type from `@abe-stack/kernel` (contracts-level interface)
...
- `NotificationService` from `@abe-stack/kernel/contracts` (re-exported via `@abe-stack/kernel`) is the correct domain interface for `IServiceContainer.notifications`. The infra package's `NotificationProviderService` structurally satisfies it.
...
- Logger imports from `@abe-stack/kernel/infrastructure/logger` (core package internal path)
...
- **`shared/core/vitest.config.ts`**: Updated `@abe-stack/kernel/contracts` path alias from `../contracts` → `../../infra/contracts` to match new directory structure
...
- **Inlined `UserRole` type**: `seed.ts` replaces `import type { UserRole } from '@abe-stack/kernel'` with a local `type SeedUserRole = 'admin' | 'user'` to avoid pulling `@abe-stack/kernel` as a tools dependency for a single union type.
...
- `logger/types.ts` — deleted (consumers import from `@abe-stack/kernel/infrastructure/logger`)
- `logger/index.ts` — stripped to local Fastify-specific exports only

Updated all consumers to import directly from packages (`@abe-stack/kernel`, `@abe-stack/infra`, `@abe-stack/billing`).
...
**Objective**: Verify that `@abe-stack/kernel` contains only domain-agnostic primitives and contracts with no business logic leakage.
...
- Removed duplicate `@abe-stack/kernel` dependency in `client/api/package.json`
...
- `pnpm type-check --filter @abe-stack/kernel` passed
