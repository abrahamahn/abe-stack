...
- [Saturday (01-31)](#saturday-january-31-2026) — Server migration completion, module extraction (admin/billing/users), monorepo error fix, code refactoring plan, UI hooks audit, BaseContext contract, ops/security/quickstart docs, directory restructure (`packages/` → `infra/`, `modules/`, `shared/`, `sdk/`), migrate `@abe-stack/stores` to `client/stores`, rename server `infrastructure/` → `infra/`, rename `sdk/` → `client/` (`@abe-stack/sdk` → `@abe-stack/client`)
