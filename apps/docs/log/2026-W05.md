...
- [Saturday (01-31)](#saturday-january-31-2026) — Server migration completion, module extraction (admin/billing/users), monorepo error fix, code refactoring plan, UI hooks audit, BaseContext contract, ops/security/quickstart docs, directory restructure (`packages/` → `infra/`, `modules/`, `shared/`, `sdk/`), migrate `@abe-stack/stores` to `client/stores`, rename server `infrastructure/` → `infra/`, rename `sdk/` → `client/` (`@abe-stack/sdk` → `@abe-stack/client`), config migration to packages, flatten `server/src/config/`, remove dead `packageManager` config, move DB scripts to `tools/scripts/db/`
...
- `IServiceContainer.cache` type changed from `import('@abe-stack/core').CacheProvider` → `import('@abe-stack/cache').CacheProvider` (type was moved to cache package during earlier restructure)
...
- Added `vi.mock('@abe-stack/db')` — fixed pre-existing bug where `requireValidSchema`, `createDbClient`, and `getRepositoryContext` were mocked inside `./infrastructure/index` but `app.ts` imports them from `@abe-stack/db`
- Updated `requireValidSchema` references in start tests from `./infrastructure/index` → `@abe-stack/db`
...
- The `@abe-stack/http` router and Fastify's native typed route generics already handle parameter extraction with proper type safety — these helpers bypass that system
...
| `utils/request-utils.ts`         | Delete (future)   | Dead code, superseded by `@abe-stack/http` router                       |
...
- **`apps/server/src/app.ts`** — Redirected imports:
  ...
  - `NotificationService` type from `@abe-stack/core` (contracts-level interface)
...
- `NotificationService` from `@abe-stack/contracts` (re-exported via `@abe-stack/core`) is the correct domain interface for `IServiceContainer.notifications`. The infra package's `NotificationProviderService` structurally satisfies it.
...
- Logger imports from `@abe-stack/core/infrastructure/logger` (core package internal path)
...
### Rename `sdk/` → `client/` (`@abe-stack/sdk` → `@abe-stack/client`)

Renamed the SDK directory and package from `sdk/` to `client/` and updated all cross-codebase references from `@abe-stack/sdk` to `@abe-stack/client`.
...
| Dependencies    | `apps/web/package.json`, `shared/ui/package.json`                                                                         | `@abe-stack/sdk` → `@abe-stack/client`                                   |
...
| Docker          | `ops/docker/Dockerfile.web`                                                                                               | Build filter `@abe-stack/client`                                         |
| Tools/Scripts   | 9 files in `tools/scripts/`                                                                                               | Export configs, path scripts, audit scripts, test runner                 |
| Source code     | ~150 TypeScript/TSX files                                                                                                 | All `import from '@abe-stack/sdk'` → `'@abe-stack/client'`               |
| Test mocks      | ~30 test files                                                                                                            | `vi.mock('@abe-stack/sdk')` → `vi.mock('@abe-stack/client')`             |
...
**Net result**: 8 barrel files → 3 barrels. 4-level nesting → 2-level. Dead `infra/search/` code deleted (duplicated in `@abe-stack/db`).

#### Dead Code Removal

- Deleted `infra/search/` — 5 files duplicated in `infra/db/src/search/` (already exported by `@abe-stack/db`)
...
| File                        | Old Import                                      | New Import           |
| --------------------------- | ----------------------------------------------- | -------------------- |
| `app.ts`                    | `getSearchProviderFactory` from `./infra/index` | from `@abe-stack/db` |
| `app.ts`                    | `logStartupSummary` from `./infra/index`        | from `@health`       |
| `app.ts`                    | `registerRoutes` from `./modules/index`         | from `@routes`       |
| `types/context.ts`          | `ServerSearchProvider` from `@/infra/index`     | from `@abe-stack/db` |
...
- **`shared/core/vitest.config.ts`**: Updated `@abe-stack/contracts` path alias from `../contracts` → `../../infra/contracts` to match new directory structure
...
| `config/infra/database.ts`         | `infra/db/src/config/database.ts` (already existed)       |
| `config/infra/cache.ts`            | `infra/cache/src/config.ts`                               |
| `config/infra/queue.ts`            | `infra/jobs/src/config.ts`                                |
| `config/infra/server.ts`           | `infra/http/src/config.ts`                                |
| `config/infra/storage.ts`          | `infra/storage/src/config.ts`                             |
| `config/services/billing.ts`       | `modules/billing/src/config.ts`                           |
| `config/services/email.ts`         | `infra/email/src/config.ts`                               |
...
| Source                      | Target                          |
| --------------------------- | ------------------------------- |
| `config/services/search.ts` | `infra/db/src/config/search.ts` |

Search config moved to `@abe-stack/db` since it co-locates with search providers (SQL, Elasticsearch) already in that package.
...
- **Inlined `UserRole` type**: `seed.ts` replaces `import type { UserRole } from '@abe-stack/core'` with a local `type SeedUserRole = 'admin' | 'user'` to avoid pulling `@abe-stack/core` as a tools dependency for a single union type.
