...
- **Config duplication preserved**: `packages/backend-core/src/config/` still has copies alongside `packages/shared/src/config/`. ~~Backend-core should re-export from shared in a future pass.~~ **RESOLVED**: Config consolidation (2026-02-03) deleted `packages/shared/src/config/` and made `packages/backend-core/src/config/` the single source of truth.
...
3. ~~`packages/backend-core/src/config/` should re-export from `@abe-stack/shared/config`~~ **DONE**: Config consolidation deleted `packages/shared/src/config/`, `packages/backend-core/src/config/` is now canonical
...
**Objective**: Consolidate configuration to two canonical locations — `packages/backend-core/src/config/` (types, schemas, parsers, loaders) and `apps/server/src/config/` (domain-specific loaders & factory) — eliminating all scattered config duplicates.

#### Step 1: Add `./config` Subpath Export

Added `./config` subpath export to `packages/backend-core/package.json`, enabling `import { AuthConfig } from '@abe-stack/backend-core/config'`.

#### Step 2: Rewire Imports (~60 files)

| Category                                   | Files | Change                                                                                             |
| ------------------------------------------ | ----- | -------------------------------------------------------------------------------------------------- |
| `apps/server/src/config/`                  | 24    | `@abe-stack/shared/config` → `@abe-stack/backend-core/config`                                      |
| `apps/server/src/` (app, server, tests)    | 6     | `AppConfig` from `@abe-stack/shared` → `@abe-stack/backend-core/config`                            |
| `modules/auth/src/`                        | ~15   | `AuthConfig`, `AuthStrategy`, `OAuthProviderConfig` → `@abe-stack/backend-core/config`             |
| `apps/server/src/modules/auth/`            | ~15   | Same as above (server duplicates)                                                                  |
| `modules/billing/src/`                     | 8     | `BillingConfig`, `StripeProviderConfig`, `PayPalProviderConfig` → `@abe-stack/backend-core/config` |
| `apps/server/src/modules/billing/`         | 8     | Same as above (server duplicates)                                                                  |
| `modules/admin/src/types.ts`               | 1     | `BillingConfig` → `@abe-stack/backend-core/config`                                                 |
| `modules/users/src/types.ts`               | 1     | `Argon2Config` → `@abe-stack/backend-core/config`                                                  |
| `modules/system/src/types.ts`              | 1     | `AppConfig` → `@abe-stack/backend-core/config`                                                     |
| `packages/backend-core/src/mailer/smtp.ts` | 1     | `@abe-stack/shared/config` → relative `../config`                                                  |
...
Added `Argon2Config` type alias (`AuthConfig['argon2']`) to `packages/backend-core/src/config/types/auth.ts` and exported it. This type was referenced by password utilities but never existed as a standalone export.

#### Step 5: Delete Scattered Config Files (~30 files)

| Location                                                 | Action                                                                       |
| -------------------------------------------------------- | ---------------------------------------------------------------------------- |
| `packages/shared/src/config/` (entire directory)         | Deleted — duplicate of `packages/backend-core/src/config/`                   |
...
- **Two canonical locations**: `packages/backend-core/src/config/` (types/schemas/parsers — reusable core) and `apps/server/src/config/` (domain loaders — app-specific). No other location should define config types or loaders.
- **Config types vs domain types**: Config types (`AuthConfig`, `BillingConfig`, `StripeProviderConfig`, etc.) moved to `@abe-stack/backend-core/config`. Domain types (`BillingService`, `NormalizedWebhookEvent`, `BillingProviderNotConfiguredError`, etc.) remain in `@abe-stack/shared`.
...
- **Final fix (02-04)**: Fixed last `AppConfig` import in `apps/server/src/types/context.ts` — moved from `@abe-stack/shared` to `@abe-stack/backend-core/config`, merged duplicate `@abe-stack/shared` imports, fixed import ordering.
...
The auto-formatter resolved conflicts between `core` and `domain` exports by aliasing:

- `userSchema` (domain) → `domainUserSchema`
- `PaginationError` (utils) → `UtilPaginationError`
- `WebSocket` (pubsub) → `PubSubWebSocket`
- `RequestInfo` (http) → `HttpRequestInfo`
...
| Namespaced `export * as` (kept)     | 9                  |
| Type-check (`@abe-stack/shared`)    | 0 errors           |
| Test files                          | 86/86 passed       |
| Total tests                         | 2,275/2,275 passed |
...
- **Conflict aliases in root barrel**: Rather than omitting conflicting symbols, the auto-formatter resolved collisions with semantic prefixes (`domainUserSchema`, `UtilPaginationError`). This preserves the ability to import any symbol from the package root.

---

### Config Helpers DRY: Consolidate `isStrategyEnabled` / `getRefreshCookieOptions`

**Objective**: Eliminate triple-duplication of `isStrategyEnabled` and `getRefreshCookieOptions` by moving them to the canonical config location: `packages/backend-core/src/config/`.
...
All three were identical implementations importing `AuthConfig`/`AuthStrategy` from `@abe-stack/backend-core/config`.

#### Solution

Created a single canonical file at `packages/backend-core/src/config/auth-helpers.ts` and updated all consumers.

#### Files Created (2)

- `packages/backend-core/src/config/auth-helpers.ts` — canonical implementation
- `packages/backend-core/src/config/auth-helpers.test.ts` — 12 tests (moved from module copy)
...
- `packages/backend-core/src/config/index.ts` — added `getRefreshCookieOptions`, `isStrategyEnabled` exports
...
- `modules/auth/src/utils/cookies.ts` — import from `@abe-stack/backend-core/config`
- `modules/auth/src/magic-link/handlers.ts` — import from `@abe-stack/backend-core/config`
- `apps/server/src/modules/auth/utils/cookies.ts` — import from `@abe-stack/backend-core/config`
- `apps/server/src/modules/auth/magic-link/handlers.ts` — import from `@abe-stack/backend-core/config`
- `modules/auth/src/magic-link/handlers.test.ts` — updated `vi.mock` and inline imports to `@abe-stack/backend-core/config`
...
- **Single source of truth in `packages/backend-core/src/config/`**: These functions operate purely on `AuthConfig` types already defined in the same package. Zero new dependencies introduced. All consumers now import from `@abe-stack/backend-core/config`.
