...
#### Client-Side Integration (`shared/ui/src/hooks/useFileUpload.ts`)
...
#### Core Media Utilities (`shared/core/src/media/`)
...
#### Token Storage Security (`shared/core/src/utils/index.ts`)
...
#### `shared/core/src/utils/storage.ts` (new file)
...
- `@abe-stack/shared/crypto` - Server-only JWT utilities (uses `node:crypto`)
- `@abe-stack/shared` - All browser-safe exports including `normalizeStorageKey`
...
- `.config/tsconfig.*.json` base configs
...
- Path aliases defined in `tooling/sync/sync-aliases.ts` (Vite alias exports + tsconfig sync)
...
- Replaced `nanoid` with native `crypto.randomUUID()` in `shared/core/src/stores/toastStore.ts`
- Removed `nanoid` from `shared/core/package.json`
...
**New File: `apps/docs/dev/legacy.md` (417 lines)**
...
**Reorganized `apps/docs/TODO.md` (298 lines):**
...
**Reorganized `apps/docs/ROADMAP.md` (383 lines):**
...
Consolidated two parallel error systems into a single, feature-rich error module in `@abe-stack/shared`.

**Problem Solved:**

- `shared/core/src/errors.ts` had simple `HttpError` class (8 error types, no code/details)
...
**New Structure (`shared/core/src/errors/`):**
...
- `shared/core/src/contracts/common.ts` - `errorResponseSchema` now includes error, code, details fields
- `apps/server/src/shared/index.ts` - Re-exports errors from `@abe-stack/shared`

**Deleted:**

- `shared/core/src/errors.ts` (old HttpError system)
...
- Created `shared/core/src/validation/passwordStrength.ts` (~5KB)
...
- Created `client/src/persistence/idb.ts` (~100 lines)
...
| ------------- | -------------------- | ------- |
| @ts-rest/core | 3.45.1 (shared/core) | ^3.52.1 |
...
- Applies to: `shared/core`, `sdk`, `shared/ui`
...
#### `apps/docs/dev/config-setup.md` (~725 lines) - Core configuration
...
#### `apps/docs/dev/sync-scripts.md` (~450 lines) - DX automation scripts
...
#### `apps/docs/dev/dev-environment.md` (~350 lines) - Development & CI
...
Updated `apps/docs/INDEX.md` with new document references and keyword routing.
...
- Fixed `@abeahn/ui` → `@abe-stack/ui` in `shared/ui/README.md`
- Fixed `@abeahn/shared` → `@abe-stack/shared` in `shared/ui/README.md`
- Fixed `@abeahn-ui` → `@abe-stack/ui` in `apps/docs/dev/testing.md`
- Fixed `@abeahn/ui` → `@abe-stack/ui` in `shared/ui/docs/_TEMPLATE.md`

#### Security Documentation Updates

- Fixed bcrypt → Argon2id in `apps/docs/README.md` (reflects actual implementation)
- Updated `apps/docs/todo/security/index.md` to reflect completed Phase 1 features
- Updated password hashing references throughout security docs

#### Test Count Updates

- Updated test count from `1000+` to `1900+` in `apps/docs/README.md`
- Updated test coverage table in `apps/docs/dev/testing.md`:
  - shared/ui: 78 files, 772+ tests
  - shared/core: 8 files, 280+ tests
  - sdk: 5 files, 48+ tests
  - apps/web: 25 files, 337+ tests
  - apps/server: 36 files, 508+ tests

#### Path Reference Fixes in `apps/docs/INDEX.md`
...
- Updated all outdated dates (January 10/15) to January 17, 2026 across:
  ...
  - `apps/docs/INDEX.md`
  - `apps/docs/todo/security/*.md`
  - `apps/docs/todo/realtime/*.md`
  - `apps/docs/todo/ui/todo.md`

#### Testing Documentation

- Updated test example syntax from Jest to Vitest in `shared/ui/README.md`
...
- Changed from package paths (`@abe-stack/ui/docs/...`) to relative paths (`../../../../../../shared/ui/docs/...`)
...
- Uncompleted items are covered by `apps/docs/TODO.md` (more detailed product roadmap)
- Advanced features (CHET-Stack) are in `apps/docs/ROADMAP.md` Milestone 2
- Security features (MFA, RBAC) are in `apps/docs/ROADMAP.md` Milestone 3
- Single source of truth: `apps/docs/TODO.md` for product features, `apps/docs/ROADMAP.md` for architectural milestones
...
- `shared/core/src/index.ts` - All exports now explicit
- `shared/core/src/contracts/index.ts` - All exports now explicit
- `shared/core/src/validation/index.ts` - All exports now explicit
- `client/src/index.ts` - All exports now explicit
- `shared/ui/src/index.ts` - Manually converted (preserves namespace exports like `export * as elements`)
- `shared/ui/src/layouts/index.ts` - All exports now explicit
...
- Fixed `apps/web/src/features/demo/utils/lazyDocs.ts`: Changed glob imports from package alias (`@abe-stack/ui/docs/...`) to relative paths (`../../../../../../shared/ui/docs/...`) since UI package exports don't include docs folder
...
Added `shared/core/src/constants/` module with shared constants:
