...
- [Thursday (01-23)](#thursday-01-23) - Package extraction, custom router, @abe-stack/infra
...
| @abe-stack/web     | 793       |
| @abe-stack/ui      | 1,394     |
| @abe-stack/engine  | 672       |
| @abe-stack/shared    | 1,592     |
...
### @abe-stack/infra (Raw SQL Query Builder)
...
| `drizzle-orm`, `drizzle-kit` | runtime | Replaced with @abe-stack/infra |
...
```
shared/core/src/contracts/billing.ts    → API contracts & validation
infra/src/db/schema/billing.ts         → Database types
infra/src/db/repositories/billing/     → Data access layer
...
client/src/billing/                 → API client + React hooks
shared/ui/src/components/billing/       → UI components
...
```
shared/core/src/contracts/users.ts      → Profile/session contracts & validation
infra/src/db/schema/auth.ts            → Session metadata (ipAddress, userAgent)
infra/src/db/schema/users.ts           → Avatar URL field
...
- Web admin hooks tests now mock `@abe-stack/engine` query helpers and client config to satisfy strict TS.
...
| :------------------------ | :-------------------------------------------------------- | :---------------------------------------- |
| **Server Code Isolation** | Moved PubSub/Node logic to `@abe-stack/shared/pubsub`       | Fixed `Buffer is not defined` in browser  |
...
| **Simplified API Surface** | Consolidated `@abe-stack/shared/config` entry point  | Eliminated deep imports and cross-package dependency mess |
