...
- [Thursday (01-23)](#thursday-01-23) - Package extraction, custom router, @abe-stack/db
...
infra/db/src/schema/billing.ts         → Database types
infra/db/src/repositories/billing/     → Data access layer
...
infra/db/src/schema/auth.ts            → Session metadata (ipAddress, userAgent)
infra/db/src/schema/users.ts           → Avatar URL field
...
- Web admin hooks tests now mock `@abe-stack/client` query helpers and client config to satisfy strict TS.
