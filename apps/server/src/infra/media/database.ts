// apps/server/src/infra/media/database.ts
/**
...
export class InMemoryMediaDatabase implements MediaDatabaseAdapter {
  ...
  getUserMediaFiles(
    ...
    for (const record of this.records.values()) {
      if (record.ownerId === userId) {
        if (!contentType || record.contentType?.startsWith(contentType)) {
          files.push(record);
        }
      }
    }
  ...
  getFileById(fileId: string, userId?: string): Promise<MediaProcessingRecord | null> {
    ...
    if (userId && record.ownerId !== userId) return Promise.resolve(null);
