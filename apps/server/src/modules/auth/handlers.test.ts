  ...
  mockMapErrorToResponse: vi.fn((error: unknown, ctx: { log?: { error?: unknown } }) => {
    ...
    if (error instanceof Error) {
      switch (error.name) {
        ...
        default:
          if (log !== undefined && log !== null && typeof log.error === 'function') {
            (log.error as (obj: object, msg: string) => void)(
              { error: error.message },
              'Internal error',
            );
          }
    ...
    if (log !== undefined && log !== null && typeof log.error === 'function') {
      (log.error as (obj: object, msg: string) => void)({ error: 'Unknown' }, 'Internal error');
    }
    return { status: 500, body: { message: ERROR_MESSAGES.INTERNAL_ERROR } };
  }),
}));
