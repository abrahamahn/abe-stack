...
import { MAX_PROGRESSIVE_DELAY_MS, PROGRESSIVE_DELAY_WINDOW_MS } from './constants';
 ...
 */
export async function isAccountLocked(db: DbClient, email: string): Promise<boolean> {
 ...
 */
export async function getProgressiveDelay(db: DbClient, email: string): Promise<number> {
 ...
 */
export async function applyProgressiveDelay(db: DbClient, email: string): Promise<void> {
  const delay = await getProgressiveDelay(db, email);
  if (delay > 0) {
 ...
 */
export async function getAccountLockoutStatus(db: DbClient, email: string): Promise<LockoutStatus> {
  const lockoutWindow = new Date(Date.now() - authConfig.lockout.lockoutDurationMs);
  const failedAttempts = await countFailedAttempts(db, email, lockoutWindow);
