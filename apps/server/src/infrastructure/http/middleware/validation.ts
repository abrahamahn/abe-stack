// apps/server/src/infrastructure/http/middleware/validation.ts
/**
 ...
 * will NOT trigger false positives. The patterns look for SQL syntax context:
 * - UNION SELECT (SQL injection pattern)
 * - SELECT ... FROM (query structure)
 * - SQL comment sequences (--, /*, *\/)
 * - Classic tautologies (OR 1=1, AND 1=1)
 * - Escaped hex sequences commonly used in attacks
 *
 * If you need to allow specific patterns for an endpoint (e.g., a code editor),
 * use the SQLInjectionDetectionOptions.enabled = false option.
 */
