// apps/server/src/infrastructure/data/files/fastify-server.ts
/**
...
/**
 * Validate that an ID contains only safe characters (alphanumeric and hyphens).
 * Prevents path traversal attacks via malicious IDs like "../../../etc/passwd".
 */
 ...
 * Rejects hidden files (starting with dot) and path traversal attempts.
 */
function isValidFilename(filename: string): boolean {
  // Must not start with a dot (hidden files) or contain path separators
...
/**
 * Verify that a resolved file path is contained within the upload directory.
...
export function registerFileServer(config: FilesConfig, app: FastifyInstance): void {
  ...
  app.put<{ Params: FileRouteParams; Querystring: FileRouteQuery }>(
    ...
    async (request, reply) => {
      ...
      // Validate ID and filename to prevent path traversal
      if (!isValidId(id)) {
      ...
      // Defense-in-depth: verify path is contained within upload directory
      ...
      try {
        ...
        // Check if body is already parsed (e.g., from inject() or content type parser)
        // If so, write the buffer directly; otherwise stream from raw request
  ...
  app.get<{ Params: FileRouteParams; Querystring: FileRouteQuery }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        // For smaller files or when response is being collected (e.g., during testing),
        // read the entire file into memory. For larger files, stream directly.
