...
describe('SqlSearchProvider', () => {
  ...
  describe('constructor', () => {
    test('should create provider with default ', () => {
      const provider = new SqlSearchProvider(mockDb, mockRepos, table);

      expect(provider.name).toBe('sql');
    });

    test('should create provider with custom ', () => {
      const provider = new SqlSearchProvider(mockDb, mockRepos, table, {
        name: 'custom-sql',
        maxPageSize: 500,
      });
  ...
  describe('createSqlSearchProvider factory', () => {
    test('should create provider with factory function', () => {
      const provider = createSqlSearchProvider(mockDb, mockRepos, table);
