...
describe('listen function', () => {
  it('should use fallback port when primary port is in use', async () => {
    ...
    // Mock listen to avoid binding real ports
    server1.listen = vi.fn().mockResolvedValue(undefined);
    server2.listen = vi.fn().mockImplementation(async ({ port }: { port: number }) => {
      if (port === port1) {
    ...
    await listen(server2, WithFallback);
  ...
  it('should throw error when no ports are available', async () => {
    ...
    // Mock listen to avoid binding real ports
    server1.listen = vi.fn().mockResolvedValue(undefined);
    server2.listen = vi.fn().mockResolvedValue(undefined);
    server3.listen = vi.fn().mockRejectedValue({ code: 'EADDRINUSE' });

    // Try to listen on server3 with no available ports
    const { listen } = await import('../server.js');
    await expect(listen(server3, WithFallback)).rejects.toThrow('No available ports');
  ...
  it('should deduplicate port list', async () => {
    ...
    const { listen } = await import('../server.js');
    await listen(server, WithDuplicates);
  ...
  it('should propagate non-EADDRINUSE errors', async () => {
    ...
    const { listen } = await import('../server.js');
    await expect(listen(server, test)).rejects.toMatchObject({ code: 'EACCES' });
