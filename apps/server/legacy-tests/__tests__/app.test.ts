...
vi.mock('@/services/service-container', () => {
  class ServiceContainer {
    ...
    constructor(options: ServiceContainerOptions) {
      ...
      this.pubsub = options.pubsub ?? { setAdapter: vi.fn() };
      this.cache = options.cache ?? { get: vi.fn(), set: vi.fn(), delete: vi.fn() };
      this.pgPubSub = null;
    }
...
describe('App', () => {
  ...
  describe('context', () => {
    ...
    it('should provide service container via serviceContainer property', () => {
      app = createApp(mock);
