...
import type { Lockout, LockoutStatus } from '../types';
...
describe('Lockout Functions', () => {
  ...
  describe('applyProgressiveDelay', () => {
    test('should not delay when no progressive delay needed', async () => {
      ...
      await applyProgressiveDelay(asMockDb(mockDb), 'test@example.com', defaultLockout);
  ...
  describe('concurrent login attempts', () => {
    ...
    test('should handle concurrent lockout status checks', async () => {
      ...
      const promises = [
        isAccountLocked(asMockDb(mockDb), 'test@example.com', defaultLockout),
        isAccountLocked(asMockDb(mockDb), 'test@example.com', defaultLockout),
        isAccountLocked(asMockDb(mockDb), 'test@example.com', defaultLockout),
      ];
    ...
    test('should handle concurrent progressive delay calculations', async () => {
      ...
      const promises = [
        getProgressiveDelay(asMockDb(mockDb), 'test@example.com', defaultLockout),
        getProgressiveDelay(asMockDb(mockDb), 'test@example.com', defaultLockout),
        getProgressiveDelay(asMockDb(mockDb), 'test@example.com', defaultLockout),
      ];
