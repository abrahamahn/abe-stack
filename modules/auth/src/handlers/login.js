// modules/auth/src/handlers/login.ts
/**
 * Login Handler
 *
 ...
 * @complexity O(1)
 */
export async function handleLogin(ctx, body, request, reply) {
    const { ipAddress, userAgent } = request.requestInfo;
    try {
        const { email, password } = body;
        const result = await authenticateUser(ctx.db, ctx.repos, ctx.config.auth, email, password, ctx.log, ipAddress, userAgent, (userId) => {
