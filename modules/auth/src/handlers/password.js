// modules/auth/src/handlers/password.ts
/**
 * Password Handlers
 *
 ...
 * @complexity O(1)
 */
export async function handleForgotPassword(ctx, body) {
    try {
        const { email } = body;
        ...
        if (error instanceof Error && error.name === 'EmailSendError') {
            ...
            ctx.log.error({ email: body.email, originalError: emailError.originalError?.message }, 'Failed to send password reset email');
 ...
 * @complexity O(1)
 */
export async function handleResetPassword(ctx, body) {
    try {
 ...
 * @complexity O(1)
 */
export async function handleSetPassword(ctx, body, req) {
    try {
