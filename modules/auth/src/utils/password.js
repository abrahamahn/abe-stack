// modules/auth/src/utils/password.ts
/**
 ...
 * @complexity O(1)
 */
export function needsRehash(hash, config = DEFAULT_ARGON2_CONFIG) {
 ...
 */
export async function initDummyHashPool(config = DEFAULT_ARGON2_CONFIG) {
 ...
 */
async function getRandomDummyHash(config = DEFAULT_ARGON2_CONFIG) {
    if (dummyHashPool.length > 0) {
        const randomIndex = Math.floor(Math.random() * dummyHashPool.length);
        return dummyHashPool[randomIndex];
    }
 ...
 * @complexity O(1)
 */
export function isDummyHashPoolInitialized() {
 ...
 * @complexity O(1)
 */
export function resetDummyHashPool() {
    dummyHashPool = [];
 ...
 */
export async function verifyPasswordSafe(password, hash) {
    const hashToVerify = hash !== undefined && hash !== null && hash !== '' ? hash : await getRandomDummyHash();
