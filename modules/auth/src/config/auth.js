// modules/auth/src/config/auth.ts
/**
 ...
 */
export function loadAuthConfig(env, apiBaseUrl) {
    const isProduction = env.NODE_ENV === 'production';
    const jwtSecret = env.JWT_SECRET;
    const buildUrl = (path) => `${apiBaseUrl.replace(/\/$/, '')}${path}`;
    // Helper to build OAuth config object conditionally
    const buildOAuthProviders = () => {
        const providers = {};
        const googleConfig = createOAuth(env.GOOGLE_CLIENT_ID, env.GOOGLE_CLIENT_SECRET, env.GOOGLE_CALLBACK_URL ?? buildUrl('/api/auth/oauth/google/callback'));
        if (googleConfig !== undefined) {
            providers.google = googleConfig;
        }
        const githubConfig = createOAuth(env.GITHUB_CLIENT_ID, env.GITHUB_CLIENT_SECRET, env.GITHUB_CALLBACK_URL ?? buildUrl('/api/auth/oauth/github/callback'));
        if (githubConfig !== undefined) {
            providers.github = githubConfig;
        }
        const facebookConfig = createOAuth(env.FACEBOOK_CLIENT_ID, env.FACEBOOK_CLIENT_SECRET, env.FACEBOOK_CALLBACK_URL ?? buildUrl('/api/auth/oauth/facebook/callback'));
    ...
    const config = {
        ...
        password: {
            ...
            minZxcvbnScore: env.PASSWORD_MIN_SCORE,
        },
        lockout: {
...
export function validateAuthConfig(config) {
    ...
    if (lockout.maxAttempts < 3 || lockout.maxAttempts > 20)
        throw new AuthValidationError('Lockout attempts must be between 3 and 20', 'lockout.maxAttempts');
    if (lockout.lockoutDurationMs < 60000)
        throw new AuthValidationError('Lockout duration must be at least 60000ms', 'lockout.lockoutDurationMs');
    if (refreshToken.expiryDays < 1 || refreshToken.expiryDays > 30)
        throw new AuthValidationError('Refresh expiry must be between 1 and 30 days', 'refreshToken.expiryDays');
    if (password.minLength < 8)
        throw new AuthValidationError('Min password length must be at least 8 characters', 'password.minLength');
}
/**
 ...
 */
export function isStrategyEnabled(config, strategy) {
    return config.strategies.includes(strategy);
}
/**
 * Create an OAuth provider configuration from environment variables.
 *
 * @param id - Client ID from environment
 * @param secret - Client secret from environment
 * @param callbackUrl - Callback URL for the provider
 * @param extra - Additional provider-specific configuration
 * @returns OAuth provider config or undefined if not configured
 * @complexity O(1)
 */
function createOAuth(id, secret, callbackUrl, extra) {
    if (id === undefined || id === '')
        return undefined;
    return {
        clientId: id,
