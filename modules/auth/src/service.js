...
import { AccountLockedError, EmailNotVerifiedError, EmailSendError, InvalidCredentialsError, InvalidTokenError, validatePassword, WeakPasswordError, } from '@abe-stack/core';
import { and, EMAIL_VERIFICATION_TOKENS_TABLE, eq, insert, isNull, PASSWORD_RESET_TOKENS_TABLE, toCamelCase, update, USER_COLUMNS, USERS_TABLE, withTransaction, } from '@abe-stack/db';
import { applyProgressiveDelay, getAccountLockoutStatus, isAccountLocked, logAccountLockedEvent, logLoginAttempt, } from './security';
import { createAccessToken, createAuthResponse, createRefreshTokenFamily, hashPassword, needsRehash, rotateRefreshToken as rotateRefreshTokenUtil, verifyPasswordSafe, } from './utils';
// ============================================================================
// Service Functions
// ============================================================================
/**
 ...
 */
export async function registerUser(db, repos, emailService, emailTemplates, config, email, password, name, baseUrl) {
    ...
    if (existingUser !== null) {
        ...
        return {
            ...
            message: 'Registration successful! Please check your email inbox and click the confirmation link to complete your registration.',
            email,
        };
    }
    ...
    const { user, verificationToken } = await withTransaction(db, async (tx) => {
        ...
        const newUser = toCamelCase(newUserRows[0], USER_COLUMNS);
    ...
    try {
        ...
        throw new EmailSendError('Failed to send verification email', error instanceof Error ? error : new Error(String(error)));
    }
    return {
        status: 'pending_verification',
        message: 'Registration successful! Please check your email inbox and click the confirmation link to complete your registration.',
 ...
 * @complexity O(1) - constant database operations
 */
export async function authenticateUser(db, repos, config, email, password, logger, ipAddress, userAgent, onPasswordRehash) {
    ...
    // Create access token
    const accessToken = createAccessToken(user.id, user.email, user.role, config.jwt.secret, config.jwt.accessTokenExpiry);
 ...
 * @complexity O(1) - constant database operations
 */
export async function refreshUserTokens(db, _repos, config, oldRefreshToken, ipAddress, userAgent) {
    const result = await rotateRefreshTokenUtil(db, oldRefreshToken, ipAddress, userAgent, config.refreshToken.expiryDays, config.refreshToken.gracePeriodSeconds);
    if (result === null) {
        throw new InvalidTokenError();
    }
    // Create new access token
    const accessToken = createAccessToken(result.userId, result.email, result.role, config.jwt.secret, config.jwt.accessTokenExpiry);
    return {
        accessToken,
 ...
 * @complexity O(1)
 */
export async function logoutUser(_db, repos, refreshToken) {
 ...
 * @complexity O(1)
 */
async function handleFailedLogin(db, config, email, reason, ipAddress, userAgent) {
 ...
 */
function rehashPassword(db, config, userId, password, logger, callback, retryCount = 3) {
    // Fire and forget - don't block login
    hashPassword(password, config.argon2)
        .then((newHash) => db.execute(update(USERS_TABLE).set({ password_hash: newHash }).where(eq('id', userId)).toSql()))
 ...
 * @complexity O(1)
 */
export async function requestPasswordReset(db, repos, emailService, emailTemplates, email, baseUrl) {
    ...
    await withTransaction(db, async (tx) => {
        ...
        await tx.execute(update(PASSWORD_RESET_TOKENS_TABLE)
            .set({ used_at: new Date() })
            .where(and(eq('user_id', user.id), isNull('used_at')))
            .toSql());
        // Create a new password reset token
        await tx.execute(insert(PASSWORD_RESET_TOKENS_TABLE)
            .values({
            user_id: user.id,
            token_hash: hash,
    ...
    try {
        ...
        throw new EmailSendError('Failed to send password reset email', error instanceof Error ? error : new Error(String(error)));
    }
}
/**
 ...
 * @complexity O(1)
 */
export async function resetPassword(db, repos, config, token, newPassword) {
    ...
    await withTransaction(db, async (tx) => {
        await tx.execute(update(USERS_TABLE)
 ...
 * @complexity O(1)
 */
export async function setPassword(_db, repos, config, userId, newPassword) {
 ...
 * @complexity O(1)
 */
export async function createEmailVerificationToken(dbOrRepos, userId) {
    const { plain, hash } = await generateSecureToken();
    const expiresAt = new Date(Date.now() + TOKEN_EXPIRY_HOURS * 60 * 60 * 1000);
    ...
    if ('emailVerificationTokens' in dbOrRepos) {
        ...
        await dbOrRepos.execute(insert(EMAIL_VERIFICATION_TOKENS_TABLE)
            .values({
            user_id: userId,
            token_hash: hash,
            expires_at: expiresAt,
 ...
 * @complexity O(1)
 */
export async function resendVerificationEmail(db, repos, emailService, emailTemplates, email, baseUrl) {
    // Using repository for user lookup
    const user = await repos.users.findByEmail(email);
    if (user === null) {
        // Don't reveal user doesn't exist - silently succeed
    ...
    const verificationToken = await withTransaction(db, async (tx) => {
        ...
        await tx.execute(update(EMAIL_VERIFICATION_TOKENS_TABLE)
            .set({ used_at: new Date() })
            .where(and(eq('user_id', user.id), isNull('used_at')))
            .toSql());
 ...
 * @complexity O(1) - constant database operations
 */
export async function verifyEmail(db, repos, config, token) {
    // Hash the token to look it up
    const tokenHash = await hashPassword(token, TOKEN_HASH_CONFIG);
    // Find valid token (not expired, not used) - using repository
    ...
    const { user, refreshToken } = await withTransaction(db, async (tx) => {
        ...
        const updatedUserRows = await tx.query(update(USERS_TABLE)
        ...
        const updatedUser = toCamelCase(updatedUserRows[0], USER_COLUMNS);
        // Mark token as used
        await tx.execute(update(EMAIL_VERIFICATION_TOKENS_TABLE)
            .set({ used_at: new Date() })
            .where(eq('id', tokenRecord.id))
        ...
        const { token: refreshTok } = await createRefreshTokenFamily(tx, updatedUser.id, config.refreshToken.expiryDays);
