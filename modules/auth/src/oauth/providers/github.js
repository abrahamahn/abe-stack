 ...
 * @complexity O(1)
 */
export function createGitHubProvider(clientId, clientSecret) {
    return {
        provider: 'github',
        ...
        async exchangeCode(code, redirectUri) {
            ...
            if (!response.ok) {
                ...
                throw new OAuthError(`Failed to exchange code: ${error}`, 'github', 'TOKEN_EXCHANGE_FAILED');
            }
            const data = (await response.json());
            if (data.error != null && data.error !== '') {
                throw new OAuthError(`GitHub OAuth error: ${data.error_description ?? data.error}`, 'github', 'TOKEN_EXCHANGE_FAILED');
            }
        ...
        async getUserInfo(accessToken) {
            ...
            if (email == null || email === '') {
                throw new OAuthError('No email found on GitHub account. Please make your email public or add a verified email.', 'github', 'NO_EMAIL');
            }
            return {
