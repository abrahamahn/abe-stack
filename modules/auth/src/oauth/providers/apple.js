// modules/auth/src/oauth/providers/apple.ts
/**
 ...
 *
 * @returns Array of Apple JWK public keys
 * @throws {OAuthError} If fetching keys fails
 * @complexity O(1) when cached, O(n) network call otherwise
 */
async function fetchApplePublicKeys() {
    ...
    if (!response.ok) {
        throw new OAuthError(`Failed to fetch Apple public keys: ${String(response.status)}`, 'apple', 'KEYS_FETCH_FAILED');
    }
 ...
 *
 * @param jwk - Apple JWK public key
 * @returns PEM-encoded public key
 * @throws {OAuthError} If conversion fails
 * @complexity O(1)
 */
 ...
 * @throws {OAuthError} If JWT format is invalid
 * @complexity O(1)
 */
 ...
 *
 * @param idToken - Apple id_token JWT
 * @throws {OAuthError} If signature verification fails
 * @complexity O(n) where n is the number of Apple public keys
 */
async function verifyIdTokenSignature(idToken) {
    ...
    if (header.alg !== 'RS256') {
        throw new OAuthError(`Unsupported algorithm: ${header.alg}. Expected RS256`, 'apple', 'INVALID_ALGORITHM');
    }
    ...
    if (key == null) {
        throw new OAuthError(`No matching public key found for kid: ${header.kid}`, 'apple', 'KEY_NOT_FOUND');
    }
    // Convert JWK to PEM
 ...
 * @complexity O(1)
 */
async function generateClientSecret(options) {
    ...
    // Base64url encode
    const base64url = (data) => Buffer.from(JSON.stringify(data))
        .toString('base64')
        .replace(/\+/g, '-')
        .replace(/\//g, '_')
        .replace(/=/g, '');
    ...
    const signatureBuffer = sign('sha256', Buffer.from(message), key);
 ...
 * @throws {OAuthError} If token format is invalid
 * @complexity O(1)
 */
function decodeIdTokenPayload(idToken) {
    ...
    try {
        ...
        const payloadStr = Buffer.from(payloadB64 + padding, 'base64').toString('utf8');
 ...
 *
 * @param idToken - Apple id_token JWT
 * @param clientId - Your Apple client_id (Services ID)
 * @returns Verified and decoded payload
 * @throws {OAuthError} If any verification step fails
 * @complexity O(n) where n is Apple's key count
 */
async function verifyAndDecodeIdToken(idToken, clientId) {
    // Step 1: Verify signature
    await verifyIdTokenSignature(idToken);
    // Step 2: Decode payload
    const payloadDecoded = decodeIdTokenPayload(idToken);
    // Step 3: Validate issuer
    if (payloadDecoded.iss !== APPLE_ISSUER) {
        throw new OAuthError(`Invalid issuer: ${payloadDecoded.iss}. Expected: ${APPLE_ISSUER}`, 'apple', 'INVALID_ISSUER');
    }
    // Step 4: Validate audience (must be our client_id)
    if (payloadDecoded.aud !== clientId) {
        throw new OAuthError(`Invalid audience: ${payloadDecoded.aud}. Expected: ${clientId}`, 'apple', 'INVALID_AUDIENCE');
    }
    ...
    return {
        ...
        async exchangeCode(code, redirectUri) {
            ...
            if (data.error != null && data.error !== '') {
                throw new OAuthError(`Apple OAuth error: ${data.error_description ?? data.error}`, 'apple', 'TOKEN_EXCHANGE_FAILED');
            }
            return {
        ...
        async getUserInfo(accessToken) {
            ...
            const payloadDecoded = await verifyAndDecodeIdToken(accessToken, clientId);
            if (payloadDecoded.email == null || payloadDecoded.email === '') {
                throw new OAuthError('No email found in Apple id_token. User may have chosen to hide their email.', 'apple', 'NO_EMAIL');
            }
            // Apple returns email_verified as string "true" or boolean
            const emailVerified = payloadDecoded.email_verified === true || payloadDecoded.email_verified === 'true';
            return {
 ...
 * @complexity O(n) where n is Apple's key count
 */
export async function extractAppleUserFromIdToken(idToken, clientId) {
    // Fully verify the id_token (signature, issuer, audience, expiration)
    const payloadDecoded = await verifyAndDecodeIdToken(idToken, clientId);
    if (payloadDecoded.email == null || payloadDecoded.email === '') {
 ...
 * @complexity O(1)
 */
export function clearAppleKeysCache() {
    appleKeysCache = null;
}
