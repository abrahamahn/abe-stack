// modules/auth/src/oauth/handlers.ts
/**
 * OAuth Handlers
 *
 ...
 */
import { mapErrorToHttpResponse, OAuthError, TooManyRequestsError } from '@abe-stack/core';
import { OAUTH_PROVIDERS } from '@abe-stack/db';
import { authRateLimiters, logOAuthLinkSuccessEvent, logOAuthLoginFailureEvent, logOAuthLoginSuccessEvent, logOAuthUnlinkFailureEvent, logOAuthUnlinkSuccessEvent, } from '../security';
import { createErrorMapperLogger } from '../types';
import { setRefreshTokenCookie } from '../utils';
import { getAuthorizationUrl, getConnectedProviders, handleOAuthCallback, unlinkOAuthAccount, } from './service';
// ============================================================================
// Validation
// ============================================================================
/**
...
function validateProvider(provider) {
    if (!isValidProvider(provider)) {
        throw new OAuthError(`Invalid OAuth provider: ${provider}. Supported: ${OAUTH_PROVIDERS.join(', ')}`, provider, 'INVALID_PROVIDER');
    }
    return provider;
}
/**
 ...
 * @complexity O(1)
 */
function getCallbackUrl(ctx, provider) {
    const providerConfig = ctx.config.auth.oauth[provider];
...
async function checkRateLimit(endpoint, ip) {
    ...
    if (!info.allowed) {
        ...
        throw new TooManyRequestsError(`Rate limit exceeded for ${endpoint}. Please try again in ${String(retryAfter)} seconds.`, retryAfter);
    }
}
// ============================================================================
// Handlers
 ...
 * @complexity O(1)
 */
export async function handleOAuthInitiate(ctx, params, request, _reply) {
    try {
        // Rate limit check
 ...
 * @complexity O(1)
 */
export async function handleOAuthCallbackRequest(ctx, params, query, request, reply) {
    try {
        // Rate limit check
        ...
        if (query.error != null && query.error !== '') {
            throw new OAuthError(query.error_description ?? query.error, provider, `PROVIDER_ERROR_${query.error.toUpperCase()}`);
        }
        ...
        const redirectUri = getCallbackUrl(ctx, provider);
        const result = await handleOAuthCallback(ctx.db, ctx.repos, ctx.config.auth, provider, query.code, query.state, redirectUri);
        if (result.isLinking) {
            // Log successful link event
            const user = request.user;
            if (user != null) {
                await logOAuthLinkSuccessEvent(ctx.db, user.userId, '', // Email not available in link flow
                provider, request.ip, request.headers['user-agent']);
            }
            return {
                status: 200,
        ...
        await logOAuthLoginSuccessEvent(ctx.db, result.auth.user.id, result.auth.user.email, provider, result.auth.isNewUser, request.ip, request.headers['user-agent']);
        ...
        await logOAuthLoginFailureEvent(ctx.db, providerName, errorMessage, undefined, request.ip, request.headers['user-agent']);
        return mapErrorToHttpResponse(error, createErrorMapperLogger(ctx.log));
    }
}
/**
 ...
 * @complexity O(1)
 */
export async function handleOAuthLink(ctx, params, request, _reply) {
    try {
        // Rate limit check
 ...
 * @complexity O(1)
 */
export async function handleOAuthUnlink(ctx, params, request, _reply) {
    try {
        // Rate limit check
        ...
        await logOAuthUnlinkSuccessEvent(ctx.db, user.userId, '', // Email not directly available
        provider, request.ip, request.headers['user-agent']);
        return {
            status: 200,
        ...
        if (userForLog != null) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            await logOAuthUnlinkFailureEvent(ctx.db, userForLog.userId, '', params.provider, errorMessage, request.ip, request.headers['user-agent']);
        }
        return mapErrorToHttpResponse(error, createErrorMapperLogger(ctx.log));
    }
}
 ...
 */
export async function handleGetConnections(ctx, request, _reply) {
    try {
        const user = request.user;
        if (user == null) {
            return {
