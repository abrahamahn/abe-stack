// modules/auth/src/oauth/service.ts
/**
 * OAuth Service
 *
...
import { ConflictError, EmailAlreadyExistsError, NotFoundError, OAuthError, OAuthStateMismatchError, } from '@abe-stack/core';
import { insert, OAUTH_CONNECTIONS_TABLE, toCamelCase, USER_COLUMNS, USERS_TABLE, withTransaction, } from '@abe-stack/db';
import { createAccessToken, createRefreshTokenFamily } from '../utils';
import { createAppleProvider, createGitHubProvider, createGoogleProvider, extractAppleUserFromIdToken, } from './providers';
// ============================================================================
// Token Encryption
// ============================================================================
 ...
 * @complexity O(1)
 */
export function createOAuthState(provider, redirectUri, isLinking, userId) {
    return {
 ...
 * @complexity O(1)
 */
export function encodeOAuthState(state, encryptionKey) {
 ...
 * @complexity O(1)
 */
export function decodeOAuthState(encoded, encryptionKey) {
    try {
 ...
 * @complexity O(1)
 */
export function getProviderClient(provider, config) {
    const providerConfig = config.oauth[provider];
    if (providerConfig == null) {
        throw new OAuthError(`OAuth provider ${provider} is not configured`, provider, 'NOT_CONFIGURED');
    }
    switch (provider) {
        case 'google':
            return createGoogleProvider(providerConfig.clientId, providerConfig.clientSecret);
        case 'github':
            return createGitHubProvider(providerConfig.clientId, providerConfig.clientSecret);
        case 'apple': {
            // Apple requires additional configuration
            const appleConfig = providerConfig;
            if (appleConfig.teamId !== undefined &&
            ...
            throw new OAuthError('Apple OAuth requires teamId, keyId, and privateKey configuration', 'apple', 'INCOMPLETE_CONFIG');
        }
        default: {
            const exhaustiveCheck = provider;
            throw new OAuthError(`Unsupported OAuth provider: ${String(exhaustiveCheck)}`, provider, 'UNSUPPORTED');
        }
    }
}
// ============================================================================
 ...
 * @complexity O(1)
 */
export function getAuthorizationUrl(provider, config, redirectUri, isLinking, userId) {
 ...
 */
export async function handleOAuthCallback(db, repos, config, provider, code, state, redirectUri) {
 ...
 * @complexity O(1) - constant database operations
 */
async function authenticateOrCreateWithOAuth(db, repos, config, provider, userInfo, tokens) {
    const encryptionKey = config.cookie.secret;
    // Check if OAuth connection already exists (using repository)
    const existingConnection = await repos.oauthConnections.findByProviderUserId(provider, userInfo.id);
    if (existingConnection != null) {
        ...
        const { token: refreshToken } = await createRefreshTokenFamily(db, user.id, config.refreshToken.expiryDays);
        return {
            accessToken,
            refreshToken,
            user: {
    ...
    if (existingUser != null) {
        ...
        throw new EmailAlreadyExistsError(`An account with email ${userInfo.email} already exists. Please log in and link your ${provider} account.`);
    }
    ...
    const result = await withTransaction(db, async (tx) => {
        ...
        await tx.execute(insert(OAUTH_CONNECTIONS_TABLE)
            .values({
            user_id: newUser.id,
            provider,
        ...
        const { token: refreshToken } = await createRefreshTokenFamily(tx, newUser.id, config.refreshToken.expiryDays);
 ...
 */
export async function linkOAuthAccount(_db, repos, config, userId, provider, userInfo, tokens) {
    const encryptionKey = config.cookie.secret;
    ...
    if (existingConnection != null) {
        throw new ConflictError(`${provider} is already linked to your account`, 'OAUTH_ALREADY_LINKED');
    }
    if (otherConnection != null) {
        throw new ConflictError(`This ${provider} account is already linked to another user`, 'OAUTH_LINKED_TO_OTHER');
    }
 ...
 * @complexity O(1) - parallel validation queries
 */
export async function unlinkOAuthAccount(_db, repos, userId, provider) {
    // Run all validation queries in parallel (no dependencies between them) - using repositories
    ...
    if (connections.length === 1 && !userHasPassword) {
        throw new ConflictError('Cannot unlink the only authentication method. Please set a password first or link another provider.', 'CANNOT_UNLINK_ONLY_AUTH');
    }
 ...
 */
export async function getConnectedProviders(_db, repos, userId) {
    // Using repository
 ...
 * @complexity O(1)
 */
export async function findUserByOAuthProvider(_db, repos, provider, providerUserId) {
